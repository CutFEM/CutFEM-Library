...
class ShapeOfArray{ protected:
  ...
  { K_throwassert((sub.last())*old.step <= old.last());}
...
class KN :public KN_<R> { public:
  ...
  KN(list<R>& u ) : KN_<R>(new R[u.size()], u.size()){
    int ii=0;
    for(auto it=u.begin();it!=u.end();++it) this->v[ii++] = *it;
  }
  ...
  void init(long nn) {
    this->n=nn;this->step=1;this->next=-1;
    if(this->v) delete [] this->v;
    this->v=new R[nn];fill0(this->v,this->n) ;}
...
class KNM: public KNM_<R>{ public:
  ...
  void destroy(){assert((this->next)<0);
    if(this->next++ ==-1) {
      delete [] this->v; this->v=0;this->n=0;}
  }
  void increment() {assert((this->next)<0);  this->next--;}
};
