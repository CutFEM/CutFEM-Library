...
template<typename GMesh>
void 
BuildAdjacencyOfMesh<GMesh>::addFaceAlreadySeen(typename HashTable<SArray,int>::iterator p, 
					 const SArray& a) {
  ASSERTION(p->v>=0);
...
template<typename T,typename B,typename V>
DataFENodeDF GenericMesh<T,B,V>::BuildDFNumbering(int nbDofOnItem[NbTypeItemElement],
