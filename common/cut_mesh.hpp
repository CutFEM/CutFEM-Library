...
template <typename E> struct Cut_Part {
    ...
    Cut_Part(const Physical_Partition<E> p, int s)
        : pp(p), sign_cut_(s), ip(p.T) {
        partition_ = &pp;
    }
    ...
    CutElement<E> get_element(int k) const {
        return partition_->get_element(k);
    }
...
template <typename Mesh> class ActiveMesh {
  ...
  public:
    ...
    static const int nea =
        Element::nea; //  numbering of adj (4 in Tet,  3 in Tria, 2 in seg)
    static const int nva =
        Element::nva; //  numbering of vertex in Adj hyperface

    const Mesh &Th;

    std::vector<std::vector<int>>
        idx_in_background_mesh_; // [domain][idxK_cutMesh] -> idxK_backMesh
    std::vector<std::map<int, int>>
        idx_from_background_mesh_; // [domain](idxK_backMesh) -> idxK_cutMesh
    std::vector<std::map<std::pair<int, int>,
                         std::vector<std::pair<const Interface<Mesh> *, int>>>>
    ...
    std::vector<std::vector<std::map<int, bool>>>
        in_active_mesh_; // [dom][itq][idx_element] -> true/false
  ...
  private:
    ...
    Physical_Partition<Element> build_local_partition(const int k,
                                                      int t = 0) const;
    Physical_Partition<Face> build_local_partition(Face &face, const int k,
                                                   int ifac, int t = 0) const;
  ...
  public:
    DataFENodeDF BuildDFNumbering(int ndfv, int ndfe, int ndff, int ndft,
        ...
        assert(0);
        return Th.BuildDFNumbering(ndfv, ndfe, ndff, ndft, nndv, nnde, nndf,
                                   nndt, N, PPeriod);
    }
    ...
    Cut_Part<Element> get_cut_part(int k, int t) const {
        ...
        int kb = this->idxElementInBackMesh(k);
        if (it->second.size() == 1)
            return Cut_Part<Element>(it->second.at(0).first->get_partition(kb),
                                     it->second.at(0).second);
        else
            return Cut_Part<Element>(this->build_local_partition(k), 0);
    }

    Cut_Part<typename Element::Face> get_cut_face(Face &face, int k, int ifac,
                                                  int t) const {
        ...
        for (int i = 0; i < Face::nv; ++i)
            iv[i] = Th(kb, Element::nvhyperFace[ifac][i]);
        ...
        if (it->second.size() == 1)
            return Cut_Part<Face>(
                it->second.at(0).first->get_partition_face(face, kb, ifac),
                it->second.at(0).second);
        else
            return Cut_Part<Face>(this->build_local_partition(face, k, ifac),
                                  0);
    }
    ...
    void info() const {
        std::cout << " ------------------------------- " << std::endl;
        std::cout << " Cut Mesh has  \t" << get_nb_domain() << " domains"
                  << std::endl;
        for (int i = 0; i < get_nb_domain(); ++i) {
            std::cout << " nb elements in \t" << i << " => "
                      << this->get_nb_element(i) << std::endl;
        }
        std::cout << " nb elements in total => \t" << this->get_nb_element()
                  << std::endl;
    }

#ifdef USE_MPI
    virtual int first_element() const {
        return MPIcf::first_element(this->get_nb_element());
    }
    virtual int next_element() const {
        return MPIcf::next_element(this->get_nb_element());
    }
    virtual int last_element() const {
        return MPIcf::last_element(this->get_nb_element());
    }
...
void ActiveMesh<Mesh>::init(const Interface<Mesh> &interface) {
    ...
    for (int k = 0; k < Th.nt; ++k) {
        ...
        if (signK.cut()) {
            ...
            interface_id_[0][std::make_pair(0, nt0)].push_back(
                std::make_pair(&interface, 1));
            interface_id_[0][std::make_pair(1, nt1)].push_back(
                std::make_pair(&interface, -1));
...
void ActiveMesh<Mesh>::add(const Interface<Mesh> &interface, int sign_domain) {
    ...
    for (int d = 0; d < dom_size; ++d) {
        ...
        for (auto it_k = idx_from_background_mesh_[d].begin();
             it_k != idx_from_background_mesh_[d].end();) {
            ...
            int nb_interface = (it_gamma == interface_id_[0].end())
                                   ? 0
            ...
            if (signK.sign() == sign_domain || signK.cut()) {
                ...
                for (int i = 0; i < nb_interface; ++i) {
                    interface_id_[0][std::make_pair(new_dom_id, nt[new_dom_id])]
                        .push_back(std::make_pair(old_interface[i], ss[i]));
                }
                ...
                if (!signK.cut()) {
                    ...
                    for (int i = 0; i < nb_interface; ++i) {
                        interface_id_[0][std::make_pair(d, nt[d])].push_back(
                            std::make_pair(old_interface[i], ss[i]));
                    }
                    interface_id_[0][std::make_pair(new_dom_id, nt[new_dom_id])]
                        .push_back(std::make_pair(&interface, sign_domain));
                    interface_id_[0][std::make_pair(d, nt[d])].push_back(
...
template <typename Mesh>
void ActiveMesh<Mesh>::truncate(const Interface<Mesh> &interface,
    ...
    for (int d = 0; d < dom_size; ++d) {
        for (auto it_k = idx_from_background_mesh_[d].begin();
            ...
            // std::cout << "domain \t" << d << " element back " << kb << "\t =>
            // loc id " << k << std::endl;
            auto it_gamma = interface_id_[0].find(std::make_pair(d, k));
            const SignElement<Element> signK = interface.get_SignElement(kb);
            ...
            if (signK.cut()) {
                interface_id_[0][std::make_pair(d, nt[d])].push_back(
                    std::make_pair(&interface, -sign_domain_remove));
            }
            nt[d]++;
            it_k++;
        }
    ...
    for (int d = 0; d < dom_size; ++d) {
        for (auto it_k = idx_from_background_mesh_[d].begin();
            ...
            for (int t = 0; t < interface.size() - 1; ++t) {
                const SignElement<Element> signKi =
                    interface(t)->get_SignElement(kb);
                const SignElement<Element> signKii =
                    interface(t + 1)->get_SignElement(kb);

                if (signKi.cut() || signKii.cut() ||
                    signKi.sign() * signKii.sign() <= 0) {
                    active_element = true;
                    break;
                }
            }
            ...
            for (int it = 0; it < n_tid; ++it) {
                ...
                int nb_interface = (it_gamma == interface_id_[it].end())
                                       ? 0
                                       : it_gamma->second.size();
                std::vector<const Interface<Mesh> *> old_interface(
                    nb_interface);
                std::vector<int> ss(nb_interface);
                for (int i = 0; i < nb_interface; ++i)
                    old_interface[i] = it_gamma->second[i].first;
                ...
                // IS CUT SO NEED TO ADD INTERFACE AND SIGN
                const SignElement<Element> signK =
                    interface(it)->get_SignElement(kb);

                if (signK.cut()) {
                    interface_id_[it][std::make_pair(d, nt[d])].push_back(
                        std::make_pair(interface[it], 0));
                } else {
...
void ActiveMesh<Mesh>::init(const TimeInterface<Mesh> &interface) {
    ...
    for (int k = 0; k < Th.nt; ++k) {
        ...
        for (int t = 0; t < interface.size() - 1; ++t) {
            const SignElement<Element> signKi =
                interface(t)->get_SignElement(k);
            const SignElement<Element> signKii =
                interface(t + 1)->get_SignElement(k);
            s = signKi.sign();
            if (signKi.cut() || signKii.cut() ||
                signKi.sign() * signKii.sign() <= 0) {
                active_element = true;
        ...
        if (active_element) {
            for (int it = 0; it < n_tid; ++it) {
                const SignElement<Element> signK =
                    interface(it)->get_SignElement(k);
                int st = signK.sign();
                if (!signK.cut() && st == -1) {
                ...
                if (signK.cut()) {
                    interface_id_[it][std::make_pair(0, nt0)].push_back(
                        std::make_pair(interface[it], 1));
                    interface_id_[it][std::make_pair(1, nt1)].push_back(
                        std::make_pair(interface[it], -1));
                }
            }
            idx_in_background_mesh_[0].push_back(k);
...
template <typename Mesh>
Physical_Partition<typename Mesh::Element>
ActiveMesh<Mesh>::build_local_partition(const int k, int t) const {
    ...
    for (int i = 0; i < it->second.size(); ++i) {
        ...
        for (auto it = erased_element.begin(); it != erased_element.end();
             ++it) {
        ...
        for (auto it = new_element_idx.begin(); it != new_element_idx.end();
             ++it) {
...
template <typename Mesh>
Physical_Partition<typename Mesh::Element::Face>
ActiveMesh<Mesh>::build_local_partition(Face &face, const int k, int ifac,
                                        int t) const {
