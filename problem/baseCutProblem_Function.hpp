...
template<typename M>
void BaseCutFEM<M>::addElementContribution(const ListItemVF<Rd::d>& VF, const int k, const TimeSlab* In, int itq, double cst_time) {
...
template<typename M>
void BaseCutFEM<M>::addElementContributionOtherSide(const ListItemVF<Rd::d>& VF, const int k, const TimeSlab* In, int itq, double cst_time) {
...
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const CFacet& b) {
  ...
  for(int k=Th.first_element(); k<Th.last_element(); k+= Th.next_element()) {
    for(int ifac = 0; ifac < Element::nea; ++ifac) {    //loop over the edges / faces
      ...
      if(Th.isCutFace(k, ifac, 0)) BaseCutFEM<M>::addFaceContribution(VF, e1, e2, nullptr, 0,1.);
      else BaseFEM<M>::addFaceContribution(VF, e1, e2, nullptr, 0, 1.);
    }
    this->addLocalContribution();
  }
}
...
template<typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const CFacet& b, const TimeSlab& In, int itq) {
  assert(!VF.isRHS());
  ...
  for(int k=Th.first_element(); k<Th.last_element(); k+= Th.next_element()) {
    ...
    for(int ifac = 0; ifac < Element::nea; ++ifac) {    //loop over the edges / faces
      ...
      std::pair<int,int> e2 = std::make_pair(kn,jfac);
      // CHECK IF IT IS A CUT EDGE
      if(Th.isCutFace(k, ifac, itq)) BaseCutFEM<M>::addFaceContribution(VF, e1, e2 , &In, itq, cst_time);
      else BaseFEM<M>::addFaceContribution(VF, e1, e2, &In, itq, cst_time);
    }
    this->addLocalContribution();
  }
}
...
template<typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const CFacet& b, const TimeSlab& In, int itq) {
  assert(VF.isRHS());
...
template<typename M>
void BaseCutFEM<M>::addFaceContribution(const ListItemVF<Rd::d>& VF, const std::pair<int,int>& e1, const std::pair<int,int>& e2, const TimeSlab* In, int itq, double cst_time)  {
...
template<typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d>& VF, const CutMesh& cutTh, const CBorder& b,list<int> label) {
  assert(!VF.isRHS());
  ...
  for(int idx_be=cutTh.first_boundary_element(); idx_be<cutTh.last_boundary_element(); idx_be+= cutTh.next_boundary_element()) {
    ...
    if(util::contain(label, BE.lab) || all_label) {
      ...
      // CHECK IF IT IS A CUT EDGE
      if(cutTh.isCutFace(idxK[0], ifac, 0)) BaseCutFEM<M>::addBorderContribution(VF, K, BE, ifac, nullptr, 0, 1.);
      else {
        if(idxK.size() == 1){
...
template<typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d>& VF, const CutMesh& cutTh, const CBorder& b,list<int> label) {
  assert(VF.isRHS());
  bool all_label = (label.size() == 0);
...
template<typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const CBorder& b, const TimeSlab& In,list<int> label) {
  for(int itq=0;itq<this->get_nb_quad_point_time();++itq) {
...
template<typename M>
void BaseCutFEM<M>::addBorderContribution(const ListItemVF<Rd::d>& VF, const Element& K,const BorderElement& BE, int ifac, const TimeSlab* In, int itq, double cst_time)  {
...
void BaseCutFEM<Mesh>::setDirichlet(const FunFEM<Mesh>& gh, const CutMesh& cutTh, list<int> label) {
      ...
      for( int ic=0; ic<Vh.N;++ic) {
        for( int df=FK.dfcbegin(ic);df<FK.dfcend(ic);++df){
          ...
          if(id_item < K.nv){
            ...
            df2set.insert({df_glob, gh(df_glob)});

          }
          else {
            // std::cout << " on face  " << FK.DFOnWhat(df) << std::endl;
          }
          // std::cout << FK.DFOnWhat(df) << std::endl;
          // std::cout << FK.NodeOfDF(df) << std::endl;
...
template<typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d>& VF, const TimeInterface<M>& interface, const CRidge& innerRidge, const TimeSlab& In, int itq, list<int> label) {
  assert(!VF.isRHS());
  bool all_label = (label.size() == 0);
  auto tq = this->get_quadrature_time(itq);
  double tid = In.map(tq);
...
template<typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d>& VF, const TimeInterface<M>& gamma, const CRidge& innerRidge, const TimeSlab& In, list<int> label) {
  for(int itq=0;itq<this->get_nb_quad_point_time();++itq) {
    addLinear(VF, gamma, innerRidge, In, itq, label);
  }
}
template<typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d>& VF, const TimeInterface<M>& interface, const CRidge& innerRidge, const TimeSlab& In, int itq, list<int> label) {
  assert(VF.isRHS());
  bool all_label = (label.size() == 0);
  auto tq = this->get_quadrature_time(itq);
  double tid = In.map(tq);
...
void BaseCutFEM<M>::addFaceStabilization(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const TimeSlab& In, int itq) {
    ...
    for(int k=Th.first_element(); k<Th.last_element(); k+= Th.next_element()) {
        //std::cout << !Th.isStabilizeElement(k) << std::endl;
        if(!Th.isStabilizeElement(k)) continue;
        for(int ifac = 0; ifac < Element::nea; ++ifac) {    //loop over the edges / faces
    ...
    //std::cout << "Number of stabilized edges:\t" << num_stab_edges << std::endl;
}

// template<typename M>
// int BaseCutFEM<M>::num_stabilized_edges(const CutMesh& Th, const TimeSlab& In) {
//     int num_stab_elems = 0;
...
//               }
//         }
//         return num_stab_elems;
//     }
    
// }

// template<typename M>
// void BaseCutFEM<M>::addFaceStabilization2(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const TimeSlab& In) {
//     for(int itq=0;itq<this->get_nb_quad_point_time();++itq) {
//         addFaceStabilization2(VF, Th, In, itq);
//     }
// }


// template<typename M>
// void BaseCutFEM<M>::addFaceStabilization2(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const TimeSlab& In, int itq) {
//     assert(!VF.isRHS());
...
//             std::pair<int,int> e1 = std::make_pair(k,ifac);
//             std::pair<int,int> e2 = std::make_pair(kn,jfac);
//             BaseFEM<M>::addFaceContribution(VF, e1, e2, nullptr, itq, cst_time);
...
template<typename M>
void BaseCutFEM<M>::addFaceStabilization(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const TimeSlab& In, const TimeMacroElementSurface<M>& macro) {
...
template<typename M>
void BaseCutFEM<M>::addLagrangeMultiplier(const ListItemVF<Rd::d>& VF, double val, const CutMesh& cutTh,const CBorder& b,list<int> label) {
  assert(VF.isRHS());
  bool all_label = (label.size() == 0);
  ...
  for(int idx_be=cutTh.first_boundary_element(); idx_be<cutTh.last_boundary_element(); idx_be+= cutTh.next_boundary_element()) {
    ...
    if(util::contain(label, BE.lab) || all_label) {
      ...
      // CHECK IF IT IS A CUT EDGE
      if(cutTh.isCutFace(idxK[0], ifac, 0)) BaseCutFEM<M>::addLagrangeBorderContribution(VF, K, BE, ifac, nullptr, 0, 1.);
      else BaseFEM<M>::addLagrangeBorderContribution(VF, K, BE,ifac, nullptr, 0, 1.);
      this->addLocalContributionLagrange(ndf);
    }
  }
}
...
template<typename M>
void BaseCutFEM<M>::addLagrangeContribution(const ListItemVF<Rd::d>& VF, const int k) {
...
template<typename M>
void BaseCutFEM<M>::addLagrangeBorderContribution(const ListItemVF<Rd::d>& VF, const Element& K,const BorderElement& BE, int ifac, const TimeSlab* In, int itq, double cst_time)  {
...
template<typename M>
void BaseCutFEM<M>::addLagrangeMultiplier(const ListItemVF<Rd::d>& VF, double val, const CutMesh& Th, const CExtension& ext, const int epsE) {
  assert(VF.isRHS());
  int ndf = this->rhs_.size();
  this->rhs_.resize(ndf+1);
  this->rhs_(ndf) = val;
...
// INITIALIZE SOLUTION
template<typename M>
void BaseCutFEM<M>::initialSolution(Rn& u0) {
...
template<typename M>
void BaseCutFEM<M>::saveSolution(const Rn& sol) {

  this->mapU0_.clear();
  int id_domain_0 = 0;
  int nbTime = this->get_nb_dof_time();

  for (typename std::map<const FESpace*,int>::const_iterator q=this->mapIdx0_.begin(); q != this->mapIdx0_.end(); ++q) {
    const FESpace& Wh     = *q->first;
    const int n0          =  q->second;
    const ActiveMesh<M>& Th(Wh.get_mesh());
    const FESpace& backVh = Wh.get_back_space();
