...
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &interface,
                                const CRidge &innerRidge, const TimeSlab &In, int itq, list<int> label) {
    assert(!VF.isRHS());
    bool all_label = (label.size() == 0);
    auto tq        = this->get_quadrature_time(itq);
    double tid     = In.map(tq);
...
void BaseCutFEM<M>::addFaceStabilization(const ListItemVF<Rd::d> &VF, const CutMesh &Th, const TimeSlab &In) {
    ...
    // std::cout << number_of_stabilized_edges << "\t" << number_of_quadrature_points << std::endl;
...
// template<typename M>
// void BaseCutFEM<M>::addFaceStabilization2(const ListItemVF<Rd::d>& VF, const CutMesh& Th, const TimeSlab& In, int
// itq) {
//     assert(!VF.isRHS());
...
template <typename M> void BaseCutFEM<M>::addLagrangeContribution(const ListItemVF<Rd::d> &VF, const int k) {
