...
template <typename Mesh> class BaseCutFEM : public BaseFEM<Mesh> {
  ...
  public:
    ...
    // void addInterfaceContribution(const ListItemVF<Rd::d>& VF, const Interface<Mesh>& gamma, int ifac, double tid,
...
template <typename M> class BaseCutProblem : public BaseProblem<M> {
  public:
    ...
    BaseCutProblem(const QuadratureFormular1d &qT, int orderSpace = 5) : BaseProblem<M>(qT, orderSpace) {}
  ...
  public:
    // FEM on surface
    void addBilinear(const ListItemVF<Rd::d> &VF, const Interface &gamma, list<int> label = {},
                     const Mapping &mapping = DataMapping<Mesh>::Id) {
        return BaseProblem<M>::addBilinear(VF, gamma, label, mapping);
    };
    void addBilinear(const ListItemVF<Rd::d> &VF, const Interface &gamma, const GMacro &macro, list<int> label = {},
                     const Mapping &mapping = DataMapping<Mesh>::Id) {
    ...
    void addLinear(const ListItemVF<Rd::d> &VF, const Interface &gamma, list<int> label = {},
                   const Mapping &mapping = DataMapping<Mesh>::Id) {
    ...
    void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, const Interface &gamma, double val,
                               const Mapping &mapping = DataMapping<Mesh>::Id) {
        return BaseProblem<M>::addLagrangeMultiplier(VF, gamma, val, mapping);
    };
    void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, const Interface &gamma, const TimeSlab &In, double tq,
                               double val, const Mapping &mapping = DataMapping<Mesh>::Id) {
        return BaseProblem<M>::addLagrangeMultiplier(VF, gamma, In, tq, val, mapping);
    };

    // SpaceTime FEM on surface
    void addBilinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, const TimeSlab &In,
                     KN<const Mapping *> mapping = {}) {
        return BaseProblem<M>::addBilinear(VF, gamma, In, mapping);
    };
    void addBilinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, int itq, const TimeSlab &In,
                     KN<const Mapping *> mapping = {}) {
        return BaseProblem<M>::addBilinear(VF, gamma, itq, In, mapping);
    };
    void addLinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, const TimeSlab &In,
                   KN<const Mapping *> mapping = {}) {
        return BaseProblem<M>::addLinear(VF, gamma, In, mapping);
    };
    void addLinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, int itq, const TimeSlab &In,
                   KN<const Mapping *> mapping = {}) {
        return BaseProblem<M>::addLinear(VF, gamma, itq, In, mapping);
    };
    void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, const TimeSlab &In,
...
/*
          Add Bilinear Form
*/
template <typename M> void BaseCutProblem<M>::addBilinear(const ListItemVF<Rd::d> &VF) {
...
template <typename M> void BaseCutProblem<M>::addBilinearFormExtDomain(const ListItemVF<Rd::d> &VF, const R epsE) {
    bool extend = true;
    ...
    for (int k = Vh->first_element(); k < Vh->last_element(); k += Vh->next_element()) {
        ...
        if (Vh->isCut(k)) {   // [makes sure we extend only cut elements]
            addElementMat(
                VF, k, extend,
                epsE); // [add epsilon worth of other side, the supplied arguments limits to only cut elements]
        }
    }
}
...
template <typename M> void BaseCutProblem<M>::addBilinear(const ListItemVF<Rd::d> &VF, const MacroElement &macro) {
    for (auto it = macro.macro_element.begin(); it != macro.macro_element.end(); ++it) {
        for (int i = 0; i < it->second.idx_element.size(); ++i) {
            int k = it->second.idx_element[i];
            addElementMat(VF, k);
...
void BaseCutProblem<M>::addElementMat(const ListItemVF<Rd::d> &VF, const int k, const bool extend, const R epsE) {
    ...
    for (typename Partition::const_element_iterator it = cutK.element_begin(domPart); it != cutK.element_end(domPart);
         ++it) {
        ...
        for (int l = 0; l < VF.size(); ++l) {
            ...
            RNMK_ fu(this->databf + (same ? 0 : FKv.NbDoF() * FKv.N * lastop), FKu.NbDoF(), FKu.N,
            ...
            for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
                ...
                for (int j = FKu.dfcbegin(VF[l].cu); j < FKu.dfcend(VF[l].cu); ++j) {
                    for (int i = FKv.dfcbegin(VF[l].cv); i < FKv.dfcend(VF[l].cv); ++i) {
                        // (*this)(FKv.loc2glb(i),FKu.loc2glb(j)) +=  Cint * coef * VF[l].c *
                        // fv(i,VF[l].cv,VF[l].dv)*fu(j,VF[l].cu,VF[l].du)*val;
                        this->addToLocalContribution(FKv.loc2glb(i), FKu.loc2glb(j)) +=
                            Cint * coef * VF[l].c * fv(i, VF[l].cv, VF[l].dv) * fu(j, VF[l].cu, VF[l].du) * val;
                    }
                }
            }
        }
...
void BaseCutProblem<M>::addElementMatEdge(const ListItemVF<Rd::d> &VF, const int k, const int ifac) {
    ...
    for (int l = 0; l < VF.size(); ++l) {
        ...
        R coef =
            BaseCutProblem<M>::computeCoefEdge(VF[l], h, meas, measK, meas_cut, the_domain) * VF[l].getCoef(normal);
...
*/
template <typename M> void BaseCutProblem<M>::addLinear(const ListItemVF<Rd::d> &VF) {
    for (int k = Vh->first_element(); k < Vh->last_element(); k += Vh->next_element()) {
        // std::cout << k << std::endl;
        addElementRHS(VF, k);
    }
...
template <typename M> void BaseCutProblem<M>::addLinearFormExtDomain(const ListItemVF<Rd::d> &VF, const R epsE) {
    bool extend = true;
    ...
    for (int dom = 0; dom < 2; ++dom) {
        ...
        for (int l = 0; l < VF.size(); ++l) {
            ...
            for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
                ...
                for (int i = FKv.dfcbegin(VF[l].cv); i < FKv.dfcend(VF[l].cv); ++i) {
                    for (int j = FKu.dfcbegin(VF[l].cu); j < FKu.dfcend(VF[l].cu); ++j) {
                        // (*this)(FKv.loc2glb(i),FKu.loc2glb(j)) +=  Cst * fv(i,VF[l].cv,VF[l].dv) *
...
Add Stabilization
*/
template <typename M> void BaseCutProblem<M>::addFaceStabilization(const ListItemVF<Rd::d> &VF) {
    typedef typename Mesh::Element Element;
...
template <typename M> void BaseCutProblem<M>::saveSolution(const Rn &sol) {
    ...
    for (typename std::map<const FESpace *, int>::const_iterator q = this->mapIdx0.begin(); q != this->mapIdx0.end();
         ++q) {
