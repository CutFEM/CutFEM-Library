  ...
  for(int k=Vh.first_element(); k<Vh.last_element(); k+= Vh.next_element()) {
    ...
    it != cutK.element_end(); ++it){
      ...
      for(int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq)  {
        ...
        FK.BF(FK.T.toKref(mip), fu); // need point in local reference element
        ...
        if(ODE_method == "RK2") {
          ...
          R normDupHalf = (fabs(dphiHalf.norm())<1e-14)? 1e-9 : dphiHalf.norm();

          // RK2
          for(int i = FK.dfcbegin(0); i < FK.dfcend(0); ++i) {
            for(int j = FK.dfcbegin(0); j < FK.dfcend(0); ++j) {
              (*this)(FK.loc2glb(i),FK.loc2glb(j)) +=  Cint * (
                (1./dt)*fu(j,0,op_id)*(fu(i,0,op_id) + Tsd * (w.x*fu(i,0,op_dx) + w.y*fu(i,0,op_dy)))
            ...
            *(fu(i,0,op_id) + Tsd * (w.x*fu(i,0,op_dx) + w.y*fu(i,0,op_dy)))
            ;
          }
        }
      ...
      for(int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq)  {
        ...
        for(int i = FK.dfcbegin(0); i < FK.dfcend(0); ++i) {
          for(int j = FK.dfcbegin(0); j < FK.dfcend(0); ++j) {
            (*this)(FK.loc2glb(i),FK.loc2glb(j)) +=  Cint * (
              (1./dt)*fu(j,0,op_id)
              *(fu(i,0,op_id) + Tsd * (w.x*fu(i,0,op_dx) + w.y*fu(i,0,op_dy)+ w.z*fu(i,0,op_dz)))
              + epsilon_diffusion*fu(i,0,op_dx)*fu(j,0,op_dx)
              + epsilon_diffusion*fu(i,0,op_dy)*fu(j,0,op_dy)
          ...
          *(fu(i,0,op_id) + Tsd * (w.x*fu(i,0,op_dx) + w.y*fu(i,0,op_dy)+ w.z*fu(i,0,op_dz)))
          ...
          // + Cint*(- (w,dup) + ff)*(fu(i,0,op_id) + Tsd * (w.x*fu(i,0,op_dx) + w.y*fu(i,0,op_dy)+ w.z*fu(i,0,op_dz)))
          ;
        }
      }
    }
