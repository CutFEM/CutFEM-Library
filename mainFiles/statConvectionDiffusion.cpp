...
static double fun_kappa_E1(int i, double hh, double meas, double measK, double meas_Cut) {
    return meas_Cut/measK;
}

static double fun_kappa_E2(int i, double hh, double meas, double measK, double meas_Cut) {
    return 1-meas_Cut/measK;
}

static double fun_lambdaA (int i, double hh, double meas, double measK, double meas_Cut) {

   double Cs  = 2;
   double Cs2 = 0.6;
   double GammaE = meas;
   double aL = measK;        // area of triangle

   double Ai = (CutFEM_ParameterList::find("A"))? CutFEM_ParameterList::listParameter["A"]->expression(i): -1;
   assert(Ai > 0);
   return Cs*GammaE*Ai/aL + Cs2/hh;
}
...
void errorAnalysis(int argc, char** argv) {
    ...
    const int nms = 10;
    //int mesh_sizes[nms] = {4,8, 12, 16, 24,32, 48,64, 96,112,
    //                     128, 164, 192,216, 232, 276,
    //                   302,324, 384,400, 420};
    //int mesh_sizes[nms];
    //double errors_bulk[nms];
    //double errors_interface[nms];

    vector<int> mesh_sizes;
    vector<double> errors_bulk;
    vector<double> errors_interface;

    // Which variant of interface edge form is used
    string varName("Variant2");       // Variant name

    std::cout << " ....... Doing " << varName << " ........." << std::endl;

    for (int i = 0; i < nms; i++) {
        //std::cout << n << ", ";
        //mesh_sizes[i] = 4 + 20*i;
        mesh_sizes.push_back(10+20*i);

        // MESH AND PROBLEM PARAMETERS
        // =====================================================

        int nx = mesh_sizes.at(i), ny = mesh_sizes.at(i);
        double lx = 3., ly = 3.;
        ...
        // Penalty parameters
        double tau_a0 = 1e0, tau_a1 = 1e1, tau_a2 = 1e1;

        // OLD PARAMETERS
//        double tau_a0 = 1e5, tau_b0 = 0.1;
//        double tau_a1 = 1e5, tau_b1 = 0;
//        double tau_a2 = 1e5, tau_b2 = 0;

        // MACRO PARAMETERS
        double tau_b0   = 0, tau_b1 = 0, tau_b2 = 0;
        double lambdaB  = A1*1e1/h;    // coefficient on the outer boundary
        double lambdaA0 = A0*5e3/h;   // for the surface problem
        // WANT
        //double lambdaB = A1*1e1/h;    // coefficient on the outer boundary
        //double lambdaA0 = A0*1e1/h;   // for the surface problem

        // Variant 3 parameters
        //double tau_b0 = 1, tau_b1 = 0, tau_b2 = 0;

        // Local bulk penalty parameters
        CutFEM_Parameter Cs("Cs",2e0,6e-1);
        CutFEM_Parameter A("A", A1, A2);
        CutFEM_Parameter penalty1("lambdaA", fun_lambdaA);

        // Global bulk penalty parameter (if this is commented, the three lines above should be out-commented
...
//                + average(innerProduct((vel*t)*u0, v0),1,1)
//                + innerProduct(average((vel*t)*u0,1,1), average(v0, lambda1,-1-lambda1))
//                , interface
//                , nodeEvaluation
//        );
...
//            - innerProduct(jump(u0), average(A0*gradS(v0)*t, 0.5,-0.5))    // (3.13)/(3.82) added for symmetry
//            + innerProduct(tau_a0*A0/h*jump(u0),jump(v0))                             // (3.13)/(3.82)
//            + innerProduct(average(u0), average((vel*t)*v0,1,1))*0.5       // (3.15)/(3.84)
//            - innerProduct(average((vel*t)*u0,1,1),    average(v0))*0.5    // (3.15)/(3.84)
//            + innerProduct(penalty0div*average((vel*t)*u0,1,1),
...
//    );
        ...
        matlab::Export(convdiff.mat, "../../OutputFiles/statConvectionDiffusion/matrices/mat_n_" +
        to_string(nx) + "_" + varName + "MacroLocalWeightsAndPen.dat");


        // Write indices of bulk and surface to csv file

        std::ofstream myfile;
        myfile.open ("../../OutputFiles/statConvectionDiffusion/matrices/indices_n_" + to_string(nx)
        + varName + "MacroLocalWeightsAndPen.csv");
        myfile << idx_s0 << ", " << idx_intf << "\n";
        myfile.close();
        ...
        // Compute L2 error and add to array

        errors_bulk.push_back(L2normCut(uh, fun_uBulk   , 0,1));
        errors_interface.push_back(L2normSurf(us, fun_uSurfaceT, 0,0,1));

        std::cout << "error bulk = " << errors_bulk.back() << std::endl;
        std::cout << "error interface = " << errors_interface.back() << std::endl;
    ...
    //matlab::Export(mesh_sizes,"../../outputFiles/statConvectionDiffusion/mesh_sizes" + varName + ".dat");

    // Concatenate the two errors vectors and write them
    errors_bulk.insert(errors_bulk.end(),errors_interface.begin(),errors_interface.end());
    matlab::Export(errors_bulk, "../../outputFiles/statConvectionDiffusion/errors" + varName + "MacroLocalWeightsAndPen.dat");
...
//    double lambdaA0 = A0*1e1/h;   // for the surface problem
    ...
    CutFEM_Parameter Cs("Cs",2e0,6e-1);
    CutFEM_Parameter A("A", A1, A2);
    CutFEM_Parameter penalty1(Parameter::lambdaA);
...
//    gnuplot::save(Th, "../../outputFiles/statConvectionDiffusion/gnuplot/Th.dat");
//    gnuplot::save(cutTh, "../../outputFiles/statConvectionDiffusion/gnuplot/cutTh.dat");
//    gnuplot::save(interface, "../../outputFiles/statConvectionDiffusion/gnuplot/interface.dat");
//    gnuplot::save(Wh,0,"../../outputFiles/statConvectionDiffusion/gnuplot/Vh1.dat");
//    gnuplot::save(Wh,1,"../../outputFiles/statConvectionDiffusion/gnuplot/Vh2.dat");
