...
namespace TestMergingDrop {
  R fun_levelSet1(const R2 P) { return sqrt((P.x-0.5)*(P.x-0.5) + (P.y-0.0)*(P.y-0.0)) - 0.3;}
  R fun_levelSet2(const R2 P) { return sqrt((P.x+0.5)*(P.x+0.5) + (P.y-0.0)*(P.y-0.0)) - 0.3;}

  R fun_levelSet(const R2 P, const int i) {
    if(P.x > 0) return fun_levelSet1(P);
    else return fun_levelSet2(P);
  }
  R fun_velocity(const R2 P, const int i){
    if(i == 0) return -0.5*(1+cos(M_PI*P.x))*sin(M_PI*P.y);
    else       return  0.5*(1+cos(M_PI*P.y))*sin(M_PI*P.x);
  ...
  R fun_init_bulk(const R2 P, int i) {
    double xc = (P.x > 0)? 0.5 : -0.5;
    double r = sqrt((P.x-xc)*(P.x-xc) + P.y*P.y);
    if(r > 1.5*0.3) return 0.5*(1-P.x*P.x)*(1-P.x*P.x);
    else if(0.3 < r && r <= 1.5*0.3) return 0.5*(1-P.x*P.x)*(1-P.x*P.x)*fun_w(r);
    else return 0.;
  }
...
using namespace TestSaraExample3;
// using namespace TestMergingDrop;
...
int main(int argc, char** argv )
{
  ...
  vector<Fun2_h> ls_k(nbTime), ls(nbTime);
  ...
  while( iter < GTime::total_number_iteration ) {
    ...
    for(int i=0;i<nbTime;++i) {
      ...
      if(i<nbTime-1) {
        LevelSet2 levelSet(ls_k[i], vel, vel, dt_levelSet);
...
*/
    Normal n;
    TestFunction2 ub(Wh,1), vb(Wh,1);           // bulk function
    TestFunction2 us(cutWh,1), vs(cutWh,1);     // surface function
    ...
    Rn data_u0(bulkSurface.nDoF, 0.);
    Rn_ data_b0(data_u0(SubArray(Wh.NbDoF()   ,0)));
    Rn_ data_s0(data_u0(SubArray(cutWh.NbDoF(),idx_s0)));
    ...
    Fun2_h b0h(Wh   , data_b0);
    Fun2_h s0h(cutWh, data_s0);

    // set uh for newton
    Rn data_uh(data_u0);
    Rn_ data_bh(data_uh(SubArray(Wh.NbDoF()*In.NbDoF(),0)));
    Rn_ data_sh(data_uh(SubArray(cutWh.NbDoF()*In.NbDoF(),idx_s0)));
    Fun2_h bh(Wh   , In, data_bh);
    Fun2_h sh(cutWh, In, data_sh);
    ...
    //   // );
      ...
      Rn_ dw(bulkSurface.rhs(SubArray(bulkSurface.nDoF, 0)));
      data_uh -= dw;
    ...
    if(iter%frequencyPlottingTheSolution == 0 && writeVTKFiles && MPIcf::IamMaster()){
      ...
      if(saveSurfactantVTK) {
        Paraview2 writer(Wh, ls[0], pathOutpuFigure+"bulk_"+to_string(iterfig)+".vtk");
