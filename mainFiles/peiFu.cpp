...
// getchar();
  ...
  for(int i=0;i<niteration;++i) {
    ...
    //
    // uh_tild = uh;
    // limiter::limiter_Pei_P1(fun_uh, uh_tild, min_u0, max_u0, macro);
    ...
    u1 += dt * uh;
    u1_tild = u1;
    limiter::limiter_Pei_P1(fun_u1, u1_tild, min_u0, max_u0, macro);
    ...
    u2_tild = u2;
    limiter::limiter_Pei_P1(fun_u2, u2_tild, min_u0, max_u0, macro);
    ...
    limiter::minmaxP1(fun_uh, min_uh, max_uh);
    double min_u1, max_u1;
    limiter::minmaxP1(fun_uh_tild, min_u1, max_u1);
    ...
    if( min_u0 < min_u1 && max_u1 < max_u0) {
      ...
      if(MPIcf::IamMaster()) {
        Paraview<Mesh> writer(Khi, "maxPrinciple"+to_string(ifig++)+".vtk");
        writer.add(fun_uh, "uhNoLimiter", 0, 1);
        writer.add(fun_u1, "uhLimiter", 0, 1);
        // writer.add(fun_uM, "macroExtend", 0, 1);
      }
...
// Example to check macro extension P1dc
#ifdef EXAMPLE4
double c0 = 0.25;
R fun_levelSet(const R2 P, const int i) {
  return -P.x - P.y + c0;
...
int main(int argc, char** argv ) {
  ...
  for(int i=0;i<niteration;++i) {
    ...
    u1 = uh;
    limiter::extendToMacroP0(fun_uh, u1, macro);
    ...
    // Fun2_h femSolh(Wh, uh);
    // Expression2 femSol(femSolh, 0, op_id);
    double quh = integral(Khi, fun_uh, 0);
    double qu = integral(Khi, fun_u1, 0);
    std::cout << setprecision(16) << quh << "\t" << qu << std::endl;
    outputData << setprecision(16) << quh << "\t" << qu << std::endl;
