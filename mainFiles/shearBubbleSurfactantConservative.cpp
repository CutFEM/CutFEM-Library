...
namespace TestBenchmark2D {
  R fun_levelSet(const R2 P, const int i) { return sqrt(P.x*P.x + P.y*P.y) - 1.;}
...
int main(int argc, char** argv )
{
  ...
  GTime::total_number_iteration = 12./dT;
  dT = 12. / GTime::total_number_iteration;
  GTime::time_step = dT;
  ...
  for(int i=0;i<nbTime;++i) vel[i].init(VelVh, fun_boundary);
  ...
  // auto ftest = [](R2 a) { return a.x < a.y; };
  // levelSet.test(ftest);
  // getchar();
...
*/
    ...
    if(iter == 0) {
      interpolate(Wh, datauu, fun_boundary);
