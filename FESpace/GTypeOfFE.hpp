...
public:
   ...
   * (not for union of FE)
   */
  GTypeOfFE(const int nbdf, const int NN, const int* data,
  ...
  GTypeOfFE(const KN<GTypeOfFE<Mesh> const *> &t)
    :
    dataTypeOfFE(Element::nitemdim,t),
  ...
  //  *
  //  */
  // GTypeOfFE(const KN<GTypeOfFE<Mesh> const *> &t, const GTypeOfFE<Mesh1>* tt)
  //   :
  ...
  virtual void FB(const What_d whatd, const Element & K,const Rd &P,
		  KNMK_<R> & val) const = 0;
  virtual void FB(const What_d whatd, const Element & K,const Rd &P,
		  KNMK_<R> & val, const KNM_<R>& J) const {assert(0);};
