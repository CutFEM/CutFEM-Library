...
template <typename M> std::list<ExpressionFunFEM<M>> FunFEM<M>::expression(int n) const {
    if (n == -1)
        n = Vh->N;
    assert(n <= Vh->N);
...
template <typename M> std::list<ExpressionFunFEM<M>> FunFEM<M>::expression(int n, int i0) const {
    // if(n == -1) n = Vh->N;
    assert(n <= Vh->N);
    std::list<ExpressionFunFEM<Mesh>> l;
    for (int i = 0; i < n; ++i) {
