...
template<> void ParaviewCut<Mesh2>::ParaviewMesh::build(const ActiveMesh<Mesh>&  Vh) {
  return buildNoCut(Vh);
}
template<> void ParaviewCut<MeshQuad2>::ParaviewMesh::build(const ActiveMesh<Mesh>&  Vh) {
  return buildNoCut(Vh);
}
template<> void ParaviewCut<MeshHexa>::ParaviewMesh::build(const ActiveMesh<Mesh>&  Vh) {
  return buildCut(Vh);
}
template<> void ParaviewCut<Mesh3>::ParaviewMesh::build(const ActiveMesh<Mesh>& Vh) {
  return buildCut(Vh);
}
