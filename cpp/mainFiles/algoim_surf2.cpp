...
int main(int argc, char **argv) {
    ...
    for (int j = 0; j < iterations; ++j) {
        ...
        while (iter < total_number_iteration) {
            ...
            AlgoimInterface<Mesh,Levelset<2>>* interface0 = dynamic_cast<AlgoimInterface<Mesh,Levelset<2>>*>(interface(0));
            // AlgoimInterface<Mesh, Levelset<2>> interface0(*interface(0));

            // Create active meshes
            ActiveMesh<Mesh> ThGamma(Th);
            ActiveMesh<Mesh> ThGamma0(Th);

            ThGamma.createSurfaceMesh(interface);
            ThGamma0.createSurfaceMesh(*interface0);

            CutSpace Wh(ThGamma, Vh);
            CutSpace Wh0(ThGamma0, Vh);

            AlgoimCutFEM<Mesh, Levelset<2>> initial_condition(Wh0, phi);
            ...
            Rn datau0(surfactant.get_nb_dof(), 0.);
            surfactant.initialSolution(datau0);
            KN_<double> datas0(datau0(SubArray(Wh0.get_nb_dof(), 0)));
            if (iter == 0)
                interpolate(Wh0, datas0, fun_init_surfactant);  
            
            Fun_h u0(Wh0, datas0);
            
            FunTest s(Wh0, 1), r(Wh0, 1);
            initial_condition.addBilinearAlgoim(+innerProduct(s, r), *interface0);
            initial_condition.addFaceStabilization(+innerProduct(0.01 * h * jump(grad(s) * n), jump(grad(r) * n)),
                                                   ThGamma0);
            initial_condition.addLinearAlgoim(+innerProduct(u0.expr(), r), *interface0);
            initial_condition.solve("mumps");
            datas0 = initial_condition.rhs_;


            if (iter == 0) {
                Paraview<Mesh> writer(ThGamma0, path_figures + "surfactant_initial" + ".vtk");
                Fun_h uS_ex(Wh0, fun_sol_surfactant, 0);
            ...
            surfactant.addBilinear(-innerProduct(u, dt(v)) + innerProduct(D * gradS(u), gradS(v)) -
...
// classical scheme
            ...
            surfactant.addBilinear(+innerProduct(dt(u), v) + innerProduct(D * gradS(u), gradS(v)) +
            ...
            surfactant.addBilinear(
                +innerProduct(tau2 * h * h * grad(u) * n, grad(v) * n) +
                    innerProduct(tau2 * h * h * h * h * grad(grad(u) * n) * n, grad(grad(v) * n) * n),
                interface, In);
                ...
                if (iterations == 1) {
                    ...
                    writer.writeAlgoimQuadrature(ThGamma, phi, In, qTime, 2, 2,
                                                 path_figures + "algoim_quadrature_2_" + std::to_string(iter + 1) +
                                                     ".vtk");
                    writer.writeActiveMesh(ThGamma0, path_figures + "ThGamma0_" + std::to_string(iter+1) + ".vtk");
                }
