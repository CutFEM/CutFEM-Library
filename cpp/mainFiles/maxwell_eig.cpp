...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        */
        ...
        // Eq 1
        maxwell3D.addBilinear( // w = curl u
            +innerProduct(1. / mu * w, tau), Khi);

        matlab::Export(maxwell3D.mat_[0], "mat_M.dat");

        maxwell3D.cleanBuildInMatrix();
        maxwell3D.addBilinear(-innerProduct(u, curl(tau)), Khi);
        matlab::Export(maxwell3D.mat_[0], "mat_W.dat");
