...
// #define PROBLEM_UNFITTED_CORIOLIS_STOKESRT
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 4; ++i) { // i<3 umfpack..
        ...
        const R penaltyParam = 1e2 / hi; // 1e1/pow(hi,3);
        ...
        */
...

        {
            ...
            stokes.addLinear(+innerProduct(gh.expression(2), grad(v) * n) +
                             ...
                             ,
                             Th, INTEGRAL_BOUNDARY);
            // Space Rh(Th, DataFE<Mesh>::P2); Fun_h pph(Rh, fun_exact_p); ExpressionFunFEM<Mesh> tru_p(pph,0,op_id);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        */
        ...
        matlab::Export(stokes.mat_, "mat" + to_string(i) + ".dat");
        stokes.solve();
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 3; ++i) { // i<3
        ...
        stokes.addBilinear(-innerProduct(2 * average(mu * Eps(u) * n, kappa1, kappa2), jump(v)) // [(2muEps(u)-pI)n]=0
                               ...
                               + innerProduct(penaltyParam * jump(u), jump(v)) // [u]=0
                           ,
                           interface);
        stokes.addLinear(+innerProduct(3. / 2, average(v * n, kappa2, kappa1)) * sigma, interface);
...
R interfaceRad = sqrt(0.25) + 1e-12; // [<-- Olshanskii example]
R fun_levelSet(const R2 P, const int i) {
    return sqrt((P.x - shift) * (P.x - shift) + (P.y - shift) * (P.y - shift)) - interfaceRad;
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) { // i<3
        ...
        const R sigma        = 1e-2; // 1e-2
        double uPenParam     = 1e-2;
        double pPenParam     = 1e-2;
        ...
        */
        ...
        R mu = 1;
        {
            ...
            stokes.addBilinear(-innerProduct(average(grad(u * t) * n, 0.5, 0.5), jump(v * t)) +
                               ...
                               Khi, INTEGRAL_INNER_EDGE_2D);
            stokes.addBilinear(-innerProduct(grad(u) * n, v)                     // natural
                                   + innerProduct(u, grad(v) * n)                // symmetry
                                   + innerProduct(1. / hi * penaltyParam * u, v) // stability

                                   + innerProduct(p, v * n) // natural
                               ,
                               interface);
            // ExpressionFunFEM<Mesh> gh1(gh,0,op_id); ExpressionFunFEM<Mesh> gh2(gh,1,op_id);
            stokes.addLinear(+innerProduct(gh.expression(2), grad(v) * n) +
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        const R hi           = 1. / (nx - 1);
        const R penaltyParam = 4e3; // 4e3, 1e2 [change this like 4.00001e3 if it crashes..]
        ...
        R mu = 1;
        {
            ...
            // [Dirichlet Velocity BC]
            {
                ...
                stokes.addLinear(+innerProduct(u0 * t, tau) // [wtf why is + now correct..?]
                                                            // + innerProduct(u0*n, 1./hi*penaltyParam*v*n) // stability
                                                            // - innerProduct(u0*t,tau)
                ...
                stokes.addLagrangeVecToRowAndCol(lag_row, lagr.rhs_, 0);
                // [Stabilization]
                double wPenParam = 1e1;
                double uPenParam = 1e1; // 2; // for mixed stab
                double pPenParam = 1e1; // 2; // for mixed stab
                FunTest grad2un  = grad(grad(u) * n) * n;
                ...
                stokes.addFaceStabilization(
                    ...
                    //  //+innerProduct(wPenParam*pow(hi,1)*jump(grad(w)*n), jump(grad(tau)*n))
                    //  //+innerProduct(wPenParam*pow(hi,3)*jump(grad(grad(w))*n), jump(grad(grad(tau))*n))
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) { // i<3
        ...
        const R hi           = 1. / (nx - 1);
        const R penaltyParam = 4e3; // 4e3
        const R sigma        = 2e0; // 1e-2
        double uPenParam     = 1e0;
        double pPenParam     = 1e0;
        ...
        */
        ...
        // [grad] DOESNT WORK AT ALL!!!?
        {
            ...
            // );
            // stokes.addLinear(
            //   + innerProduct(uhout.expression(2), grad(v)*n)
            //   + innerProduct(uhout.expression(2), 1./hi*penaltyParam*v)
            //   , Khi, INTEGRAL_BOUNDARY, {2}
            // );
            // stokes.addLinear(
            //   + innerProduct(uhin.expression(2), grad(v)*n)
        ...
        // [grad -> eps]
        {
            stokes.addBilinear(contractProduct(2 * mu * Eps(u), Eps(v)) - innerProduct(p, div(v)) +
                                   innerProduct(div(u), q)

                                   - innerProduct(2 * omega * u2, v1) + innerProduct(2 * omega * u1, v2),
                               Khi);
            stokes.addLinear(innerProduct(fh.expression(2), v), Khi);
            stokes.addBilinear(-innerProduct(average(2 * mu * (Eps(u) * t) * n, 0.5, 0.5), jump(v * t)) +
                                   innerProduct(jump(u * t), average(2 * mu * (Eps(v) * t) * n, 0.5, 0.5)) +
                                   innerProduct(1. / hi * sigma * (jump(u * t)), jump(v * t)),
                               Khi, INTEGRAL_INNER_EDGE_2D);
            stokes.addBilinear(
                // - innerProduct(2*mu*Eps(u)*n, v)  //natural
                // + innerProduct(u, 2*mu*Eps(v)*n)
                +innerProduct(10 * 1. / hi * penaltyParam * u, v), interface);
            stokes.addBilinear(
                // - innerProduct(2*mu*Eps(u)*n, v)  //natural
                // + innerProduct(u, 2*mu*Eps(v)*n)
                +innerProduct(1. / hi * penaltyParam * u, v), Khi, INTEGRAL_BOUNDARY, {1, 3});
            stokes.addBilinear(-innerProduct(2 * mu * Eps(u) * n, v) // natural
                                   + innerProduct(u, 2 * mu * Eps(v) * n) + innerProduct(1. / hi * penaltyParam * u, v),
                               Khi, INTEGRAL_BOUNDARY, {2, 4});
            stokes.addLinear(+innerProduct(uhout.expression(2), 2 * mu * Eps(v) * n) +
                                 innerProduct(uhout.expression(2), 1. / hi * penaltyParam * v)

                                 ,
                             Khi, INTEGRAL_BOUNDARY, {2});
            stokes.addLinear(+innerProduct(uhin.expression(2), 2 * mu * Eps(v) * n) +
                                 innerProduct(uhin.expression(2), 1. / hi * penaltyParam * v)
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) { // i<3
        ...
        */
        ...
        R mu    = 0.01;
        R omega = 1000; // 50 // original paper 100;
        {
            // [Bulk]
            ...
            stokes.addLinear(innerProduct(fh.expression(2), v), Khi);
            stokes.addBilinear(+innerProduct(div(u), q), Khi);
            stokes.addBilinear(+innerProduct(1. / hi * penaltyParam * u * n, v * n) // stability
                               ,
                               interface);
            stokes.addBilinear(+innerProduct(1. / hi * penaltyParam * u * n, v * n) // stability
                               ,
