...
R fun_levelSet(double *P, const int i) {
   return sqrt(P[0] * P[0] + P[1] * P[1]) - 1.00001;
}
R fun_rhs(double *P, const int i) { return 0; }
R fun_boundary(double *P, const int i) { return (i == 0) ? 0.5 * P[1] : 0; }
R fun_init_surfactant(double *P, int i) { return 1.; }
// R sigma(const R w) {return 24.5 - 0.25*w;}
// R Dsigma(const R w) {return - 0.25*w;}

R fun_x(double *P, const int i) { return P[i]; }
R fun_1(double *P, const int i) { return 1; }
...
class LambdaBoundary : public VirtualParameter {
 public:
   ...
   double G_, H_;
   LambdaBoundary(const CutFEMParameter &mu, double G, double H)
       : mu_(mu), G_(G), H_(H) {}
   double evaluate(int domain, double h, double meas, double measK,
                   double measCut) const {
      double gamma = meas / h;
...
class WeightKappa : public VirtualParameter {
 public:
   ...
   double evaluate(int dom0, double h, double meas, double measK,
                   double measCut0) const {
      int dom1        = (dom0 == 0);
...
int main(int argc, char **argv) {
   ...
   std::cout << " Discretization of the problem : \n"
             << " h = " << mesh_size << "\n t_begin = " << t0
             << " and t_final = " << final_time << "\n dt = " << dT
             << "\n number of iteration = " << total_number_iteration
             << std::endl;
   std::cout << " ------------------------------------" << std::endl;
   std::cout << " Parameters of the problem : \n"
             ...
             << " beta = " << beta << " \n"
             << " D_Gamma = " << epsilon_surfactant << std::endl;
   std::cout << " ------------------------------------ " << std::endl;
   ...
   //   "../../outputFiles/shearBubbleConservative/mesh"+meshstr+"/h"+to_string(divisionMeshsize)+"/";
   //   std::string pathOutpuFigure =
   //   "../../outputFiles/shearBubbleConservative/mesh"+meshstr+"/h"+to_string(divisionMeshsize)+"/paraview/";
   ...
   //   }
   //   std::ofstream outputData(pathOutpuFolder+"dataDrop.dat",
   //   std::ofstream::out); myCout << " Creating the file \"dataDrop.dat\" to
   //   save data" << std::endl; myCout << " Creating the file \"output.txt\" to
   //   save cout" << std::endl;
   //
   //
   //   myCout << " SIMULATION OF A BUBBLE IN SHEAR FLOW WITH INSOLABLE
   //   SURFACTANT. \n"
   //             << " We consider the navier Stokes equations coupled to the
   //             insoluble surfactant problem.\n"
   //             << " We solve the non linear system using the Newton method
   //             \n"
   //             << " We approximate the surfactant concentration using P1/P1
   //             space time finite elements  \n"
   //             << " We are using the levelSet P2 with a reinitializtion step
   //             every 5 time iterations \n"
   //             << " We are using P1 finite element in time, with the Simpson
   //             rule as quadrature rule "
   //             << std::endl << std::endl;
   //
   //
   ...
   //   myCout << " Number of iteration \t" << GTime::total_number_iteration <<
   //   std::endl; myCout << " We are using the Simpson rule (3 quadrature
   //   points)" << std::endl; myCout << " number of quadrature points in time :
   //   \t" << nbTime << std::endl; myCout << " number of dof in time per time
   //   slab : \t" << ndfTime << std::endl; return 0;
   //
   //
   ...
   while (iter < total_number_iteration) {
      ...
      // ----------------------------------------------
      {
         if (iter == 0) {
            std::cout << " ------------------------------------" << std::endl;
            std::cout << " Move the levelSet and create interfaces "
                      << std::endl;
         }
         ...
         for (int i = 0; i < nb_quad_time; ++i) {
            ...
            if (i < nb_quad_time - 1) {
               LevelSet::move(ls_k[i], vel[last_quad_time], vel[last_quad_time],
                              dt_levelSet, ls_k[i + 1]);
            }
         }
         if (iter == 0) {
            globalVariable::verbose = 1;
            std::cout << " Time moving interface : \t" << MPIcf::Wtime() - t0
                      << std::endl;
         }
      }
      ...
      if (iter == 0) {
         ...
         Sh.info();
         std::cout << " TIME BUILDING CUT MESH/SPACE : \t"
                   << MPIcf::Wtime() - t0_cmesh << std::endl;
      }
      ...
      if (iter == 0) {
         ...
         std::cout << " Time initialization : \t"
                   << MPIcf::Wtime() - t0_init_sol << std::endl;
      }
      ...
      while (1) {
         ...
         if (iterNewton == 0) {
            stokes.addBilinear(innerProduct(dt(du), rho * v) +
            ...
            stokes.addFaceStabilization(
                innerProduct(1e-1 * hh * jump(grad(du) * n),
                             mu * jump(grad(v) * n)) +
                    innerProduct(1e-1 * h3 * jump(D2nu), mu * jump(D2nv)) +
                    innerProduct(1e-1 * h3 * jump(grad(dp) * n),
                                 invmu * jump(grad(q) * n)),
                Kh_i, In);

            stokes.addFaceStabilization(
                innerProduct(1e0 * jump(grad(ds) * n), jump(grad(r) * n)), Kh_0,
                In);
            ...
            stokes.addBilinear(innerProduct(ds, r), *interface(last_quad_time),
                               In, last_quad_time);
         }

         for (int i = 0; i < nb_quad_time;
              ++i) { // computation of the curvature
            if (i > 0 || iterNewton > 0)
               globalVariable::verbose = 0;
            ...
            if (iterNewton == 0) {
               stokes.addBilinear(
                   innerProduct(Hx * ds, average(vx, kappa2)) * beta * sigma0 +
                       innerProduct(Hy * ds, average(vy, kappa2)) * beta *
                           sigma0 +
                       innerProduct(gradS(ds), average(v, kappa2)) * beta *
                           sigma0,
                   interface, In, i);
            }

            stokes.addLinear(-innerProduct(Hx, average(vx, kappa2)) -
                                 innerProduct(Hy, average(vy, kappa2)),
                             interface, In, i);
            if (i == 0) {
               Paraview<Mesh> writerS(cutTh, "shearExampleCurvature_" +
                                                 to_string(ifig) + ".vtk");
         ...
         if (iter == 0 && iterNewton == 0) {
            std::cout << " TIME ASSEMBLY MATRIX \t" << MPIcf::Wtime() - tt0
                      << std::endl;
         }
         /*
         ...
         */
         ...
         stokes.addLinear(-innerProduct(s0.expression(), r), *interface(0), In,
                          0);
         ...
         stokes.addLinear(
             ...
             Kh_i, INTEGRAL_BOUNDARY, In);
         if (iter == 0 && iterNewton == 0) {
            std::cout << " TIME ASSEMBLY RHS \t" << MPIcf::Wtime() - tt0
                      << std::endl;
         }
         ...
         tt0 = MPIcf::Wtime();
         if (iter == 0 && iterNewton == 0) {
            std::cout << " TIME MATRIX MULTIPLICATION \t"
                      << MPIcf::Wtime() - tt0 << std::endl;
         }
         tt0 = MPIcf::Wtime();
         ...
         */
         ...
         ExpressionFunFEM<Mesh2> ux(uh, 0, op_id, op_id),
             uy(uh, 1, op_id, op_id);
         ...
         if (iter == 0 && iterNewton == 0) {
            std::cout << " ASSEMBLY NON LINEAR \t" << MPIcf::Wtime() - tt0
                      << std::endl;
         }
         tt0 = MPIcf::Wtime();
         ...
         if (iter == 0) {
            std::cout << " Iterartion error || du ||_infty = " << dist
                      << std::endl;
            std::cout << " Iterartion error || ds ||_infty = " << dists
                      << std::endl;
         } else {
            std::cout << "\r";
            std::cout << " ITERATION \t : \t" << iter + 1 << " / "
         ...
         if (iterNewton == 10 || max(dist, dists) < 1e-10) {
            ...
            if (iter == 0) {
               std::cout << " TIME NEWTON ITERATION : \t"
                         << MPIcf::Wtime() - t0_newton << std::endl;
            }
      ...
      //                     PLOTTING
      //  -----------------------------------------------------
      if (MPIcf::IamMaster() && (iter % frequency_plotting == 0 ||
                                 iter + 1 == total_number_iteration)) {
         ...
         {
            std::string filename =
                "shearExampleSurfactant_" + to_string(ifig) + ".vtk";
            if (iter == 0) {
               std::cout << " Plotting -> " << filename << std::endl;
            }
   ...
   std::cout << "\n\n ------------------------------------" << std::endl;
   std::cout << " -----------------------------------  " << std::endl;
   std::cout << " Computation time : \t " << MPIcf::Wtime() - cpubegin
             << std::endl;
   //
