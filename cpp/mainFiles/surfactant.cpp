...
R fun_sol_surfactant(double *P, const int i, const R t) {
   R x = P[0], y = P[1];
   //  return P[1] / sqrt(P[0] * P[0] + P[1] * P[1]) + 2;
   return exp(-t / 4) * (y / sqrt(x * x + y * y)) + 2;
...
R fun_levelSet(double *P, const int i, const R t) {
   R a = (1 + 0.25 * sin(2 * M_PI * t));
   // a = 1;
   return sqrt(P[0] * P[0] / a + P[1] * P[1]) - 1.0000079;
}
R fun_rhs(double *P, const int cc, const R t) {
   R x = P[0], y = P[1];
   ...
   //         + 64*x*x*x*x*sin(2*M_PI*t) + 960*y*y*y*y*sin(2*M_PI*t) +
   //         1024*x*x*x*x
   //         + 1024*y*y*y*y + 144*x*x*y*y*pow(sin(2*M_PI*t),2)
   //         + 4*x*x*y*y*pow(sin(2*M_PI*t),3) + 1024*x*x*y*y*sin(2*M_PI*t)))
   //   /pow((y*y*pow(sin(2*t*M_PI),2) + 8*y*y*sin(2*t*M_PI) + 16*x*x +
   //   16*y*y),3)
   //   - 4*x*y*exp(-4*t) + (x*y*M_PI*exp(-4*t)*cos(2*M_PI*t))/(sin(2*M_PI*t) +
   //   4)
   //   + (x*y*y*y*M_PI*exp(-4*t)*cos(2*M_PI*t)*(sin(2*M_PI*t) + 4))/
   //   (y*y*pow(sin(2*M_PI*t),2) + 8*y*y*sin(2*M_PI*t) + 16*x*x + 16*y*y);
   R r = (4 * x * y * exp(-t / 4) * (sin(2 * pi * t) + 4) *
          (3 * y * y * sin(2 * pi * t) * sin(2 * pi * t) +
           4 * x * x * sin(2 * pi * t) + 28 * y * y * sin(2 * pi * t) +
           64 * x * x + 64 * y * y)) /
             pow(y * y * sin(2 * t * pi) * sin(2 * t * pi) +
                     8 * y * y * sin(2 * t * pi) + 16 * x * x + 16 * y * y,
                 2) -
         (x * y * exp(-t / 4)) / 4 +
         (x * y * pi * exp(-t / 4) * cos(2 * pi * t)) / (sin(2 * pi * t) + 4) +
         (x * y * y * y * pi * exp(-t / 4) * cos(2 * pi * t) *
          (sin(2 * pi * t) + 4)) /
             (y * y * sin(2 * pi * t) * sin(2 * pi * t) +
              8 * y * y * sin(2 * pi * t) + 16 * x * x + 16 * y * y);
   return r;
}
// R fun_rhs(double * P, const int cc, const R t) {
//   R x = P[0],  y = P[1];
...
std::tuple<double, double> solveCG_1(int i, const Mesh &Kh, int nx,
                                     double tfinal, double hi, double dT) {
   const int d = 2;
   ...
   double time_step           = dT;
   double t0                  = 0.;
   Mesh1 Qh(total_number_iteration + 1, t0, tfinal);
   ...
   while (iter < total_number_iteration) {
      ...
      //           << total_number_iteration - 1 << std::endl;
      double tid = iter * dT;
      ...
      surfactant.addBilinear(innerProduct(dt(s), r) +
      ...
      surfactant.addFaceStabilization(
          innerProduct(0.1 * jump(grad(s) * n), jump(grad(r) * n)), Kh0, In);
      ...
      //     // );
      //
      surfactant.addBilinear(innerProduct(s, r), *interface(0), In, 0);
      surfactant.addLinear(innerProduct(u0.expr(), r), *interface(0), In, 0);

      Fun_h funrhs(Wh, In, fun_rhs);
      // Fun_h funrhsp(cutVh, In);
      // projection(funrhs, funrhsp, In, interface ,0);
      surfactant.addLinear(innerProduct(funrhs.expr(), r), interface, In);
      ...
      //  -----------------------------------------------------
      {
         ...
         Fun_h funuh(Wh, sol);
         errL2_0 =
             L2normSurf(funuh, fun_sol_surfactant, *interface(0), tid, 0, 1);
         //  std::cout << " t_n -> || u-uex||_2 = " << errL2 << std::endl;

         sol += uh(SubArray(Wh.get_nb_dof(), Wh.get_nb_dof()));
         errL2_1 = L2normSurf(funuh, fun_sol_surfactant, *interface(nbTime - 1),
                              tid + dT, 0, 1);
      ...
      //    // Paraview2 writer(cutVh,
      //    // pathOutpuFigure+"surfactant_"+to_string(iterfig)+".vtk");
      //    Paraview<Mesh> writer(Kh0, "surfactant_" + to_string(iter) +
      //    ".vtk");
   ...
   return {errL2_0, errL2_1};
}

std::tuple<double, double> solveCG_2(int i, const Mesh &Kh, int nx,
                                     double tfinal, double hi, double dT) {
   const int d           = 2;
   const double cpubegin = CPUtime();
   ...
   //  "../../outputFiles/surfactant2/Ellipse/mesh" + meshstr + "/h" +
   //      to_string(divisionMeshsize) + "/";
   // std::string pathOutpuFigure =
   //  "../../outputFiles/surfactant2/Ellipse/mesh" + meshstr + "/h" +
   //      to_string(divisionMeshsize) + "/paraview/";
   ...
   //  std::ofstream outputData("dataDrop" + to_string(i) + ".dat",
   //                           std::ofstream::out);
   ...
   while (iter < total_number_iteration) {
      ...
      surfactant.addBilinear(-innerProduct(s, dt(r)) +
                                 innerProduct(epsilon_S * gradS(s), gradS(r)),
                             interface, In);
      ...
      for (int i = 0; i < nbTime; ++i) {
         ExpressionFunFEM<Mesh2> vx(vel[i], 0, op_id);
         ExpressionFunFEM<Mesh2> vy(vel[i], 1, op_id);
         surfactant.addBilinear(-innerProduct(s, dx(r) * vx) -
      ...
      surfactant.rhs_ = 0.;
      surfactant.addBilinear(innerProduct(s, r), *interface(lastQuadTime), In,
                             lastQuadTime);
      surfactant.addLinear(innerProduct(1., r), *interface(lastQuadTime), In,
                           lastQuadTime);

      matlab::Export(surfactant.mat_[0], "matA.dat");
      matlab::Export(surfactant.rhs_, "rhs.dat");
      getchar();
      return {0, 0};
      ...
      //  -----------------------------------------------------
      {
         Fun_h funuh(Wh, sol);
         errL2_0 =
             L2normSurf(funuh_0, fun_sol_surfactant, interface(0), tid, 0, 1);
         //  std::cout << " t_n     -> || u-uex||_2 = " << errL2 << std::endl;

         errL2_1 = L2normSurf(funuh_1, fun_sol_surfactant,
      ...
      //  -----------------------------------------------------
      {
         ...
         std::cout << std::setw(10) << std::setfill(' ') << tid
                   << std::setw(20)
                   //  << std::setfill(' ') << q0 << std::setw(20)
                   //  << std::setfill(' ') << q1 << std::setw(20)
                   //  << std::setfill(' ') << intF << std::setw(20)
                   << std::setfill(' ') << fabs(q1 - qp1 - intF) << std::endl;
         qp1 = q1;
      }
...
// double solveDG_1(int i, const Mesh& Kh, int nx, double tfinal, double hi,
// double dT) {
//   const int d = 2;
//   // typedef Mesh2 Mesh;
//   // typedef FESpace2 Space;
...
//
//     std::cout << " ITERATION \t : \t" << iter << " / " <<
//     GTime::total_number_iteration -1 << std::endl; double tid =
//     GTime::current_time();
//     // INITIALIZE LEVELSET, VELOCITY, INTERFACE
//     ls.begin()->swap(ls[nbTime-1]);
//     for(int i=0;i<nbTime;++i) {
...
//       //   //+ innerProduct(average((vel.at(i) * t)*s, 1, 1), average(r)) //
//       TEST
...
//       // "Variant 2"
//       surfactant.addBilinear(
//           + innerProduct(average((vel[i]*conormal)*s, 0.5, -0.5),
//           jump(r))*0.5
//           - innerProduct(jump(s), average((vel[i]*conormal)*r, 0.5,
//           -0.5))*0.5
//           + innerProduct(10*jump(s), jump(r))
//           //+ innerProduct(lambdaBSdiv*average((vel[i]*t)*s, 1, 1),
//           average((vel[i]*t)*r, 1, 1))                  // (3.16) , interface
//           , innerRidge
//           , In , i
//       );
//
...
//
//       sol  += uh(SubArray(Wh.get_nb_dof(), Wh.get_nb_dof()));
//       errL2 =  L2normSurf(funuh, fun_sol_surfactant, *interface(nbTime-1),
//       tid+dT,0,1); std::cout << " t_{n+1} -> || u-uex||_2 = " << errL2 <<
//       std::endl;
//     }
//
//
...
int main(int argc, char **argv) {
   ...
   for (int iter = 0; iter < 3; ++iter) {
      ...
      double tf = 0.5;
#else
      Mesh Kh(nx, nx, -3, -3, 8., 6.);
      double h  = 8. / (nx - 1);
      double dt = h / 4;
      double tf = 0.5;
#endif

      // auto [err1_0, err1_1] = solveCG_1(iter, Kh, nx, tf, h, dt);
      auto [err2_0, err2_1] = solveCG_2(iter, Kh, nx, tf, h, dt);

      std::cout << " ------------------- " << std::endl;
      std::cout << " h              : \t" << h << std::endl;
      // std::cout << " Error method 1 : \t" << err1_0 << "\t" << err1_1
      //           << std::endl;
      std::cout << " Error method 2 : \t" << err2_0 << "\t" << err2_1
                << std::endl;
      nx = 2 * nx - 1;
   }
   return 0;
...
// cout << " Number of iteration \t" << GTime::total_number_iteration <<
// std::endl; cout << " We are using the Simpson rule (3 quadrature points)" <<
// std::endl; cout << " number of quadrature points in time : \t" << nbTime <<
// std::endl; cout << " number of dof in time per time slab : \t" << ndfTime <<
// std::endl;
//
//
...
//     std::endl; std::cout << "
//     ------------------------------------------------------------- "<<
//     std::endl; std::cout << " ITERATION \t : \t" << iter << std::endl;
...
//     cout << " Number of dof of the problem \t" << surfactant.nDoF <<
//     std::endl;
//
//
//
...
//       // cout << " error    ->    " << sqrt(integralSurf(sol*sol, cutVh, 0))
//       << std::endl;
//       // Fun_h funuh(cutVh, uh);
//       Fun_h funuh(cutVh, sol2);
//       // cout << " error    ->    " << L2normSurf(funuh,
//       fun_sol_surfactant,GTime::current_time(),0,1) << std::endl; errL2 =
//       L2normSurf(funuh, fun_sol_surfactant,GTime::current_time(),0,1);
...
//   //                     PLOTTING
//   //  -----------------------------------------------------
//   if(MPIcf::IamMaster() && iter%frequencyPlottingTheSolution == 0 &&
//   writeVTKFiles){
...
//
//       // Paraview2 writer(cutVh,
//       pathOutpuFigure+"surfactant_"+to_string(iterfig)+".vtk"); Paraview2
//       writer(cutVh, "surfactant_"+to_string(iterfig)+".vtk");
//
//       writer.add(sol , "surfactant", 0, 1);
//       writer.add(ls[0], "levelSet",  0, 1);
//       writer.add(ls[2], "levelSet2", 0, 1);
