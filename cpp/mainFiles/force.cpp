...
struct MultiLevelSet {
    ...
    MultiLevelSet(std::vector<std::shared_ptr<BaseLevelSetStruct>> levelSets) : levelSets(levelSets) {}
    ...
    std::vector<std::shared_ptr<BaseLevelSetStruct>> levelSets;
};
...
std::vector<std::shared_ptr<BaseLevelSetStruct>> generateRandomLevelSets(double xc_min, double xc_max, int n_ls,
...
int main(int argc, char **argv) {
    ...
    // // 2) solve Stokes
    auto data_stokes = solveStokes(Vh, Ph, interface, r_0);
    ...
    if (MPIcf::IamMaster()) {
        ...
        paraview_t writer(Khi, "force_geometry.vtk");
