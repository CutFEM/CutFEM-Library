...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < niteration; ++i) {
        ...
        outputData << i << "\t" << tid << "\t" << std::setprecision(16) << errU << "\t" << std::setprecision(16) << qu
                   << "\t" << std::setprecision(16) << fabs(qu - qu0) << "\t" << std::setprecision(16) << min_uh_tild
                   << "\t" << std::setprecision(16) << max_uh_tild << "\t" << std::setprecision(5) << std::endl;
...
R fun_initial(double *P, int elementComp, int domain) { return sin(pi * (P[0] + P[1])); }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < niteration; ++i) {
        ...
        std::cout << std::setprecision(16) << "|q(u) - q(u0)| = " << fabs(qu - qu0) << std::setprecision(6)
                  << std::endl;
        outputData << i << "\t" << tid << "\t" << std::setprecision(16) << errU << "\t" << std::setprecision(16) << qu
                   << "\t" << std::setprecision(16) << fabs(qu - qu0) << "\t" << std::setprecision(5) << std::endl;
    }
...
void solve_problem(const fespace_t &Wh, Rn &u0, Rn &uh, MatMap &Ah, MatMap &Mh, double tn) {
    ...
    problem.addLinear(-innerProduct(average(fluxFctN(Un)), jump(v)), Khi, INTEGRAL_INNER_EDGE_2D);
    ...
    for (int i = 0; i < niteration; ++i) {
        ...
        // =================================================
        {
            ...
            if (Vh.basisFctType == BasisFctType::P1dc) {
                ...
                solve_problem(Vh, u0, uh, Ah, Mh, tid);
                u1 += dt * uh;
                limiter::FEM::applyBoundPreservingLimiter(fun_u1, u1_tild, min_u0, max_u0);
                u2 += 3. / 4 * u0 + 1. / 4 * u1_tild;
                solve_problem(Vh, u1_tild, uh, Ah, Mh, tid + dt);
                u2 += 1. / 4 * dt * uh;
                limiter::FEM::applyBoundPreservingLimiter(fun_u2, u2_tild, min_u0, max_u0);
                solve_problem(Vh, u2_tild, uh, Ah, Mh, tid + 0.5 * dt);
                uh *= 2. / 3 * dt;
                uh += 1. / 3 * u0 + 2. / 3 * u2_tild;
                limiter::FEM::applyBoundPreservingLimiter(fun_uh, uh_tild, min_u0, max_u0);
        ...
        std::cout << " Iteration computation time \t" << getTime() - tt_iter << std::endl;
    }
...
R fun_initial(double *P, int elementComp, int domain) { return 1 + 0.5 * sin(pi * (P[0] + P[1])); }
R fun_scalar(double x) { return 1 + 0.5 * sin(pi * x); }
R fun_solution(double *P, int elementComp, int domain, double t) {
    double x0 = P[0] + P[1];
    double xy = x0;
...
int main(int argc, char **argv) {
    ...
    // DECLARATION OF THE VECTOR CONTAINING THE solution
    // =====================================================
    std::vector<double> u0_data(Vh.nbDoF, 0.);
    Rn_ u0(u0_data.data(), u0_data.size());
    interpolate(Vh, u0, fun_initial);
    Rn uh(u0);
    std::vector<double> uh_tild(u0.size(), 0.);
    ...
    for (int i = 0; i < niteration; ++i) {
        ...
        // =================================================
        {
            ...
            // 1) compute the mean of the macro element (extension for macro
            // element, not CutFEM)
            // 2) compute indicator
            // 3) do the slope limiter
            // 4)reconstruction to keep the mean on the macro elements 5) boundary
            // preserving

            if (Vh.basisFctType == BasisFctType::P1dc) {
                uh = solve_problem(Vh, u0, Ah, Mh, tid);
                u1 += dt * uh;

                std::vector<double> indicator1 = limiter::FEM::computeTroubleCellIndicator(fun_u1);

                // std::vector<double> u1_slope =
                //     limiter::FEM::applySlopeLimiter(fun_u1, indicator1, 1.);
                std::vector<double> u1_slope(u1.begin(), u1.end());
                // u1         = u1_slope.data();
                // indicator1 = limiter::FEM::computeTroubleCellIndicator(fun_u1);
                // u1_slope = limiter::FEM::applySlopeLimiter(fun_u1,
                // indicator1, 1.);

                // std::cout << u1_slope << std::endl;

                // std::vector<double> u1_tild = u1_slope;
                std::vector<double> u1_tild = limiter::FEM::applyBoundPreservingLimiter(fun_u1, min_u0, max_u0);

                uh      = u1;
                uh_tild = u1_tild;
                Fun_h fun_uh_tild0(Vh, uh_tild);
                auto [min_uh, max_uh] = limiter::FEM::findMinAndMaxValue(fun_uh);
                auto [min_u1, max_u1] = limiter::FEM::findMinAndMaxValue(fun_uh_tild0);
                std::cout << "[m, M] = [ " << min_u0 << " , " << max_u0 << " ]" << std::endl;
                std::cout << min_uh << " < u_{h,M}  < " << max_uh << std::endl;
                std::cout << min_u1 << " < u1_{h,M} < " << max_u1 << std::endl;

                Fun_h fun_indicator(Eh, indicator1);
                ...
                // KN_<double> u1_tild(u1_t.data(), u1_t.size());
                u2 += 3. / 4 * u0 + 1. / 4 * Rn_(u1_tild);
                uh = solve_problem(Vh, u1_tild, Ah, Mh, tid + dt);
                u2 += 1. / 4 * dt * uh;

                std::vector<double> indicator2 = limiter::FEM::computeTroubleCellIndicator(fun_u2);
                std::vector<double> u2_tild    = limiter::FEM::applyBoundPreservingLimiter(fun_u2, min_u0, max_u0);

                uh = solve_problem(Vh, u2_tild, Ah, Mh, tid + 0.5 * dt);
                uh *= 2. / 3 * dt;
                uh += 1. / 3 * u0 + 2. / 3 * Rn_(u2_tild);

                std::vector<double> indicator3 = limiter::FEM::computeTroubleCellIndicator(fun_u2);
                uh_tild                        = limiter::FEM::applyBoundPreservingLimiter(fun_uh, min_u0, max_u0);
