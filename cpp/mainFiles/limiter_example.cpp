...
R fun_initial1 (const R2 P, int elementComp, int domain) {
  double r0 = 0.4;
  double a  = 0.3;
  return 0.5*(1 - tanh( ((P.x-r0)*(P.x-r0)+P.y*P.y) / (a*a) -1)) +1;
}
...
R fun_solution(const R2 P, int elementComp, int domain, double t) {
  ...
  for(int i=0;i<=50;++i) {
    ...
    if(fabs(x1-x0) < 1e-12) return fun_scalar(x1);
    x0 = x1;
    if(i == 50) {
      std::cout << " Burger Newton not converged" << std::endl;
      getchar();
...
int main(int argc, char** argv ) {
  ...
  // DECLARATION OF THE VECTOR CONTAINING THE solution
  // =====================================================
  Rn u0(Vh.NbDoF(), 0.);interpolate(Vh, u0, fun_initial);
  Rn uh(u0);
  Rn uh_tild(u0);
  ...
  for(int i=0;i<niteration;++i) {
    ...
    // if(MPIcf::IamMaster() && i%5 == 0 || i+1 == niteration) {
    //   // Fun_h fun_thet (Wh, fun_theta);
    //   Paraview<Mesh> writer(Th, "test_burgerFEM_P01_"+to_string(ifig++)+".vtk");
    //   writer.add(fun_uh, "uhNoLimiter", 0, 1);
    //   // writer.add(fun_uh_tild, "uhLimiter", 0, 1);
    //   writer.add(femErrh, "error"  , 0, 1);
    //   writer.add(fun_ex , "u_exact", 0, 1);
...
R fun_solution(double* P, int elementComp, int domain, double t) {
  ...
  for(int i=0;i<=50;++i) {
    ...
    double x1 = xy-2*t*fun_scalar(x0);
    if(fabs(x1-x0) < 1e-10) return fun_scalar(x1);
    x0 = x1;
    if(i == 100) {
