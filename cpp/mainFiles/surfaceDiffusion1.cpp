...
double fun_rhs0(const R2 P, int elementComp) {
    return -(9*pow(P.y,7) - 45*pow(P.x,4)*pow(P.y,3)-27*pow(P.x,6)*P.y - 9*P.x*P.x*pow(P.y,5))/(pow(P.x*P.x + P.y*P.y,3));
}
...
void errorAnalysis(int argc, char** argv) {
    ...
    for (int i = 0; i < nms; i++) {
        ...
        //std::cout << n << ", ";
        mesh_sizes[i] = 51;//10*pow(2,i) + 1;//4 + 20*i;
        ...
        std::cout << "n = " << nx << "\t h = " << h <<   std::endl;

        double betaE = 100, betaF = 100, gamma = 1e-2;
        ...
        // Full stabililzation
        // std::cout << " STAB PARAMETER \t" << 100*h << std::endl;
        // Paper variant
        surfdiff.addBilinear(
                  innerProduct(betaF/h/h*jump(u0), jump(v0))
                + innerProduct(h*jump(grad(u0)*n), jump(grad(v0)*n))
                , innerEdge
        );
        ...
        // matlab::Export(surfdiff.mat, "matrix.dat");
...
void solve(int argc, char** argv) {
    ...
    */
    ...
    KN_<double> data_sh(surfdiff.rhs(SubArray(idx_s0,0)));
    ...
    // Integral on interface for surface variable (K_h)
    surfdiff.addBilinear(
            innerProduct(gradS(u0),gradS(v0))            // (3.12)
            , interface
    );
    ...
    // surfdiff.addBilinear(
    //         - innerProduct(average(gradS(u0)*t, 0.5,-0.5),jump(v0)) // (3.12)/(3.81)
    //         - innerProduct(jump(u0), average(gradS(v0)*t,0.5,-0.5))    // (3.13)/(3.82) added for symmetry
    //         + innerProduct(betaE/h*jump(u0),jump(v0))                             // (3.13)/(3.82)
    //         , interface
    //         , nodeEvaluation
    // );
    ...
    surfdiff.addBilinear(
            innerProduct(betaF/h/h*jump(u0), jump(v0))
            + innerProduct(100*h*jump(grad(u0)), jump(grad(v0)))
            , innerEdge
    );
