...
int main(int argc, char **argv) {
    ...
    for (int j = 0; j < iterations; ++j) {
        ...
        while (iter < total_number_iteration) {
            ...
            // convdiff.initialSolution(data_u0);                          // allocate memory        //! DANGEROUS: THIS
            // RUNS BUT GIVES INCORRECT SOLUTION
...
#ifdef conservative
                Paraview<Mesh> writer(Thi,
                                      path_output_figures + "bulk_conservative" + std::to_string(iter + 1) + ".vtk");
#else
                Paraview<Mesh> writer(Thi,
                                      path_output_figures + "bulk_nonconservative" + std::to_string(iter + 1) + ".vtk");
#endif
...
#ifdef conservative
                Paraview<Mesh> writer_surface(ThGamma, path_output_figures + "surface_conservative" +
                                                           std::to_string(iter + 1) + ".vtk");
#else
                Paraview<Mesh> writer_surface(ThGamma, path_output_figures + "surface_nonconservative" +
                                                           std::to_string(iter + 1) + ".vtk");
#endif
