...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        if (set_p) {
            ...
            // [ASSEMBLY]
            darcy.addBilinear(innerProduct(u, v) + innerProduct(div(u), div(v)), Kh);
            ...
            Ph.info();
            // [ASSEMBLY]
            darcy.addBilinear(innerProduct(u, v) + innerProduct(div(u), q) - innerProduct(p, div(v)), Kh);
...
R fun_radius2(double *P) { return (P[0] - shift) * (P[0] - shift) + (P[1] - shift) * (P[1] - shift); }
R fun_levelSet(double *P, const int i) {
    return sqrt((P[0] - shift) * (P[0] - shift) + (P[1] - shift) * (P[1] - shift)) - interfaceRad;
}
...
R fun_interfacePr(double *Pp, int compInd) { return (-1. / 8 * mu_G + c / (2. * 1e2)); }
} // namespace Data_CutMixedDarcy_Star
...
//   return -sin(P[0])*sinh(P[1]) - (cos(1) - 1)*(cosh(1) - 1);
// }
R fun_natural(double *P, int compInd) { return -sin(P[0]) * sinh(P[1]) - (cos(1) - 1) * (cosh(1) - 1); }

R fun_enforced(double *P, int compInd) { return (compInd == 0) ? cos(P[0]) * sinh(P[1]) : sin(P[0]) * cosh(P[1]); }
...
R fun_levelSet(double *P, const int i) {
    return interfaceRad - sqrt((P[0] - shift) * (P[0] - shift) + (P[1] - shift) * (P[1] - shift));
}
...
R fun_natural(double *P, int compInd) { return -sin(2 * pie * P[0]) * cos(2 * pie * P[1]); }
...
R fun_exact_p(double *P, int compInd, int dom) { return -sin(2 * pie * P[0]) * cos(2 * pie * P[1]); }
...
R fun_levelSet_in(double *P, const int i) {
    return sqrt((P[0] - shift) * (P[0] - shift) + (P[1] - shift) * (P[1] - shift)) - inRad;
}
R fun_levelSet_out(double *P, const int i) {
    return outRad - sqrt((P[0] - shift) * (P[0] - shift) + (P[1] - shift) * (P[1] - shift));
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        darcy.addLinear(-innerProduct(phat.expression(), jump(v * n)), interface_in);
        ...
        // [PLOTTING]
        {
            ...
            Paraview<Mesh> writer(Kh_i, "darcyTwoBoudaries_" + to_string(i) + ".vtk");
            writer.add(uh, "velocity", 0, 2);
            writer.add(ph, "pressure", 0, 1);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        // BULK PROBLEM
        // =====================================================
        darcy.addBilinear(innerProduct(nu * u, v) - innerProduct(p, div(v)) + innerProduct(div(u), q));

        darcy.addBilinear(innerProduct(gamma_D * invh * (u * n), (v * n)) + innerProduct(p, v * n) -
                              innerProduct(u * n, q),
                          boundary, {2, 4});
        ...
        // SURFACE PROBLEM
        // =====================================================
        darcy.addBilinear(innerProduct(nu_hat * u_hat, v_hat) - innerProduct(p_hat, divS(v_hat)) +
        ...
        darcy.addBilinear(-innerProduct(average(p_hat, 0.5, 0.5), jump(v_hat * t, 1, 1)) -
                              innerProduct(jump(u_hat * t, 1, 1), average(q_hat, 0.5, 0.5)) +
                              innerProduct(1e2 * jump(u_hat * t, 1, 1), jump(v_hat * t, 1, 1)),
                          interface, innerEdge);

        darcy.addLinear(-innerProduct(p0.expression(), v_hat * t), interface, boundary, {1, 3});
        ...
        // const CutFEM_Parameter mu("mu", 2, 2);
        R es_divU = integral(innerProduct(div(u_hat), div(v_hat)), us, us, interface);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        R maxErrDiv = 0.; // maxNorm(femSol_0dx+femSol_1dy+femSol_1dz,fun_div,Kh_i);
        ...
        // [PLOTTING]
        {
            ...
            // Paraview<Mesh> writer(Kh_i, "darcyRT2_"+to_string(i)+".vtk");
            Paraview<Mesh> writer(Kh, "darcyscottiSphereFEM_3D_" + to_string(i) + ".vtk");
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        if (MPIcf::IamMaster()) {
            ...
            // Paraview<Mesh> writer(Kh_i, "darcyRT2_"+to_string(i)+".vtk");
            Paraview<Mesh> writer(Kh_i, "darcyscottiARITIFICIAL_3D_" + to_string(i) + ".vtk");
...
R shift        = 0.5;
R interfaceRad = 0.25001;               // not exactly 1/4 to avoid interface cutting exaclty a vertex
R mu_G         = 2. / 3 * interfaceRad; // xi0*mu_G = 1/8*2/3*1/4
