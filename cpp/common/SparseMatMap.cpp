...
void multiply(int N, const std::map<std::pair<int, int>, double> &AA, const std::map<std::pair<int, int>, double> &BB,
    ...
    for (int i = 0; i < N; ++i) { // loop over line of A
        ...
        for (int jp = A.p[i]; jp < A.p[i + 1]; ++jp) { // loop over row i of A
            ...
            for (int kp = B.p[j]; kp < B.p[j + 1]; ++kp) { // loop over line j of B (corresponding to the column of
                                                           // element in A)
...
void eraseAndSetRow(int N, std::map<std::pair<int, int>, double> &A, std::span<double> b, int dof2rm, int dof2set,
    ...
    util::at(b, dof2set)                = val; // p.second;
...
void eraseAndSetRowCol(int N, std::map<std::pair<int, int>, double> &A, std::span<double> b,
                    std::map<int, double> &dof2rm) {
