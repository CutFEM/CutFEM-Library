...
#ifndef COMMON_FRACTURE_HPP_
#define COMMON_FRACTURE_HPP_
...
class Local_Partition {
 ...
 public:
   ...
   void add_triangle(const TriaIdx &iv) { elements_idx.push_back(iv); }
   void erase_triangle(int k) { elements_idx.erase(elements_idx.begin() + k); }
...
class Cut_Base {
 ...
 public:
   bool is_negative(const R2 P) const { return (f(P) <= 0); }
   bool is_positive(const R2 P) const { return (f(P) > 0); }
   bool cut_edge(const R2 A, const R2 B) const { return (f(A) * f(B) < 0); }
   ...
   void add_element_and_face(int k, int iface) {
      map_element_to_face[k] = iface;
   }
   bool is_cut_element(int k) const {
      return (map_element_to_face.find(k) != map_element_to_face.end());
   }

   void cut_partition(Local_Partition &local_partition,
                      vector<TriaIdx> &new_element_idx,
                      std::list<int> &erased_element) const {
            ...
            for (auto it = patch.element_begin(); it != patch.element_end();
               ...
               for (int i = 0; i < 3; ++i) {
                  ...
                  if (idx < 3) {
                     ...
                     local_partition.add_node(
                         get_intersection_node(K[i0], K[i1]));
                  }
               }
               // ADD THE INDICES
...
class Cut_LevelSet : public Cut_Base {
 ...
 public:
   Cut_LevelSet(double (*ff)(const R2 P), int lab = 0)
       : Cut_Base(lab), fun_levelSet(ff) {}

 private:
   double f(const R2 A) const { return fun_levelSet(A); }
};
...
class Cut_Line : public Cut_Base {
 ...
 public:
   Cut_Line(double aa, double bb, double cc, int lab = 0)
       : Cut_Base(lab), a(aa), b(bb), c(cc) {}

 private:
   double f(const R2 A) const { return (a * A.x + b * A.y + c); }
};
...
class Cut_Parametric_Line : public Cut_Base {
 ...
 public:
   Cut_Parametric_Line(geometry::Interval tt, geometry::Droite xx,
                       geometry::Droite yy, int lab = 0)
       : Cut_Base(lab), t(tt), x(xx), y(yy) {}
};
...
class Fracture {
 ...
 public:
   ...
   int CheckV(int i) const {
      ASSERTION(i >= 0 && i < vertices_.size());
      return i;
   }
   int CheckT(int i) const {
      ASSERTION(i >= 0 && i < faces_.size());
      return i;
   }
   ...
   bool is_cut_element(const int k) const {
      for (int i = 0; i < cut_lines_.size(); ++i) {
         if (cut_lines_[i]->is_cut_element(k))
            return true;
      }
      return false;
   }

   void build_local_partition(const int k,
                              Local_Partition &local_partition) const {
      ...
      for (int i = 0; i < cut_lines_.size(); ++i) {
         ...
         // WE PERFORM THE CUT
         cut_lines_[i]->cut_partition(local_partition, new_element_idx,
                                      erased_element);
 ...
 private:
   void build(Cut_Base &L1) {
      ...
      for (int k = 0; k < Th.nbElmts(); k++) {
         ...
         // std::vector<RemumberVertexPairT> zero_vertex_uses; // used to
         // renumber the zero_vertexes
         for (Uint i = 0; i < nv; ++i) {
   ...
   Face make_face(const typename RefPatch::FaceIdx &ref_tri, const Element &K,
