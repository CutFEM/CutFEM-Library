...
template <class R> class MatriceMap : VirtualMatrice<R> {
 public:
   ...
   const Map &m;
   MatriceMap(int in, int im, const Map &mm)
       : VirtualMatrice<R>(in, im), m(mm) {}
   void addMatMul(const KN_<R> &x, KN_<R> &Ax) const;
};

template <class R>
KN<R> operator*(const std::map<std::pair<int, int>, R> &A, const KN_<R> &x) {
   KN<R> Ax(x.size());
...
void MatriceMap<R>::addMatMul(const KN_<R> &x, KN_<R> &Ax) const {
   for (typename Map::const_iterator k = this->m.begin(); k != this->m.end();
        ++k) {
      int i = k->first.first;
...
template <class R>
SparseMatrixRC<R>::SparseMatrixRC(int nn, int mm,
                                  const std::map<std::pair<int, int>, R> &M)
    : VirtualMatrice<R>(nn, mm), n(nn), m(mm), nbcoef(M.size()),
      p(new int[nn + 1]), j(new int[nbcoef]), a(new R[nbcoef]) {
   R cmm = 0;

   int k = 0;
   for (int i = 0; i <= n; ++i)
      p[i] = 0; // pour les lignes vide
   for (typename std::map<std::pair<int, int>, R>::const_iterator q = M.begin();
        q != M.end(); ++q, ++k) {
      int i      = q->first.first;
      p[i + 1]   = k + 1;
...
SparseMatrixRC<R>::SparseMatrixRC(const SparseMatrixRC<R> &A)
    : VirtualMatrice<R>(A.n, A.m), n(A.n), m(A.m), nbcoef(A.nbcoef),
      p(new int[n + 1]), j(new int[nbcoef]), a(new R[nbcoef]) {
   R cmm = 0;
