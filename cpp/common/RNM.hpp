...
class ShapeOfArray {
  ...
  public:
    ...
    ShapeOfArray(const ShapeOfArray &old, const SubArray &sub) : n(sub.n), step(old.step * sub.step), next(old.next) {
    ...
    bool SameShape(const ShapeOfArray &a) const { return !step || !a.step || a.n == n; }
...
template <class R> class KN_ : public ShapeOfArray {
  ...
  private:
    ...
    KN_(const KN_<R> &u, const ShapeOfArray &sh, long startv = 0) : ShapeOfArray(sh * u.step), v(&u[startv]) {}
...
template <class R> class KNM_ : public KN_<R> {
  ...
  public:
    ...
    KNM_(R *u, long nn, long mm) : KN_<R>(u, ShapeOfArray(nn * mm)), shapei(nn, 1, nn), shapej(mm, nn, 1) {}
    KNM_(R *u, long nn, long mm, long s) : KN_<R>(u, ShapeOfArray(nn * mm, s)), shapei(nn, 1, nn), shapej(mm, nn, 1) {}
    KNM_(KN_<R> u, long n, long m) : KN_<R>(u, ShapeOfArray(m * n)), shapei(n, 1, n), shapej(m, n, 1) {}
    ...
    KNM_(KNM_<R> U, const SubArray &si, const SubArray &sj)
        : KN_<R>(U, SubArray(U.ij(si.len1(), sj.len1()) + 1, U.ij(si.start, sj.start))), shapei(U.shapei, si),
          shapej(U.shapej, sj) {}
    ...
    KNM_ operator()(const SubArray &sa, const SubArray &sb) const { return KNM_(*this, sa, sb); }

    long ij(long i, long j) const { return shapei.index(i) + shapej.index(j); }
    long indexij(long i, long j) const { return this->index(shapei.index(i) + shapej.index(j)); }
...
template <class R> class KNMK_ : public KN_<R> {
  ...
  public:
    ...
    KNMK_(const KNMK_<R> &U, const SubArray &si, const SubArray &sj, const SubArray &sk)
        : KN_<R>(U, SubArray(U.ijk(si.len1(), sj.len1(), sk.len1()) + 1, U.ijk(si.start, sj.start, sk.start))),
    ...
    long ijk(long i, long j, long k) const { return shapei.index(i) + shapej.index(j) + shapek.index(k); }
    ...
    KNMK_ operator()(SubArray si, SubArray sj, SubArray sk) const { return KNMK_(*this, si, sj, sk); }
...
template <class R> class KN : public KN_<R> {
  public:
    ...
    KN(long nn, R *p) : KN_<R>(new R[nn], nn) { KN_<R>::operator=(KN_<R>(p, nn)); }
    KN(long nn, R (*f)(long i)) : KN_<R>(new R[nn], nn) {
        for (long i = 0; i < this->n; i++)
            this->v[i] = f(i);
    }
    KN(long nn, const R &a) : KN_<R>(new R[nn], nn) { KN_<R>::operator=(a); }
    KN(long nn, long s, const R a) : KN_<R>(new R[nn], nn, s) { KN_<R>::operator=(a); }
...
template <class R> class KNM : public KNM_<R> {
  public:
    ...
    KNM(long nn, long mm, R *p) : KNM_<R>(new R[nn], nn, mm) { KNM_<R>::operator=(KNM_<R>(p, nn, mm)); }
    KNM(const KNM<R> &u) : KNM_<R>(new R[u.size()], u.N(), u.M()) { KN_<R>::operator=(u); }
    explicit KNM(const KNM_<R> &u) : KNM_<R>(new R[u.size()], u.N(), u.M()) { KNM_<R>::operator=(u); }
    ~KNM() { delete[] this->v; }
...
template <class R> class KNMK : public KNMK_<R> {
  public:
    ...
    explicit KNMK(const KNMK_<R> &u) : KNMK_<R>(new R[u.size()], u.N(), u.M(), u.K()) { KNMK_<R>::operator=(u); }
    KNMK(const KNMK<R> &u) : KNMK_<R>(new R[u.size()], u.N(), u.M(), u.K()) { KNMK_<R>::operator=(u); }
...
template <class R> class Add_KN_ {
  public:
    const KN_<R> a;
    const KN_<R> b;
    Add_KN_(const KN_<R> &aa, const KN_<R> &bb) : a(aa), b(bb) { assert(SameShape(a, b)); }
};
...
template <class R> class Sub_KN_ {
  public:
    const KN_<R> a;
    const KN_<R> b;
    Sub_KN_(const KN_<R> &aa, const KN_<R> &bb) : a(aa), b(bb) { assert(SameShape(a, b)); }
};
...
template <class R> class DotMul_KN_ {
  public:
    const KN_<R> a;
    const KN_<R> b;
    DotMul_KN_(const KN_<R> &aa, const KN_<R> &bb) : a(aa), b(bb) { assert(SameShape(a, b)); }
};
...
template <class R> class Add_Mulc_KN_ {
  public:
    const KN_<R> a, b;
    const R ca, cb;
    Add_Mulc_KN_(const Mulc_KN_<R> &aa, const Mulc_KN_<R> &bb) : a(aa.a), b(bb.a), ca(aa.b), cb(bb.b) {
        assert(SameShape(a, b));
    }
    Add_Mulc_KN_(const Mulc_KN_<R> &aa, const KN_<R> &bb, const R cbb) : a(aa.a), b(bb), ca(aa.b), cb(cbb) {
        assert(SameShape(a, b));
    }
    Add_Mulc_KN_(const KN_<R> &aa, const R caa, const KN_<R> &bb, const R cbb) : a(aa), b(bb), ca(caa), cb(cbb) {
        assert(SameShape(a, b));
    }
};
...
template <class R> class Mul_KNM_KN_ {
  public:
    const KNM_<R> &A;
    const KN_<R> &b;
    Mul_KNM_KN_(const KNM_<R> &aa, const KN_<R> &bb) : A(aa), b(bb) { assert(SameShape(A.shapej, b)); }
};
...
template <class R> inline std::ostream &operator<<(std::ostream &f, const KN<R> &v) { return f << (const KN_<R> &)v; }
template <class R> inline std::ostream &operator<<(std::ostream &f, const KNM<R> &v) { return f << (const KNM_<R> &)v; }

template <class R> inline Add_KN_<R> operator+(const KN_<R> &a, const KN_<R> &b) { return Add_KN_<R>(a, b); }
template <class R> inline Sub_KN_<R> operator-(const KN_<R> &a, const KN_<R> &b) { return Sub_KN_<R>(a, b); }
template <class R> inline Mulc_KN_<R> operator*(const KN_<R> &a, const R &b) { return Mulc_KN_<R>(a, b); }
template <class R> inline Mulc_KN_<R> operator/(const KN_<R> &a, const R &b) { return Mulc_KN_<R>(a, 1 / b); }
template <class R> inline Mulc_KN_<R> operator*(const R &b, const KN_<R> &a) { return Mulc_KN_<R>(a, b); }
template <class R> inline Mulc_KN_<R> operator-(const KN_<R> &a) { return Mulc_KN_<R>(a, -1); }
template <class R> inline DotMul_KN_<R> operator*(const KN_<R> &a, const KN_<R> &b) { return DotMul_KN_<R>(a, b); }
...
template <class R> inline Mul_KNM_KN_<R> operator*(const KNM_<R> &A, const KN_<R> &b) { return Mul_KNM_KN_<R>(A, b); }

inline bool SameShape(const ShapeOfArray &a, const ShapeOfArray &b) { return a.SameShape(b); }
template <class R> inline bool SameShape(const ShapeOfArray &a, const Add_Mulc_KN_<R> &b) { return SameShape(a, b.a); }
template <class R> inline bool SameShape(const ShapeOfArray &a, const Add_KN_<R> &b) { return SameShape(a, b.a); }
template <class R> inline bool SameShape(const ShapeOfArray &a, const Sub_KN_<R> &b) { return SameShape(a, b.a); }
template <class R> inline bool SameShape(const ShapeOfArray &a, const Mulc_KN_<R> &b) { return SameShape(a, b.a); }
template <class R> inline bool SameShape(const ShapeOfArray &a, const Mul_KNM_KN_<R> &b) { return a.n == b.A.N(); }
