...
template <typename T> class SignElement {
...
// from a pattern
template <typename T> class SignPattern {
   typedef T Element;
   static const int nve = Element::nv;
 ...
 protected:
 public:
   Ubyte num_root_vert_; ///< number of vertices, where the level set function
                         ///< is zero.
   Ubyte num_root_;      ///< number of roots of the level set function;
                         ///< num_root_vert <= num_root
   byte sign_[Element::nv]; // Sign of the level set function of the vertices;
                            // \f$\in\{-1,0,1\}\f$
   Ubyte cut_simplex_[Element::nvc]; ///< local number with respect to the
                                     ///< reference tetra of the object on the
                                     ///< cut: [0,num_root_vert): vertex numbers
                                     ///< in (0..2); [num_root_vert, num_root):
                                     ///< edge numbers in (0..3). Both parts are
 ...
 public:
   ...
   SignPattern()
       : num_root_vert_(3), num_root_(2) {} ///< Uninitialized default state
   SignPattern(const byte ls[nve]) : sign_element(ls) {
      assign(ls);
   } ///< Assign the sign pattern on the vertices; throws if ls is identically
     ///< 0.
   ...
   void
   assign(const double ls[nve]); ///< Assign a sign pattern on the vertices;
                                 ///< throws if ls is identically 0.
   ...
   bool empty() const {
      return num_root_ == 0;
   } ///< True, iff there is no intersection.

   bool no_zero_vertex() const {
      return num_root_vert_ == 0;
   } ///< True, iff there is no vertex, in which ls vanishes.

   Ubyte num_cut_simplexes() const {
      return num_root_;
   } ///< Number of edges and vertices with a root of ls.
   Ubyte num_zero_vertexes() const {
      return num_root_vert_;
   } ///< Number of vertices of the tetra that are roots of ls.

   bool is_nd() const { return num_root_vert_ == dim + 1; }
   bool is_hyperPlan() const { return num_root_ > dim - 1; }

   /// Return local number of edges/verts with a root of ls. For edges, []
   /// returns a edge number in 0..5, and () returns an extended vertex number
   /// in 4..9.
   ///@{
...
template <typename T> void SignPattern<T>::assign(const byte ls[Element::nv]) {
   ...
   for (Ubyte i = 0; i < Element::nv; ++i)
      sum += (sign_[i] = ls[i]);
   if (sum == Element::nv || sum == -Element::nv)
      return;
   compute_cuts();
}
...
void SignPattern<T>::assign(const double ls[Element::nv]) {
   ...
   if (sum == Element::nv ||
       sum == -Element::nv) // optimize the case of uncut tetras
      return;
   compute_cuts();
}

template <typename T> void SignPattern<T>::compute_cuts() {
   for (Ubyte i = 0; i < Element::nv; ++i) {
      if (sign(i) == 0) {
...
template <typename E> int instance_idx(const byte ls[E::nv]) {
   int s = 0;
...
template <typename Element> class RefPatch {
 public:
   static const int nv  = Element::nv;
   static const int dim = Element::RdHat::d;

   typedef SortArray<Ubyte, dim>
       FaceIdx; ///< the vertices of a triangle of the cut: the tetra's vertices
                ///< are denoted by 0..3, the edge-cuts by edge-num + 4, which
                ///< is in 4..9.
   typedef const FaceIdx *const_face_iterator;
   typedef FaceIdx *face_iterator;

   /// \brief Initializes RefTetraPatchCL::instance_array_ below in the
   /// constructor by calling RefTetraPatchCL::instance() for all legal sign
   /// patterns.
   class InitializerCL {
    private:
 ...
 public:
   ...
   RefPatch(const SignPattern<Element> &cut) {
      assign(cut);
   } ///< Initialize with sign pattern on the vertices
   bool assign(const SignPattern<Element>
                   &cut); ///< Assign a sign pattern on the vertices; returns
                          ///< the value of empty()
 ...
 private:
   ...
   Ubyte
       is_boundary_face_; ///< true if the triangle is one of the tetra's faces.
 ...
 public:
   ...
   static inline const RefPatch<Element> &instance(const byte ls[Element::nv]) {
      ...
      RefPatch<Element> &instance =
          instance_array_[instance_idx<Element>(ls) + m0];
   ...
   // bool is_boundary_triangle () const { return is_boundary_triangle_ == 1; }
   // ///< true, iff the triangle is one of the tetra's faces.
...
template <typename E> class RefPartition {
 public:
   ...
   typedef SortArray<Ubyte, dim + 1>
       ElementIdx; // the vertices of a triangle of the cut:
   ...
   ElementIdx
       elements_[max_nb_element]; // upper bound => Hexa cut made of 10 tetra
   ...
   // e,f,g are assumed to be the equally-oriented edges of the quadrilateral
   // faces.
   void AddPentagone(Ubyte e0, Ubyte e1, Ubyte f0, Ubyte f1, Ubyte g1,
                     int sign) {
   ...
   // e,f,g are assumed to be the equally-oriented edges of the quadrilateral
   // faces.
   void AddPrism(Ubyte e0, Ubyte e1, Ubyte f0, Ubyte f1, Ubyte g0, Ubyte g1,
                 int sign) {
   ...
   void AddHexa(Ubyte e0, Ubyte e1, Ubyte e2, Ubyte e3, Ubyte e4, Ubyte e5,
                Ubyte e6, Ubyte e7, int sign) {
 ...
 public:
   ...
   int whatSign(const_element_iterator t) const {
      return t < elements_ + start_array ? -1 : 1;
   }
   ...
   // const_element_iterator element_begin (ElementSignEnum s = AllElement)
   // const { if(s == NoElement) return end_;
...
template <typename E>
RefPartition<E> RefPartition<E>::instance_array_[E::nb_sign_pattern];

template <typename E> int RefPartition<E>::InitializerCL::init_count_ = 0;

template <typename E> class CutElement {
...
template <typename E> class Virtual_Partition {
 ...
 public:
   virtual void get_list_node(std::vector<Rd> &node, int s) const      = 0;
   ...
   virtual Rd mapToPhysicalElement(const_element_iterator it,
                                   const RdHat Phat) const             = 0;
   virtual int get_sign_node(int i) const                              = 0;
};

// Class that does computation on cut elements from refPartition
template <typename E> class Partition : public Virtual_Partition<E> {
 ...
 public:
   ...
   //      : (((the_domain==0 && ls[0]>0) || (the_domain==1 && ls[0]<0))?
   //      AllElement : NoElement);
   //  }
   ...
   double measure(const_element_iterator it) const {
      ...
      for (int i = 0; i < dim + 1; ++i) { // triangle or tet
         ...
         if (idx < E::nv)
            ...
            const Ubyte v0 = Element::nvedge[idx - E::nv][0],
                        v1 = Element::nvedge[idx - E::nv][1];
   ...
   double measure(int s) const {
      ...
      R mes = 0;
      Rd N[dim + 1];
      for (const_element_iterator it = element_begin(s); it != element_end(s);
           ++it) {
 ...
 private:
   ...
   //     else {
   //       const Ubyte v0= Element::nvedge[idx - 3][0], v1= Element::nvedge[idx
   //       - 3][1]; const R t = -ls[v0]/(ls[v1]-ls[v0]); return (1-t) * ls[v0]
   //       + t * ls[v1] ;
   //     }
   //   }
 public:
   void get_list_node(std::vector<Rd> &node, int s) const; //{assert(0);};
   CutElement<E> get_element(int k) const {
      assert(0);
      return CutElement<E>();
   };
...
template <typename E> class Physical_Partition : public Virtual_Partition<E> {
 ...
 public:
   ...
   CutElement<E> get_element(int k) const {
      return CutElement<E>(vertices_, elements_idx[k]);
   }
   const_element_iterator element_begin(int s) const { return elements_idx; }
   const_element_iterator element_end(int s) const {
      return elements_idx + nb_element_;
   }

   void get_list_node(std::vector<Rd> &node, int s) const {
      assert(0);
      node = vertices_;
   }
   Rd get_vertex(const_element_iterator it, const int i) const {
   ...
   double measure(int domain) const {
      assert(0);
      return 0.;
   }
   double measure(const_element_iterator it) const {
