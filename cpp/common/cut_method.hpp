...
template <typename T> void SignPattern<T>::get_ordered_list_node(std::vector<int> &list_cut) const {
...
template <typename T> int SignPattern<T>::find_other_cutEdge_on_face(const int e0, int &f0) const {
    int e_next = -1;
    int f_next = -1;
...
template <typename Element> class RefPatch {
  public:
    ...
    typedef SortArray<Ubyte, dim> FaceIdx; ///< the vertices of a triangle of the cut: the tetra's vertices
  ...
  public:
    ...
    bool assign(const SignPattern<Element> &cut);              ///< Assign a sign pattern on the vertices; returns
                                                               ///< the value of empty()
...
template <typename E> RefPatch<E> RefPatch<E>::instance_array_[E::nb_sign_pattern];
...
template <typename E> class RefPartition {
  public:
    ...
    Ubyte first_uncut_edge(const SignPattern<E> &cut) const { return cut[0] == 1 ? 0 : (cut[1] == 2 ? 1 : 2); }

    //
  public:
    RefPartition() : begin_(elements_ + max_nb_element), end_(elements_) { InitializerCL(); }
...
template <typename E> class Partition : public Virtual_Partition<E> {
  ...
  public:
    ...
    Partition(const Element &t, const double lls[E::nv]) : patch(RefPartition<E>::instance(lls)), T(t) {
        for (int i = 0; i < E::nv; ++i)
            ls[i] = lls[i];
    }
...
template <typename E> class Physical_Partition : public Virtual_Partition<E> {
  ...
  public:
    ...
    const_element_iterator element_end(int s) const { return elements_idx + nb_element_; }

    void get_list_node(std::vector<Rd> &node, int s) const {
        assert(0);
        node = vertices_;
    }
    Rd get_vertex(const_element_iterator it, const int i) const { return vertices_[(*it)[i]]; }
