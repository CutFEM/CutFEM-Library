...
*/
#ifndef COMMON_GENERIC_INTERFACE_HPP
#define COMMON_GENERIC_INTERFACE_HPP
#include <iostream>
#include <cassert>
#include <bitset>
...
struct CutData3 {
   ...
   bool face_is_cut(int ifac) const {
      for (int e = 0; e < 3; ++e) {
         if (edge_is_cut(Element::edgeOfFace[ifac][e]))
            return true;
      }
      return false;
   }
...
template <int d> struct TypeCutData {
   typedef CutData2 CutData;
};
template <> struct TypeCutData<2> {
   typedef CutData2 CutData;
};
template <> struct TypeCutData<3> {
...
template <typename M> class GenericInterface {
 ...
 public:
   ...
   typedef std::pair<Uint, Uint>
       RemumberVertexPairT; ///< Helper type to handle zero-vertexes
 ...
 private:
   ...
   // Rd make_normal_noGrad(const typename Mesh::Element& K, const double
   // lset[Element::nv]);

 public:
   // GenericInterface() : backMesh(nullptr) { }
   GenericInterface(const Mesh &MM) : backMesh(&MM) {}
   GenericInterface(const Mesh &MM, const KN<double> &ls, int label)
       : backMesh(&MM) {
      make_patch(ls, label);
   }
   ...
   virtual Rd mapToFace(const FaceIdx &f,
...
void GenericInterface<M>::make_patch(const KN<double> &ls, int label) {
   ...
   std::vector<RemumberVertexPairT>
       zero_vertex_uses; // used to renumber the zero_vertexes
...
void GenericInterface<M>::add(const KN<double> &ls, int label) {
   ...
   for (int i = 0; i < ls.size(); ++i) {
      ...
      if (s_old * s_new < 0)
         ls_sign(i) = util::fsign(-1);
   }
}

template <typename M>
typename GenericInterface<M>::Rd
GenericInterface<M>::make_normal(const typename Mesh::Element &K,
                                 const double lset[Element::nv]) {
...
template <typename M>
const typename GenericInterface<M>::FaceIdx GenericInterface<M>::make_face(
    const typename RefPatch::FaceIdx &ref_tri, const typename Mesh::Element &K,
    const double lset[Element::nv],
   ...
   for (Uint j = 0; j < nve; ++j) {
      ...
      if (loc_vert_num < K.nv) { // zero vertex
         ...
         Rd Q =
             (1.0 - t) * ((Rd)K[i0]) + t * ((Rd)K[i1]); // linear interpolation
         vertices_.push_back(Q);
         triIdx[j] = vertices_.size() - 1;
...
//
//
//   std::vector<RemumberVertexPairT> zero_vertex_uses; // used to renumber the
//   zero_vertexes const Uint nb_vertex_K = Element::nv; double
//   loc_ls_sign[nb_vertex_K];
//
//
...
template <typename M> double GenericInterface<M>::distance(Rd P) const {
   double l = 1e300;
