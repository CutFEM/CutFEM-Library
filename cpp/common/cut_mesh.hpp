...
template <typename E> struct Cut_Part {
    ...
    Cut_Part(const Partition<E> p, int s) : ip(p), sign_cut_(s), pp(p.T) { partition_ = &ip; }
    Cut_Part(const Physical_Partition<E> p, int s) : pp(p), sign_cut_(s), ip(p.T) { partition_ = &pp; }
    ...
    void get_list_node(std::vector<typename E::Rd> &node) const { partition_->get_list_node(node, sign_cut_); }
    ...
    // ITERATORS
    const_element_iterator element_begin() const { return partition_->element_begin(sign_cut_); }
    const_element_iterator element_end() const { return partition_->element_end(sign_cut_); }
...
template <typename Mesh> class ActiveMesh {
  ...
  private:
    ...
    int idxK_in_domain(int k, int i) const { return k - this->idx_element_domain[i]; }
...
template <typename Mesh>
Physical_Partition<typename Mesh::Element::Face> ActiveMesh<Mesh>::build_local_partition(Face &face, const int k,
