...
template <typename Mesh> class ActiveMesh {
 ...
 public:
   ...
   void info() const {
      ...
      std::cout << " Number of subdomains          :\t" << get_nb_domain()
                << std::endl;
      for (int i = 0; i < get_nb_domain(); ++i) {
         std::cout << " Number of elements in Omega_" << i << " :\t"
                   << this->get_nb_element(i) << std::endl;
      }
      std::cout << " Total number of elements      :\t"
                << this->get_nb_element() << std::endl;
   }
...
void ActiveMesh<Mesh>::add(const Interface<Mesh> &interface, int sign_domain) {
   ...
   for (int d = 0; d < dom_size; ++d) {
      ...
      for (auto it_k = idx_from_background_mesh_[d].begin();
         ...
         int nb_interface =
             (it_gamma == interface_id_[0].end()) ? 0 : it_gamma->second.size();
...
template <typename Mesh>
void ActiveMesh<Mesh>::addArtificialInterface(
   ...
   bool sub_is_cut = false;
   for (auto it_k = idx_from_background_mesh_[0].begin();
        it_k != idx_from_background_mesh_[0].end(); ++it_k) {
      ...
      if (signK.cut()) {
         interface_id_[0][std::make_pair(0, k)].push_back(
             std::make_pair(&interface, sign_domain));
      }
   }
}
