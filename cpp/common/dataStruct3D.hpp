...
struct DataTriangle3 {
    ...
    static R mesure(V *pv[NbOfVertices]) { return (R3(*pv[0], *pv[1]) ^ R3(*pv[0], *pv[2])).norme() * 0.5; }
};
...
class Quad3 : public GenericElement<DataQuad3> {
  public:
    ...
    Rd operator()(const RdHat &Phat) const {
        Rd r =
            (1. - Phat.sum()) * (*(Rd *)vertices[0]) + Phat[0] * (*(Rd *)vertices[1]) + Phat[1] * (*(Rd *)vertices[3]);
        return r;
    }
    // std::array<int, 2> index_adjacent_element_;
    // void set_adjacent_element(int k1, int k2) {
...
class Hexa : public GenericElement<DataHexa> {
  public:
    ...
    Hexa(Vertex *v0, int *iv, int r = 0, double mss = globalVariable::UnSetMesure) {
        this->set(v0, iv, r, mss);
    }; // constructor empty for array

    Rd operator()(const RdHat &Phat) const {
        Rd r = (1. - Phat.sum()) * (*(Rd *)vertices[0]) + Phat[0] * (*(Rd *)vertices[1]) +
               Phat[1] * (*(Rd *)vertices[3]) + Phat[2] * (*(Rd *)vertices[4]);
        return r;
    }
...
// Forward declaration
template <> const std::vector<std::vector<int>> GenericElement<DataTriangle3>::nvedge;
template <> const std::vector<std::vector<int>> GenericElement<DataTriangle3>::nvface;
template <> const std::vector<std::vector<int>> GenericElement<DataTriangle3>::nvhyperFace;
template <> const std::vector<std::vector<int>> GenericElement<DataTriangle3>::edgeOfFace;
...
template <> const std::vector<std::vector<int>> GenericElement<DataTet>::edgeOfFace;
template <> const std::vector<std::vector<int>> GenericElement<DataTet>::faceOfEdge;
template <> const std::vector<std::vector<int>> GenericElement<DataTet>::commonVertOfEdges;

template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::nvedge;
template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::nvface;
template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::nvhyperFace;
template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::edgeOfFace;
template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::faceOfEdge;
template <> const std::vector<std::vector<int>> GenericElement<DataHexa>::commonVertOfEdges;
