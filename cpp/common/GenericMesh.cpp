...
template <typename GMesh>
void BuildAdjacencyOfMesh<GMesh>::addFaceAlreadySeen(
    typename std::map<SArray, int>::iterator p, const SArray &a) {
   assert(p->second >= 0);
...
void BuildAdjacencyOfMesh<GMesh>::addBoundary(const int k) {
   ...
   if (p == h.end()) {
      ...
      mesh.BoundaryElementHeadLink[k] =
          p->second < 0 ? -p->second - 1 : p->second;
   }
}
...
template class BuildAdjacencyOfMesh<
    GenericMesh<Triangle2, BoundaryEdge2, Vertex2>>;
