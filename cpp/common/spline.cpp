...
void Spline::init(const vector<double> &x, const vector<double> &y) {
   ...
   for (int i = 1; i < n; ++i)
      alpha[i] =
          (3 * (a[i + 1] - a[i]) / h[i] - 3 * (a[i] - a[i - 1]) / h[i - 1]);
...
double Spline::evaluate(const double x) const {
   ...
   double y  = mySplineSet[j].a + mySplineSet[j].b * dx +
              mySplineSet[j].c * dx * dx + mySplineSet[j].d * dx * dx * dx;
   return y;
}
double Spline::evaluate(int j, const double x) const {
   ...
   if (j == X.size() - 2) {
      if (x < mySplineSet[j].x) {
         std::cout << " Spline evaluation on specified interval failed => "
                      "return classic evaluation"
                   << std::endl;
         return evaluate(x);
      }
   } else if (x < mySplineSet[j].x || mySplineSet[j + 1].x < x) {
      std::cout << " Spline evaluation on specified interval failed => return "
                   "classic evaluation"
                << std::endl;
      return evaluate(x);
   }
...
R2 Spline2D::evaluate(double t) const { return R2(splineX(t), splineY(t)); }
...
void display_spline2d(const Spline &splineX, const Spline &splineY,
