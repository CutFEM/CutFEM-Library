...
inline void
copy_levelset_sign (const KN<double>& src, KN<byte>& dst)
{
  dst.resize( src.size());
...
public :
  MuteStdOstream() : bout_(std::cout.rdbuf()), berr_(std::cerr.rdbuf()),
		     blog_(std::clog.rdbuf()) {}

  void Mute( std::ostream& os) {os.rdbuf(&devnull_);}
  void Mute() {Mute(std::cout); Mute(std::clog);}
  void Recover() const {std::cout.rdbuf(bout_); std::cerr.rdbuf(berr_);
