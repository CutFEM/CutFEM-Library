...
// create a function that interpolate a cutFEM fct on a background mesh with help of levelset
template <typename Mesh> void set_velocity(FunFEM<Mesh> &uh, FunFEM<Mesh> &vel, FunFEM<Mesh> &ls, R tt) {

    typedef GFESpace<Mesh> FESpace;
    typedef typename FESpace::FElement FElement;
    using cutmesh_t = ActiveMesh<Mesh>;
    using Partition = Partition<typename cutmesh_t::Element>;
    ...
    for (int k = 0; k < velVh.NbElement(); ++k) {
        ...
        if (cutK.is_cut()) {
            ...
            int k = (loc_ls[0] > 0) ? Vh.idxElementFromBackMesh(kback, 0) : Vh.idxElementFromBackMesh(kback, 1);
