...
template <typename Mesh, typename L> class AlgoimMacro : public AlgoimBaseMacro<Mesh> {
  public:
  ...
  private:
    ...
    const ActiveMesh<Mesh> &Th;
    void findSmallElement(const TimeSlab * = nullptr, const QuadratureFormular1d * = nullptr) override;
};
...
AlgoimMacro<Mesh, L>::AlgoimMacro(const ActiveMesh<Mesh> &Th_, const double C_, L &phi_, const TimeSlab &In_,
                                  const QuadratureFormular1d &qTime_)
    : phi(phi_), Th(Th_), AlgoimBaseMacro<Mesh>(Th_, C_, In_, qTime_) {
...
//                     else {
//                         MK.inner_edge.push_back(std::make_pair(kn, je));
//                     }
//                 }
//             }
//         }
