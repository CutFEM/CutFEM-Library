...
class TypeOfFE_RT1_2d : public InitTypeOfRTk_2d, public GTypeOfFE<Mesh2> {
 ...
 public:
   ...
   TypeOfFE_RT1_2d()
       : InitTypeOfRTk_2d(1), GTypeOfFE<Mesh2>(
                                  ndf, 2, Data,
                                  2 * 2 * 3 * QFE.n +
                                      QFK.n * 4,    // nb coef mat interpole
   ...
   void get_Coef_Pi_h(const GbaseFElement<Mesh2> &K, KN_<double> &v)
       const { // compute the coef of interpolation ...
      const Element &T     = K.T;
      int k                = 0;
      int arrEdgeOrient[3] = {T.EdgeOrientation(0), T.EdgeOrientation(1),
                              T.EdgeOrientation(2)};
      double aa            = T.measure();
      double s[3]          = {1., 1.,
                              1.}; //{T.lenEdge(0)/aa, T.lenEdge(1)/aa, T.lenEdge(2)/aa};
...
void TypeOfFE_RT1_2d::FB_Freefem(const What_d whatd, const Element &K,
                                 const Rd &Phat, RNMK_ &bfMat) const {
   ...
   double s[4] = {
       1., 1., 1.,
       1.}; //{K.lenEdge(0)/aa, K.lenEdge(1)/aa, K.lenEdge(2)/aa, sb};
   ...
   if ((whatd & Fop_D1) || (whatd & Fop_D2)) {
      R2 DL[3] = {K.H(0), K.H(1),
                  K.H(2)}; // [heights?? represents derivative of refBaryc]
      ...
      if (whatd & Fop_dx) {
         for (int dof = 0; dof < 8; ++dof) {
            ...
            for (int k = 0; k < 3; ++k) {
               ...
               fd += cI[dof][k] * (DL[lI[dof][k]].x * phi[pI[dof][k]] +
                                   refBaryc[lI[dof][k]] * e1);
            }
      ...
      if (whatd & Fop_dy) {
         for (int dof = 0; dof < 8; ++dof) {
            ...
            for (int k = 0; k < 3; ++k) {
               fd += cI[dof][k] * (DL[lI[dof][k]].y * phi[pI[dof][k]] +
                                   refBaryc[lI[dof][k]] * e2);
            }
      ...
      if (whatd & Fop_D2) {
         ...
         if (whatd & Fop_dxy) {
            for (int dof = 0; dof < 8; ++dof) {
               ...
               for (int k = 0; k < 3;
                    ++k) { // [take expression from Fop_dx and diff wrt y]
                  // std::cout << pI[dof][k] << std::endl;
                  ...
                  // e1);
                  fd += cI[dof][k] *
                        (DL[lI[dof][k]].x * e2 + DL[lI[dof][k]].y * e1);
               }
         ...
         if (whatd & Fop_dyy) {
            for (int dof = 0; dof < 8; ++dof) {
               ...
               for (int k = 0; k < 3; ++k) {
                  ...
                  // e2);
                  fd += cI[dof][k] *
                        (DL[lI[dof][k]].y * e2 + DL[lI[dof][k]].y * e2);
               }
