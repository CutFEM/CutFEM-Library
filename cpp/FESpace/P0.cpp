...
void TypeOfFE_P0Polynomial1d::FB(const What_d whatd, const Element &K, const R1 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
...
void TypeOfFE_P0Lagrange2d::FB(const What_d whatd, const Element &K, const R2 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
...
//   public:
//     static const int k   = 0;
//     static const int ndf = 0;
//     static int Data[];
//     static double alpha_Pi_h[];

//     TypeOfFE_NumberSpace2d() : GTypeOfFE<Mesh2>(0, 0, Data, 0, 0, alpha_Pi_h) {
...
// P0 Quad 2D
class TypeOfFE_P0LagrangeQ2d : public GTypeOfFE<MeshQuad2> {
  ...
  public:
    ...
    TypeOfFE_P0LagrangeQ2d() : GTypeOfFE<MeshQuad2>(1, 1, Data, 1, 1, alpha_Pi_h) {
...
double TypeOfFE_P0LagrangeQ2d::alpha_Pi_h[] = {1.};

void TypeOfFE_P0LagrangeQ2d::FB(const What_d whatd, const Element &K, const R2 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
...
static TypeOfFE_P0LagrangeQ2d P0Q_2d;
GTypeOfFE<MeshQuad2> &P0LagrangeQ2d(P0Q_2d);
template <> GTypeOfFE<MeshQuad2> &DataFE<MeshQuad2>::P0 = P0Q_2d;
...
void TypeOfFE_P0ScLagrange2d::FB(const What_d whatd, const Element &K, const R2 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
...
void TypeOfFE_P0Lagrange3d::FB(const What_d whatd, const Element &K, const R3 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
...
void TypeOfFE_P0LagrangeQ3d::FB(const What_d whatd, const Element &K, const R3 &P, RNMK_ &val) const {
    assert(val.N() >= 1);
    assert(val.M() == 1);
