...
class TypeOfFE_RT0_2d : public GTypeOfFE<Mesh2> {
  ...
  public:
    ...
    TypeOfFE_RT0_2d()
        : GTypeOfFE<Mesh2>(3,    // #dof on the element
                           2,    // dim of basis fun
                           ...
                           )  // Inputs to GTypeOfFE
    {
        ...
        for (int l = 0, k = 0; l < nbDof; l++) { // Fortin interpolator (defines p_l=coorDof[l], j_k, l_k (or
                                                 // p_k), and i_k)
...
void TypeOfFE_RT0_2d::FB(const What_d whatd, const Element &K, const R2 &PHat, RNMK_ &bfMat) const {
...
void TypeOfFE_RT0m_2d::FB(const What_d whatd, const Element &K, const R2 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 3); // if statement inside not true -- STOP code, in this
                            // case bfMat.N() = #rows of matrix = 3
    bfMat = 0.;             // set to zeros Basis function matrix: rows dofs; cols id, dx, dy
    R2 P(K(PHat));
...
void TypeOfFE_RT0_3d::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 4);
    assert(bfMat.M() == 3);
...
class TypeOfFE_RT0_scaled_face_3d : public GTypeOfFE<Mesh3> {
  public:
    typedef Mesh3 Mesh;
    typedef typename Mesh::Element Element;
    // typedef GFElement<Mesh3> FElement;
...
int TypeOfFE_RT0_scaled_face_3d::dfon[] = {0, 0, 1, 0};
int TypeOfFE_RT0_scaled_face_3d::Data[]{
    // geometry=vertex,edge,face,volume; 0,1,2 vertices, 3,4,5 edges, 6 face
    10, 11, 12, 13, // on what hard coded geometry the dofs are located
    0,  0,  0,  0,  // the (number of the dof - 1) on corresponding geometry/node (here each edge has 1 dof)
    0,  1,  2,  3,  // the geometry(vertex) of the basis fcn corresponding to the dof
...
TypeOfFE_RT0_scaled_face_3d::TypeOfFE_RT0_scaled_face_3d() : GTypeOfFE<Mesh3>(4, 3, Data, 12, 4) {
...
void TypeOfFE_RT0_scaled_face_3d::get_Coef_Pi_h(const GbaseFElement<Mesh> &K, KN_<double> &v) const {
    const Element &T = K.T;
    for (int f = 0, k = 0; f < 4; f++) {
        R3 N = T.N_notNormalized(f); //  exterior and  ||N|| = 2* area f
        N *= T.faceOrient(f) / 2.;
...
void TypeOfFE_RT0_scaled_face_3d::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 4);
    assert(bfMat.M() == 3);
    // wi = signe * (x - qi)/ (volume*d)
    bfMat   = 0;
...
static TypeOfFE_RT0_scaled_face_3d RT0_scaled_face_3d;
GTypeOfFE<Mesh3> &RT0scaledface3d(RT0_scaled_face_3d);
template <> GTypeOfFE<Mesh3> &DataFE<Mesh3>::RT0_scaled_face = RT0scaledface3d;
