                                        ...
                                        KN_<double> &v) const // ,int ocoef,int odf,int *nump ) const
{
    ...
    for (int e = 0; e < 6; ++e) {
        ...
        R3 E = T.EdgeOrientation(e) * T.Edge(e); //  exterior and  ||N|| = 2* area f
        R scaling = T.Edge(e).norm();


        for (int q = 0; q < QFE.n; ++q, ++p) {
            for (int c = 0; c < 3; c++, i++) {

                v[i] = E[c] * QFE[q].a / scaling;   
...
void TypeOfFE_Ned0_scaled_edge_kind1::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    ...
    if (whatd & Fop_D0) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            R scaling = K.Edge(i).norm();

            R3 wi = scaling * (l[i0] * D[i1] - l[i1] * D[i0]); 
    ...
    if (whatd & Fop_D1) {
        for (int i = 0; i < 6; ++i) {
            ...
            if (whatd & Fop_dx) {
                R3 wi              = scaling*(D[i0].x * D[i1] - D[i1].x * D[i0]);
                bfMat(i, 0, op_dx) = wi.x;
                bfMat(i, 1, op_dx) = wi.y;
                bfMat(i, 2, op_dx) = wi.z;
            }
            if (whatd & Fop_dy) {
                R3 wi              = scaling*(D[i0].y * D[i1] - D[i1].y * D[i0]); 
                bfMat(i, 0, op_dy) = wi.x;
                bfMat(i, 1, op_dy) = wi.y;
                bfMat(i, 2, op_dy) = wi.z;
            }
            if (whatd & Fop_dz) {
                R3 wi              = scaling*(D[i0].z * D[i1] - D[i1].z * D[i0]);
                bfMat(i, 0, op_dz) = wi.x;
                bfMat(i, 1, op_dz) = wi.y;
                bfMat(i, 2, op_dz) = wi.z;
            }
