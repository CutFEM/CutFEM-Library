...
class TypeOfFE_Ned0_scaled_edge_kind1 : public GTypeOfFE<Mesh3> {
  public:
    typedef Mesh3 Mesh;
    typedef Mesh3::Element Element;
    // typedef GFElement<Mesh3> FElement;
    ...
    static const int d = Mesh::Rd::d;
    const QuadratureFormular1d &QFE; //   static const GQuadratureFormular<R1> QFe;
    TypeOfFE_Ned0_scaled_edge_kind1();
    // int edgeface[4][3] ;
    static int Data[];
...
// int TypeOfFE_Ned0_kind1::dfon[]={0,1,0,0};
int TypeOfFE_Ned0_scaled_edge_kind1::Data[]{
    // geometry=vertex,edge,face,volume; 0,1,2 vertices, 3,4,5 edges, 6 face
    4, 5, 6, 7, 8, 9, // on what hard coded geometry the dofs are located
    0, 0, 0, 0, 0, 0, // the (number of the dof - 1) on corresponding geometry/node (here each edge has 1 dof)
    0, 1, 2, 3, 4, 5, // the geometry(vertex) of the basis fcn corresponding to the dof
...
TypeOfFE_Ned0_scaled_edge_kind1::TypeOfFE_Ned0_scaled_edge_kind1()
    : GTypeOfFE<Mesh3>(6, 3, Data,
                       18 * 2, // Element::ne*3*QFe.n,
                       6 * 2), // Element::ne*QFe.n)
      QFE(QF_GaussLegendre2) {
...
void TypeOfFE_Ned0_scaled_edge_kind1::get_Coef_Pi_h(const GbaseFElement<Mesh> &K,
                                        KN_<double> &v) const // ,int ocoef,int odf,int *nump ) const
{
    // compute de coef d'interpolation
    const Element &T = K.T;
...
void TypeOfFE_Ned0_scaled_edge_kind1::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 6);
    assert(bfMat.M() == 3);
    R l[] = {1. - PHat.sum(), PHat.x, PHat.y, PHat.z};
    R3 D[4];
...
static TypeOfFE_Ned0_scaled_edge_kind1 Ned0_scaled_edge_kind1;
GTypeOfFE<Mesh3> &Ned0scalededgekind1(Ned0_scaled_edge_kind1);
template <> GTypeOfFE<Mesh3> &DataFE<Mesh3>::Ned0_scaled_edge = Ned0scalededgekind1;



class TypeOfFE_Ned0_scaled_kind1 : public GTypeOfFE<Mesh3> {
  public:
    typedef Mesh3 Mesh;
    typedef Mesh3::Element Element;
    // typedef GFElement<Mesh3> FElement;
...
int TypeOfFE_Ned0_scaled_kind1::Data[]{
    // geometry=vertex,edge,face,volume; 0,1,2 vertices, 3,4,5 edges, 6 face
    4, 5, 6, 7, 8, 9, // on what hard coded geometry the dofs are located
    0, 0, 0, 0, 0, 0, // the (number of the dof - 1) on corresponding geometry/node (here each edge has 1 dof)
    0, 1, 2, 3, 4, 5, // the geometry(vertex) of the basis fcn corresponding to the dof
...
TypeOfFE_Ned0_scaled_kind1::TypeOfFE_Ned0_scaled_kind1()
    : GTypeOfFE<Mesh3>(6, 3, Data,
                       18 * 2, // Element::ne*3*QFe.n,
                       6 * 2), // Element::ne*QFe.n)
      QFE(QF_GaussLegendre2) {
...
void TypeOfFE_Ned0_scaled_kind1::get_Coef_Pi_h(const GbaseFElement<Mesh> &K,
                                        KN_<double> &v) const // ,int ocoef,int odf,int *nump ) const
{
    // compute de coef d'interpolation
    const Element &T = K.T;
...
void TypeOfFE_Ned0_scaled_kind1::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 6);
    assert(bfMat.M() == 3);
    R l[] = {1. - PHat.sum(), PHat.x, PHat.y, PHat.z};
    R3 D[4];
...
static TypeOfFE_Ned0_scaled_kind1 Ned0_scaled_kind1;
GTypeOfFE<Mesh3> &Ned0_scaledkind1(Ned0_scaled_kind1);
template <> GTypeOfFE<Mesh3> &DataFE<Mesh3>::Ned0_scaled = Ned0_scaledkind1;
...
// v -> h*v
// Nedelec type 1 element 3D
class TypeOfFE_Ned0_kind1_mult : public GTypeOfFE<Mesh3> {
  public:
    typedef Mesh3 Mesh;
    typedef Mesh3::Element Element;
    // typedef GFElement<Mesh3> FElement;
...
// int TypeOfFE_Ned0_kind1::dfon[]={0,1,0,0};
int TypeOfFE_Ned0_kind1_mult::Data[]{
    // geometry=vertex,edge,face,volume; 0,1,2 vertices, 3,4,5 edges, 6 face
    4, 5, 6, 7, 8, 9, // on what hard coded geometry the dofs are located
    0, 0, 0, 0, 0, 0, // the (number of the dof - 1) on corresponding geometry/node (here each edge has 1 dof)
    0, 1, 2, 3, 4, 5, // the geometry(vertex) of the basis fcn corresponding to the dof
...
TypeOfFE_Ned0_kind1_mult::TypeOfFE_Ned0_kind1_mult()
    : GTypeOfFE<Mesh3>(6, 3, Data,
                       18 * 2, // Element::ne*3*QFe.n,
                       6 * 2), // Element::ne*QFe.n)
      QFE(QF_GaussLegendre2) {
...
void TypeOfFE_Ned0_kind1_mult::get_Coef_Pi_h(const GbaseFElement<Mesh> &K,
                                        KN_<double> &v) const // ,int ocoef,int odf,int *nump ) const
{
    // compute de coef d'interpolation
    const Element &T = K.T;
...
void TypeOfFE_Ned0_kind1_mult::FB(const What_d whatd, const Element &K, const R3 &PHat, RNMK_ &bfMat) const {
    assert(bfMat.N() >= 6);
    assert(bfMat.M() == 3);
    R l[] = {1. - PHat.sum(), PHat.x, PHat.y, PHat.z};
    R3 D[4];
...
static TypeOfFE_Ned0_kind1_mult Ned0_kind1_mult;
GTypeOfFE<Mesh3> &Ned0kind1mult(Ned0_kind1_mult);
template <> GTypeOfFE<Mesh3> &DataFE<Mesh3>::Ned0_mult = Ned0kind1mult;
