...
*/
template <typename F>
void interpolate(const F &Mh, KN_<double> &fh, R (*f)(double *)) {
   ...
   KN<R> ggf(
       Mh.MaxNbDFPerElement); // stock the values of the dof of the interpolate
   ...
   for (int t = 0; t < Mh.NbElement(); t += 1) {
      ...
      for (int p = 0; p < K.tfe->NbPtforInterpolation;
...
*/
template <typename F>
void interpolate(const F &Mh, KN_<double> &fh,
                 R (*f)(double *, int ii, int dom)) {
   typedef typename F::Rd Rd;
   typedef typename F::Element::RdHat RdHat;
   assert(fh.size() == Mh.nbDoF);
...
template <typename F>
void interpolate(const F &Mh, KN_<double> &fh,
                 R (*f)(double *, int i, int domain, R t), R tid) {
   // std::cout << " need to double check this interpolate function and add MPI"
   // << std::endl; assert(0);
   typedef typename F::Rd Rd;
   typedef typename F::Element::RdHat RdHat;
...
*/
template <typename F>
void interpolate(const F &Mh, KN_<double> &fh, R (*f)(double *, int, R),
                 R tid) {
   // std::cout << " need to double check this interpolate function and add MPI"
   // << std::endl; assert(0);
   typedef typename F::Rd Rd;
...
*/
template <typename F>
void interpolate(const F &Mh, const TimeSlab &In, KN_<double> &fh,
                 R (*f)(double *, int, R)) {
   // std::cout << " need to double check this interpolate function and add MPI"
   // << std::endl; assert(0);
   typedef typename F::Rd Rd;
