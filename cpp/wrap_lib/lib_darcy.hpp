...
template <typename Kernel> class Darcy {
  ...
  public:
    ...
    void add_bulk_integral(double (*f)(double *, int, int)) {
        ...
        darcy.addBilinear(innerProduct(u, v) - innerProduct(p, div(v)) + innerProduct(div(u), q), *Khi_p);
    ...
    void add_macro_stabilization(double dlt_i) {
        ...
        darcy.addFaceStabilization(innerProduct(Cu * h_i * jump(u),
    ...
    double L2error_p(double (*f)(double *, int, int)) {
        ...
        FunFEM<Mesh> ph(*Ph_p.get(), data_ph);
    ...
    void write_vtk_file(std::string filename) {
        ...
        FunFEM<Mesh> uh(*Wh_p.get(), data_uh);
        FunFEM<Mesh> ph(*Ph_p.get(), data_ph);
        auto femSol_0dx = dx(uh.expr(0));
        auto femSol_1dy = dy(uh.expr(1));
...
template <> void Darcy<KernelDarcy2>::init_mesh() {
    Kh_p = std::make_shared<Mesh>(mesh_param.nx[0], mesh_param.nx[1], mesh_param.orx[0], mesh_param.orx[1],
