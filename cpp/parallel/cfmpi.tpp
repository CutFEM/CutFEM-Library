...
template <typename T> inline long MPIcf::Bcast(T &a, int who, int size) {
   return WBcast(&a, size, who, Communicator_);
}

template <typename T>
inline long MPIcf::Bcast(T &a, int who, int size, const MPI_Comm &comm) {
...
inline void MPIcf::Reduce(const T &myData, T &globalData, int size,
   ...
   MPI_Reduce(&myData, &globalData, size, MPI_TYPE<T>::TYPE(), op, root,
              Communicator_);
}
...
template <typename T>
inline void MPIcf::AllReduce(const T &myData, T &globalData, const MPI_Op &op) {
   MPI_Allreduce(&myData, &globalData, 1, MPI_TYPE<T>::TYPE(), op,
                 Communicator_);
}
...
inline void MPIcf::AllReduce(const T &myData, T &globalData, int size,
                             const MPI_Op &op) {
   MPI_Allreduce(&myData, &globalData, size, MPI_TYPE<T>::TYPE(), op,
                 Communicator_);
}
...
inline void MPIcf::AllReduce(const KN<T> &myData, KN<T> &globalData,
   ...
   MPI_Allreduce((T *)(myData), globalData, myData.size(), MPI_TYPE<T>::TYPE(),
                 op, Communicator_);
}
