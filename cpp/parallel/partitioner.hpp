...
// struct GraphPart {                              // struct with input for
// partitioner
 ...
 public:
   ...
   virtual void reorder(const KN<double> &rhs, KN<double> &rhsMapped) const {
      ...
      for (int i = 0; i < rhs.size(); ++i)
         rhsMapped(iperm_[i]) = rhs(i);
   }

   virtual void inverseMapp(const KN<double> &rhs,
                            KN<double> &rhsMapped) const {
      rhsMapped.resize(rhs.size());
      for (int i = 0; i < rhs.size(); ++i)
         rhsMapped(i) = rhs(iperm_[i]);
   }
...
class ConsecutiveMatrixOrdering : public MatrixOrdering {
 ...
 public:
   ConsecutiveMatrixOrdering(const int nn,
...
//   MetisMatrixOrdering(const int nn, const std::map<std::pair<int,int>,double>
//   & M) :
//     MatrixOrdering(nn) {
//     R t0 = MPIcf::Wtime();
...
//     ParMetisMatrixOrdering(const int nn, const
...
//   IndexArray eptr;                     // starting value of nodes of element
//   in eind IndexArray eind;                     // node of element IndexArray
//   vwgt;                     // weight of element IndexArray vsize; // size of
//   element (not used) idx_t ncommon;                       // number of nodes
//   for connected elements idx_t nparts;                        // nb of
//   partitions real_t * tpwgts;                     // weight for each
//   partition IndexArray options; idx_t objval; IndexArray epart; // partition
//   of the element IndexArray npart;                    // partition of the
//   nodes
...
//   // std::cout << " CPU Time For BuildGraph \t " << cpuend - cpubegin <<
//   std::endl;
...
//   int msg = METIS_PartMeshDual(&ne,
//   &nn,eptr,eind,vwgt,vsize,&ncommon,&nparts,tpwgts,
