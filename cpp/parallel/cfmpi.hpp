...
template<> struct MPI_TYPE<Uint>    { static MPI_Datatype TYPE(){return MPI_UNSIGNED;}};
template<> struct MPI_TYPE<Ulint>   { static MPI_Datatype TYPE(){return MPI_UNSIGNED_LONG;}};
template<> struct MPI_TYPE<Usint>   { static MPI_Datatype TYPE(){return MPI_UNSIGNED_SHORT;}};
template<> struct MPI_TYPE<double>  { static MPI_Datatype TYPE(){return MPI_DOUBLE;}};
template<> struct MPI_TYPE<char>    { static MPI_Datatype TYPE(){return MPI_CHAR;}};
template<> struct MPI_TYPE<byte>    { static MPI_Datatype TYPE(){return MPI_CHAR;}};
...
template<class R>
long WBcast(R * v,int  n,int who,MPI_Comm comm)
{
  assert(v && n>0);
...
public :
  ...
  static void setLoopSplitWork(string f){ loopDivision = (f=="block")?1 : 0;}
  ...
  static inline long Send(double &a,int who){return WSend(&a,1,who,
							  MPI_TAG<double>::TAG,Communicator_,rq);}
  static inline long Send(long &a,int who)  {return WSend(&a,1,who,
							  MPI_TAG<long>::TAG,Communicator_,rq);}

  static inline long Recv(double &a,int who){return WRecv(&a,1,who,
							  MPI_TAG<double>::TAG,Communicator_,rq);}
  static inline long Recv(long &a, int who){return WRecv(&a,1,who,
							 MPI_TAG<long>::TAG ,Communicator_,rq);}


  template<typename T>
  static inline long Bcast(T &a, int who, int size);
  template<typename T>
  static inline long Bcast(T &a, int who, int size, const MPI_Comm& comm);
  //   return WBcast(&a, 1, who,comm);}
  // static inline long Bcast(int &a, int who)   {return WBcast(&a, 1, who,Communicator_);}
  // static inline long Bcast(int &a, int who, const MPI_Comm& comm)   {
  //   return WBcast(&a, 1, who,comm);}
  // static inline long Bcast(long &a, int who)   {return WBcast(&a, 1, who,Communicator_);}
  // static inline long Bcast(long &a, int who,const MPI_Comm& comm )   {
  //   return WBcast(&a, 1, who,comm);}


  // template<typename T>
  // static inline long Bcast(T &a, int who); {return WBcast(&a, 1, who,Communicator_);}
  template<typename T>
  static inline long Bcast(const KN<T> &a, int who) ;
  template<typename T>
