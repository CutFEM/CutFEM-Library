...
TEST_CASE("Test class MeshQuad 2D", "[mesh]") {
    ...
    // Test mappings between physical element and reference element
    MeshQuad2 mesh2(13, 5, -1.5, -1., 3., 2.);  // hx = 0.25, hy = 0.5
    const auto &T0(mesh2[0]); // first physical element

    // From reference elememt to physical element:

    // assert that the vertices of the reference element maps to
    // the vertices of the physical element
    REQUIRE(isEqual(T0(R2(0., 0.))[0], -1.5));
    REQUIRE(isEqual(T0(R2(0., 0.))[1], -1.));

    REQUIRE(isEqual(T0(R2(1., 0.))[0], -1.25));
    REQUIRE(isEqual(T0(R2(1., 0.))[1], -1.));

    REQUIRE(isEqual(T0(R2(0., 1.))[0], -1.5));
    REQUIRE(isEqual(T0(R2(0., 1.))[1], -.5));

    REQUIRE(isEqual(T0(R2(1., 1.))[0], -1.25));
    REQUIRE(isEqual(T0(R2(1., 1.))[1], -.5));

    // assert that the center of the reference element (0.5, 0.5) maps to
    // the center of the physical element (-1.375, -0.75)
    REQUIRE(isEqual(T0(R2(.5, .5))[0], -1.375));
    REQUIRE(isEqual(T0(R2(.5, .5))[1], -.75));

    // From physical element to reference element:

    // vertices to vertices
    REQUIRE(isEqual(T0.mapToReferenceElement(T0[0])[0], 0.));
    REQUIRE(isEqual(T0.mapToReferenceElement(T0[0])[1], 0.));

    REQUIRE(isEqual(T0.mapToReferenceElement(T0[1])[0], 1.));
    REQUIRE(isEqual(T0.mapToReferenceElement(T0[1])[1], 0.));

    REQUIRE(isEqual(T0.mapToReferenceElement(T0[2])[0], 1.));
    REQUIRE(isEqual(T0.mapToReferenceElement(T0[2])[1], 1.));

    REQUIRE(isEqual(T0.mapToReferenceElement(T0[3])[0], 0.));
    REQUIRE(isEqual(T0.mapToReferenceElement(T0[3])[1], 1.));

    // center to center
    REQUIRE(isEqual((T0.mapToReferenceElement(T0[0])[0]+T0.mapToReferenceElement(T0[1])[0])/2, .5));
    REQUIRE(isEqual((T0.mapToReferenceElement(T0[0])[1]+T0.mapToReferenceElement(T0[3])[1])/2, .5));

    // Measure edge length
    REQUIRE(isEqual(T0.mesureBord(0), 0.25));
    REQUIRE(isEqual(T0.mesureBord(1), 0.5));

    // Map point on unit interval to point on edge i on reference element
    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 0)[0], 0.25));
    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 0)[1], 0.));

    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 1)[0], 1.));
    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 1)[1], 0.25));

    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 2)[0], .75));
    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 2)[1], 1.));

    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 3)[0], 0.));
    REQUIRE(isEqual(T0.mapToReferenceElement(0.25, 3)[1], .75));
