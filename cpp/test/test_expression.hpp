...
TEST_CASE("Test Expression class for scalar functions", "[Expression]") {
    ...
    }
    {
        ...
        const auto q = fabs(p);
        REQUIRE(isEqual(q->eval(0, x1, nullptr), std::fabs(pow(f2(x1), 3)), 1e-12));
        REQUIRE(isEqual(q->eval(0, x2, nullptr), std::fabs(pow(f2(x2), 3)), 1e-12));

        const auto r = sqrt(q);
        REQUIRE(isEqual(r->eval(0, x1, nullptr), std::sqrt(std::fabs(pow(f2(x1), 3))), 1e-12));
        REQUIRE(isEqual(r->eval(0, x2, nullptr), std::sqrt(std::fabs(pow(f2(x2), 3))), 1e-12));
    }
    {
