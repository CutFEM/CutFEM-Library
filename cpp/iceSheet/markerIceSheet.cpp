...
void MarkerIceSheet::setBoundaryFace() {
   ...
   for (int k = 0; k < Th.nt; ++k) {
      ...
      if (isNotCut(k) && domain_sign(Th(k, 0)) > 0)
         continue; // only Omega2
      ...
      for (int i = 0; i < Element::ne; ++i) {
         ...
         if (k_next != -1)
            continue; // only want border element

         int ie[2] = {Element::nvedge[i][1], Element::nvedge[i][0]};

         if (domain_sign(Th(k, ie[0])) > 0 && domain_sign(Th(k, ie[1])) > 0)
            continue; // not cut && not in domain
