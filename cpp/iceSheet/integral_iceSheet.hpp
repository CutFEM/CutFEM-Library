 ...
 public:
   ...
   IntegralIceSheet(const IceSheet &pp)
       : // Integration((*pp.Vh)[0].NbDoF(),
         // pp.Vh->NbDoF(),
         // 2),
         Vh(*pp.Vh), problem(pp), qfb(*QF_Simplex<RdHatBord>(5)),
         qf(*QF_Simplex<RdHat>(5)), marker(*(Vh.marker[0])) {
      nbDoF = Vh[0].NbDoF();
   ...
   void locFullAssembly(const FElement &FK, Rnm &ML, Rn &VL, Rn &VP,
                        const Partition &cutK) {
         ...
         for (int ipq = 0; ipq < qf.n; ++ipq) {
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                  for (int jp = FK.dfcbegin(cp); jp < FK.dfcend(cp); ++jp) {
                     ML(i, jp) -=
                         Cint * w(jp, cp, op_id) * w(i, ci, MyOp::D(ci));
                     ML(jp, i) +=
                         Cint * w(jp, cp, op_id) * w(i, ci, MyOp::D(ci));
                  }
               }
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int j = FK.dfcbegin(ci); j < FK.dfcend(ci); ++j) {
                  // for(int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i){
                  VL(j) += Cint * rho_ * w(j, ci, op_id) *
                           problem.frhs(mip)[ci]; // w(j,op_id) * fxj(i);
               }
            }
            //   	}
   ...
   void locSurfAssembly(const FElement &FK, const int iface, Rnm &ML, Rn &VL) {
      ...
      const R h =
          Max(Vh[0].T.lenEdge(0), Vh[0].T.lenEdge(1), Vh[0].T.lenEdge(2));
      const R invh  = 1. / h;
      ...
      for (int ipq = 0; ipq < qfb.n; ++ipq) { // loop over integration points
         ...
         if (face.lab == symmetry || face.lab == bedRock) {
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                  Snn(i) = 2 * mu_ *
                           (w(i, ci, MyOp::D(0)) * normal[0] +
                            w(i, ci, MyOp::D(1)) * normal[1]) *
                           normal[ci];
                  Stn(i) = 2 * mu_ *
                           (w(i, ci, MyOp::D(0)) * normal[0] +
                            w(i, ci, MyOp::D(1)) * normal[1]) *
         ...
         if (face.lab == floating || face.lab == cliff) {
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                  VL(i) -=
                      Cint * w(i, ci, op_id) * problem.Pwater(mip) * normal[ci];
               }
            }
         }
         ...
         if (face.lab == symmetry) {
            ...
            for (int ci = 0; ci <= Rd::d; ++ci) {
               for (int cj = 0; cj < Rd::d; ++cj) {
                  for (int j = FK.dfcbegin(cj); j < FK.dfcend(cj); ++j) {
                     for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                        ML(i, j) -=
                            Cint * (Snn(i) * w(j, cj, op_id) * normal[cj]);
                        ML(j, i) -=
                            Cint * (Snn(i) * w(j, cj, op_id) * normal[cj] +
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int j = FK.dfcbegin(ci); j < FK.dfcend(ci); ++j) {
                  for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                     ML(i, j) +=
                         Cint * (Beta * w(i, ci, op_id) * w(j, ci, op_id) +
                                 1e3 * invh * w(i, ci, op_id) * normal[ci] *
                                     w(j, ci, op_id) * normal[ci]);
                  }
               }
            }
         }
         ...
         if (face.lab == bedRock) { //}|| face.lab == symmetry){// || face.lab
                                    //== floating || face.lab == cliff){
            ...
            for (int ci = 0; ci < Rd::d; ++ci) {
               for (int cj = 0; cj < Rd::d; ++cj) {
                  for (int j = FK.dfcbegin(cj); j < FK.dfcend(cj); ++j) {
                     for (int i = FK.dfcbegin(ci); i < FK.dfcend(ci); ++i) {
                        ML(i, j) += Cint * (Beta * w(i, ci, op_id) *
                                                w(j, cj, op_id) * (ci == cj) +
                                            1e3 * invh * invh *
                                                w(i, ci, op_id) * normal[ci] *
                                                w(j, cj, op_id) * normal[cj]);
                     }
                  }
               }
            }
   ...
   int locStabilization(KNM<R> &ML, KNM<R> &ML_M, KNM<R> &MLN,
      ...
      const R h =
          Max(Vh[0].T.lenEdge(0), Vh[0].T.lenEdge(1), Vh[0].T.lenEdge(2));
      int the_domain = 1; // FK.whichDomain();
      int k_back     = Vh.Th(FK.T);
      int ifacn      = ifac;
      int kn_back    = Vh.Th.ElementAdj(k_back, ifacn);
      if (kn_back == -1)
         return -1; // no neighboor

      int kn = Vh.idxElementFromBackMesh(
          kn_back, 0); // the_domain);   // not in the domain
      if (kn == -1)
         return -1;
   ...
   //       const R mes = cutK.mesure(it);
   //       for(int ipq = 0; ipq < qf.n; ++ipq)  {             // loop over
   //       integration points
