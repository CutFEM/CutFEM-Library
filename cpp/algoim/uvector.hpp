...
namespace algoim {
template <typename T, int N> class uvector;

template <int N, typename T> struct uvector_expr {
    T eval;
};

namespace detail {
template <typename T, int N> constexpr auto eval(const uvector<T, N> &e, int i) { return e(i); }

template <int N, typename T> constexpr auto eval(const uvector_expr<N, T> &expr, int i) { return expr.eval(i); }

template <typename E> constexpr auto eval(const E &e, int i) { return e; }

template <int N, typename T> constexpr auto make_uvector_expr(const T &eval) { return uvector_expr<N, T>{eval}; }

template <typename T> struct extent_of {
    static constexpr int value = 0;
};

template <typename T, int N> struct extent_of<uvector<T, N>> {
    static constexpr int value = N;
};

template <int N, typename T> struct extent_of<uvector_expr<N, T>> {
    static constexpr int value = N;
};

template <typename T> using extent = std::integral_constant<int, extent_of<std::decay_t<T>>::value>;
} // namespace detail
    ...
    auto operator op(E1 &&e1, E2 &&e2) {                                                                               \
        ...
        if constexpr (std::is_lvalue_reference<E1 &&>::value)                                                          \
            if constexpr (std::is_lvalue_reference<E2 &&>::value)                                                      \
                return detail::make_uvector_expr<N>(                                                                   \
                    [&e1, &e2](int i) { return detail::eval(e1, i) op detail::eval(e2, i); });                         \
            else                                                                                                       \
                return detail::make_uvector_expr<N>(                                                                   \
                    [&e1, e2](int i) { return detail::eval(e1, i) op detail::eval(e2, i); });                          \
...
algoim_decl_binary_op(+) algoim_decl_binary_op(-) algoim_decl_binary_op(*) algoim_decl_binary_op(/)
    algoim_decl_binary_op(<) algoim_decl_binary_op(<=) algoim_decl_binary_op(>) algoim_decl_binary_op(>=)
        algoim_decl_binary_op(==) algoim_decl_binary_op(!=) algoim_decl_binary_op(%)
...
template <typename E, std::enable_if_t<detail::extent<E>::value >= 1, bool> = true> auto abs(const E &e) {
    using std::abs;
    constexpr int N = detail::extent<E>::value;
    if constexpr (std::is_lvalue_reference<E &&>::value)
...
template <typename T, int N> class uvector {
  ...
  public:
    ...
    template <typename U> uvector(const U &x) { *this = x; }
...
#define algoim_decl_assign_op(op)                                                                                      \
    template <typename E> uvector &operator op(const E &x) {                                                           \
...
template <typename T> class uvector<T, 0> {};
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto min(const T &u) {
    constexpr int N = detail::extent<T>::value;
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto max(const T &u) {
    constexpr int N = detail::extent<T>::value;
    auto x          = detail::eval(u, 0);
    for (int i = 1; i < N; ++i) {
        auto y = detail::eval(u, i);
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> int argmin(const T &u) {
    constexpr int N = detail::extent<T>::value;
    auto x          = detail::eval(u, 0);
    int ind         = 0;
    for (int i = 1; i < N; ++i) {
...
// is attained at multiple places, the smallest corresponding index is returned
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> int argmax(const T &u) {
    constexpr int N = detail::extent<T>::value;
    auto x          = detail::eval(u, 0);
    int ind         = 0;
    for (int i = 1; i < N; ++i) {
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto sum(const T &u) {
    constexpr int N = detail::extent<T>::value;
    auto x          = detail::eval(u, 0);
    for (int i = 1; i < N; ++i)
        x += detail::eval(u, i);
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto prod(const T &u) {
    constexpr int N = detail::extent<T>::value;
    auto x          = detail::eval(u, 0);
    for (int i = 1; i < N; ++i)
        x *= detail::eval(u, i);
...
template <typename E1, typename E2,
          std::enable_if_t<detail::extent<E1>::value == detail::extent<E2>::value && detail::extent<E1>::value >= 1,
                           bool> = true>
...
template <typename T, int N> auto max(const uvector<T, N> &u, const uvector<T, N> &v) {
    using std::max;
    uvector<T, N> x;
    for (int i = 0; i < N; ++i)
        x(i) = max(u(i), v(i));
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> bool any(const T &u) {
    constexpr int N = detail::extent<T>::value;
    bool x          = false;
    for (int i = 0; i < N; ++i)
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> bool all(const T &u) {
    constexpr int N = detail::extent<T>::value;
    bool x          = true;
    for (int i = 0; i < N; ++i)
        x &= static_cast<bool>(detail::eval(u, i));
...
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto sqrnorm(const T &u) {
    return dot(u, u);
}

// Euclidean norm of a uvector or uvector expression
template <typename T, std::enable_if_t<detail::extent<T>::value >= 1, bool> = true> auto norm(const T &u) {
    using std::sqrt;
    return sqrt(sqrnorm(u));
}

// alter the extent of a uvector by truncation or zero backfill
template <int M, typename T, int N> uvector<T, M> alter_extent(const uvector<T, N> &u) {
    uvector<T, M> v;
...
template <typename T, int N> uvector<T, N - 1> remove_component(const uvector<T, N> &u, int index) {
    uvector<T, N - 1> v;
    if constexpr (N > 1)
        for (int i = 0; i < N - 1; ++i)
            v(i) = u(i < index ? i : i + 1);
...
template <typename T, int N> uvector<T, N + 1> add_component(const uvector<T, N> &u, int index, T value) {
    if constexpr (N == 0)
...
template <typename T, int N> uvector<T, N> set_component(uvector<T, N> u, int i, T value) {
    u(i) = value;
    return u;
}

// increment the i'th component of a uvector by a given value
template <typename T, int N> uvector<T, N> inc_component(uvector<T, N> u, int i, T value) {
    u(i) += value;
    return u;
}
} // namespace algoim
