...
template <typename M> class BaseCutProblem : public BaseProblem<M> {
 public:
   typedef M Mesh;
   typedef GFESpace<Mesh> FESpace;
   typedef typename FESpace::FElement FElement;
   ...
   BaseCutProblem(const FESpace &vh) : BaseProblem<M>(vh) {
      if (Vh->gamma.size() == 0) {
         std::cout << " There is no interface linked to your FESpace "
                   << std::endl;
         assert(0);
      }
   }
 ...
 public:
   ...
   void addBilinearFormExtDomain(const ListItemVF<Rd::d> &VF,
                                 const MacroElement &macro, const R epsE);
   void addLinearFormExtDomain(const ListItemVF<Rd::d> &VF, const R epsE = 0);
   void addLinearFormExtDomain(const ListItemVF<Rd::d> &VF,
                               const MacroElement &macro, const R epsE = 0);
   ...
   void addBilinear(const ListItemVF<Rd::d> &VF, const CFacet &b) {
      return BaseProblem<M>::addBilinear(VF, b);
   };
   void addBilinear(const ListItemVF<Rd::d> &VF, const CFacet &b,
                    const GMacro &macro) {
      return BaseProblem<M>::addBilinear(VF, b, macro);
   };
   void addLinear(const ListItemVF<Rd::d> &VF, const CFacet &b) {
      return BaseProblem<M>::addLinear(VF, b);
   };
   // face stabilization
   void addFaceStabilization(const ListItemVF<Rd::d> &VF);
   void addFaceStabilization(const ListItemVF<Rd::d> &VF,
                             const MacroElement &bigMac);
   // lagrange multiplier
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, double val,
                              const int itq = 0);

 protected:
   void addElementMat(const ListItemVF<Rd::d> &VF, const int k,
                      const bool extend = false, const R epsE = 0);
   void addElementRHS(const ListItemVF<Rd::d> &VF, const int k,
                      const bool extend = false, const R epsE = 0);
   void addElementLagrange(const ListItemVF<Rd::d> &VF, const int k,
                           const int itq);
   void addElementMatEdge(const ListItemVF<Rd::d> &VF, const int k,
                          const int ifac);
   void addElementRHSEdge(const ListItemVF<Rd::d> &VF, const int k,
                          const int ifac);
   void addElementMatEdge(KNMK<double> &basisFunMat,
                          const ListItemVF<Rd::d> &VF, const int k, int ifac);
   void addElementMatBorder(const ListItemVF<Rd::d> &VF, const int ifac,
                            int dom = 0);
   void addElementRHSBorder(const ListItemVF<Rd::d> &VF, const int ifac,
                            int dom = 0);
 ...
 public:
   ...
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, double val,
                              const TimeSlab &In);
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF, double val, int itq,
                              const TimeSlab &In);
 ...
 public:
   ...
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                              ...
                              const Mapping &mapping = DataMapping<Mesh>::Id) {
      return BaseProblem<M>::addLagrangeMultiplier(VF, gamma, val, mapping);
   };
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                              const Interface &gamma, const TimeSlab &In,
                              double tq, double val,
                              const Mapping &mapping = DataMapping<Mesh>::Id) {
      return BaseProblem<M>::addLagrangeMultiplier(VF, gamma, In, tq, val,
                                                   mapping);
   };
   ...
   void addBilinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma,
      ...
      return BaseProblem<M>::addBilinear(VF, gamma, In, mapping);
   };
   void addBilinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma,
                    int itq, const TimeSlab &In,
                    KN<const Mapping *> mapping = {}) {
      return BaseProblem<M>::addBilinear(VF, gamma, itq, In, mapping);
   };
   void addLinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma,
                  const TimeSlab &In, KN<const Mapping *> mapping = {}) {
      return BaseProblem<M>::addLinear(VF, gamma, In, mapping);
   };
   void addLinear(const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma,
                  int itq, const TimeSlab &In,
                  KN<const Mapping *> mapping = {}) {
      return BaseProblem<M>::addLinear(VF, gamma, itq, In, mapping);
   };
   void addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                              const TimeInterface<M> &gamma, const TimeSlab &In,
                              double val, KN<const Mapping *> mapping = {}) {
      return BaseProblem<M>::addLagrangeMultiplier(VF, gamma, In, val, mapping);
   };

 public:
   R computeCoef(const ItemVF<Rd::d> &, int,
                 const typename Mesh::Partition &cutK) const;
};

template <typename M>
R BaseCutProblem<M>::computeCoef(const ItemVF<Rd::d> &item, int domain,
                                 const typename Mesh::Partition &cutK) const {
   R val        = 1;
...
template <typename M>
void BaseCutProblem<M>::addBilinearFormExtDomain(const ListItemVF<Rd::d> &VF,
                                                 const R epsE) {
   bool extend = true;
   ...
   for (int k = Vh->first_element(); k < Vh->last_element();
      ...
      if (Vh->isCut(k)) {   // [makes sure we extend only cut elements]
         addElementMat(VF, k, extend,
                       epsE); // [add epsilon worth of other side, the supplied
                              // arguments limits to only cut elements]
      }
   }
}
...
template <typename M>
void BaseCutProblem<M>::addElementMat(const ListItemVF<Rd::d> &VF, const int k,
                                      const bool extend, const R epsE) {
   ...
   if (extend) {
      assert(domain !=
             -1); // [if for some reason gets called in standard no cut pb]
   ...
   for (typename Partition::const_element_iterator it =
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         const int ku =
             (VF[l].domu != -1)
                 ? VF[l].fespaceU->idxElementFromBackMesh(kb, VF[l].domu)
                 : k;
         const int kv =
             (VF[l].domv != -1)
                 ? VF[l].fespaceV->idxElementFromBackMesh(kb, VF[l].domv)
                 : k;
         ...
         for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
            ...
            const R Cint =
                (extend) ? epsE * meas * ip.getWeight() : meas * ip.getWeight();
            ...
            for (int j = FKu.dfcbegin(VF[l].cu); j < FKu.dfcend(VF[l].cu);
                 ++j) {
               for (int i = FKv.dfcbegin(VF[l].cv); i < FKv.dfcend(VF[l].cv);
                    ++i) {
                  // (*this)(FKv.loc2glb(i),FKu.loc2glb(j)) +=  Cint * coef *
                  // VF[l].c *
                  // fv(i,VF[l].cv,VF[l].dv)*fu(j,VF[l].cu,VF[l].du)*val;
                  this->addToLocalContribution(FKv.loc2glb(i),
                                               FKu.loc2glb(j)) +=
                      Cint * coef * VF[l].c * fv(i, VF[l].cv, VF[l].dv) *
                      fu(j, VF[l].cu, VF[l].du) * val;
               }
            }
         }
      }
...
template <typename M>
void BaseCutProblem<M>::addElementMatEdge(const ListItemVF<Rd::d> &VF,
                                          const int k, const int ifac) {
   typedef typename Mesh::Partition Partition;
   ...
   for (int l = 0; l < VF.size(); ++l) {
      ...
      // R coef = BaseProblem<M>::computeCoef(VF[l],h,meas, measK,the_domain) *
      // VF[l].getCoef(normal);
      R coef = BaseCutProblem<M>::computeCoefEdge(VF[l], h, meas, measK,
                                                  meas_cut, the_domain) *
               VF[l].getCoef(normal);
      ...
      for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
         ...
         const Rd mip = cutK.toEdge(ifac, (RdHatBord)ip,
                                    the_domain); // mip is in the global edge
...
template <typename M>
void BaseCutProblem<M>::addLinearFormExtDomain(const ListItemVF<Rd::d> &VF,
                                               const R epsE) {
   bool extend = true;
...
template <typename M>
void BaseCutProblem<M>::addElementRHS(const ListItemVF<Rd::d> &VF, const int k,
                                      const bool extend, const R epsE) {
   typedef typename Mesh::Partition Partition;
   typedef typename QF::QuadraturePoint QuadraturePoint;
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
            ...
            for (int i = FKv.dfcbegin(VF[l].cv); i < FKv.dfcend(VF[l].cv);
                 ++i) {
               (*this)(FKv.loc2glb(i)) +=
                   Cint * coef * val_fh * VF[l].c * fv(i, VF[l].cv, VF[l].dv);
            }
         }
      }
   }
...
template <typename M>
void BaseCutProblem<M>::addElementMatBorder(const ListItemVF<Rd::d> &VF,
                                            const int ifac, int dddd) {
   ...
   const int kb = Vh->Th.BoundaryElement(
       ifac, e); // index of element (triangle), ifaceK gets modified inside
   if (!Vh->containBackElement(kb))
      return;
   ...
   for (int dom = 0; dom < 2; ++dom) {
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
            ...
            const Rd mip =
                cutK.toEdge(e, (RdHatBord)ip, dom); // mip is in the global edge
...
template <typename M>
void BaseCutProblem<M>::addElementRHSBorder(const ListItemVF<Rd::d> &VF,
...
void BaseCutProblem<M>::addFaceStabilization(const ListItemVF<Rd::d> &VF,
   ...
   for (auto me = bigMac.macro_element.begin();
        me != bigMac.macro_element.end(); ++me) {
...
template <typename M>
void BaseCutProblem<M>::addElementRHSEdge(const ListItemVF<Rd::d> &VF,
                                          const int k, const int ifac) {
...
template <typename M>
void BaseCutProblem<M>::addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                                              double val, const int itq) {
...
template <typename M>
void BaseCutProblem<M>::addElementLagrange(const ListItemVF<Rd::d> &VF,
                                           const int k, const int itq) {
   typedef typename Mesh::Partition Partition;
   typedef typename QF::QuadraturePoint QuadraturePoint;
      ...
      for (typename Partition::const_element_iterator it =
         ...
         for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
            ...
            for (int j = FK.dfcbegin(VF[l].cv); j < FK.dfcend(VF[l].cv); ++j) {
               (*this)(nend, FK.loc2glb(j) + this->mapIdx0[&Wh]) +=
                   Cint * VF[l].c * fv(j, VF[l].cv, VF[l].dv);
               (*this)(FK.loc2glb(j) + this->mapIdx0[&Wh], nend) +=
                   Cint * VF[l].c * fv(j, VF[l].cv, VF[l].dv);
            }
         }
      }
...
template <typename M> void BaseCutProblem<M>::saveSolution(const Rn &sol) {
   ...
   for (typename std::map<const FESpace *, int>::const_iterator q =
            this->mapIdx0.begin();
        q != this->mapIdx0.end(); ++q) {
      ...
      // std::cout << id_domain_0 << "\t" << Wh.getNumberOfSubDomain() <<
      // std::endl;
...
template <typename M> void BaseCutProblem<M>::initialSolution(Rn &u0) {
      ...
      for (int k = 0; k < Wh.NbElement(); ++k) {
         ...
         if (the_part != NoElement) {
            for (int ic = 0; ic < Wh.N; ++ic) { // ESSAYER VH->N
               for (int i = FK.dfcbegin(ic); i < FK.dfcend(ic); ++i) {
                  u0S(FK(i)) =
                      this->mapU0[std::make_pair(id_domain, FKback(i))];
               }
            }
         }
      }
