...
template <typeMesh M> struct ItemTestFunction {
    ...
    std::vector<int> ar_nu,
        conormal; ///< arrays of normal and conormal components of test function //? is it which normal component this
                  ///< ItemTestFunction is from root test function? e.g. if ar_nu = {1}, then this ItemTestFunction is
                  ///< the normal component of root test function in direction y?
    ...
    const testfunction_t *root_fun_p =
        nullptr; ///< the root test function object     ///< pointer to the root test function
    ...
    void addParameter(const VirtualParameter &x) {
        coefu.push_back(&x);
    } ///< Adds a parameter multiple to the test function
    void addParameter(const VirtualParameter *x) {
        coefu.push_back(x);
    } ///< Adds a parameter multiple to the test function
...
template <typeMesh mesh_t>
TestFunction<mesh_t> operator*(const TestFunction<mesh_t> &T,
                               const std::vector<std::shared_ptr<ExpressionVirtual>> &fh) {
    return fh * T;
}
...
template <typeMesh mesh_t> TestFunction<mesh_t> operator*(ExpressionFunFEM<mesh_t> &fh, const TestFunction<mesh_t> &T) {
    ...
    std::vector<std::shared_ptr<ExpressionFunFEM<mesh_t>>> ff = {fh_p};
    return ff * T;
}
...
template <typeMesh mesh_t> TestFunction<mesh_t> grad(const TestFunction<mesh_t> &T) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int d = 0; d < D; ++d) {
            ...
            for (auto &item : new_list.U) {
                item.du =
                    nextDerivative(d, item.du); // take first derivative if test function not already differentiated,
...
template <typeMesh mesh_t> TestFunction<mesh_t> rotgrad(const TestFunction<mesh_t> &T) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int d = 0; d < D; ++d) {
            ...
            // If T is a scalar, we want to return a vector, so we push the differentiated item list into the row
            // corresponding to the physical dimension If T is a vector, we want to return a scalar, so we push the
            // differentiated item list into the same spot in the matrix
