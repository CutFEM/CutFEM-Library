...
template <typeMesh M> struct ItemTestFunction {
    ...
    const fespace_t *fespace                 = nullptr; ///< so that integration is performed on the right space
    const testfunction_t *root_fun_p         = nullptr; ///< the root test function object
...
template <typeMesh mesh_t> TestFunction<mesh_t> grad(const TestFunction<mesh_t> &T) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int d = 0; d < D; ++d) {
            ...
            auto new_list =
                T.getList(i, 0); // e.g. if T is a sum of two test functions, new_list will have two components

            // Iterate over the test functions in the item list
            for (auto &item : new_list.U) {
                item.du = nextDerivative(d, item.du); // take first derivative if test function not already
                                                      // differentiated, otherwise take second derivative
            }
            int irow =
                T.isScalar()
                    ? d
