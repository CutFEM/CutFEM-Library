...
template <int N = 2> struct ItemTestFunction {
   ...
   ItemTestFunction(const ItemTestFunction &F, void (*f)(RNMK_ &, int, int))
       : c(F.c), cu(F.cu), du(F.du), dtu(F.dtu), ar_nu(F.ar_nu),
         conormal(F.conormal), domain_id_(F.domain_id_),
         face_side_(F.face_side_), expru(F.expru), fespace(F.fespace), pfun(f) {
      for (int i = 0; i < F.coefu.size(); ++i)
         coefu.push_back(F.coefu[i]);
   }
   // ItemTestFunction(const FunFEM<Mesh>& ff, int ic) : c(1.),
   // cu(ic),du(op_id),dtu(0),domain_id_(-1),fespace(ff.Vh),alloc_expr(true){
   //   expru = new ExpressionFunFEM<Mesh>(ff, ic, op_id);
   // }
   // ItemTestFunction(const GFESpace<Mesh>& vh,  const ExpressionVirtual& ff) :
   // c(1.), cu(0),du(-1),dtu(0),domain_id_(-1),fespace(&vh){
   //   expru = &ff;
   // }
   ...
   ItemTestFunction &operator=(const ItemTestFunction &L) {
      ...
      dtu = L.dtu;
      ar_nu.init(L.ar_nu);
      conormal.init(L.conormal), domain_id_ = L.domain_id_;
...
template <int N = 2> class ItemList {
 ...
 public:
   ...
   ItemList(double cc, int i, int j, int dd) : U(1) {
      U(0) = new ItemTestFunction<N>(cc, i, j, 0, dd,
                                     std::vector<const VirtualParameter *>());
   }
   // ItemList(double cc,int i,int j,int tu, int dd, const vector<const
   // VirtualParameter*>& cuu) : U(1) {
   //   U(0) = new ItemTestFunction<N>(cc,i,j,tu,dd,cuu);
   // }

   ItemList(int l) : U(l) {
      for (int i = 0; i < l; ++i)
         U(i) = new ItemTestFunction<N>();
   }

   ItemList(const ItemList &L) : U(L.U.size()) {
      for (int i = 0; i < U.size(); ++i)
         U(i) = new ItemTestFunction<N>(*L.U(i));
   }

   ItemList(const ItemList &L, void (*f)(RNMK_ &, int, int)) : U(L.U.size()) {
      for (int i = 0; i < U.size(); ++i)
         U(i) = new ItemTestFunction<N>(*L.U(i), f);
   }
   ...
   ItemList(const FESpace &Vh, double cc, int i, int j, int dd) : U(1) {
      U(0)          = new ItemTestFunction<N>(cc, i, j, 0,
                                     dd); //,vector<const VirtualParameter*>());
      U(0)->fespace = &Vh;
   }
...
template <int dim = 2> class TestFunction {
 ...
 public:
   TestFunction(const FESpace &Vh, int d) {
      A.init(d, 1);
      for (int i = 0; i < d; ++i)
         A(i, 0) = new ItemList<dim>(Vh, 1, i, 0, -1);
   }
   TestFunction(const FESpace &Vh, int d, int comp0) {
      A.init(d, 1);
      for (int i = 0; i < d; ++i)
         A(i, 0) = new ItemList<dim>(Vh, 1, comp0 + i, 0, -1);
   }
   TestFunction(const FESpace &Vh, int d, int comp0, int domm) {
      A.init(d, 1);
      for (int i = 0; i < d; ++i)
         A(i, 0) = new ItemList<dim>(Vh, 1, comp0 + i, 0, domm);
   }
   ...
   // }
   // TestFunction(const FESpace& Vh, const ExpressionVirtual& ff, const
   // ExpressionVirtual& gg) {
   //   A.init(2,1);
   //   A(0,0) = new ItemList<dim> (Vh,ff);
   //   A(1,0) = new ItemList<dim> (Vh,gg);
   // }
   // TestFunction(const FESpace& Vh, const ExpressionVirtual& f1, const
   // ExpressionVirtual& f2, const ExpressionVirtual& f3, const
   // ExpressionVirtual& f4) {
   //   A.init(4,1);
   ...
   //   int n = nb_comp;
   //   A.init(n,1);
   //   for(int i=i0,j=0;i<i0+nb_comp;++i,++j)  A(j,0) = new ItemList<dim> (ff,
   //   i);
   // }
   ...
   template <int N>
   friend TestFunction<N> jump(const TestFunction<N> &U,
                               const TestFunction<N> &V);
   template <int N>
   friend TestFunction<N> jump(const TestFunction<N> &T, int c1, int c2);
   template <int N>
   friend TestFunction<N> jump(const TestFunction<N> &U,
                               const TestFunction<N> &V, int c1, int c2);

   template <int N>
   friend TestFunction<N> average(const TestFunction<N> &T, double v1,
                                  double v2);
   template <int N>
   friend TestFunction<N> average(const TestFunction<N> &T,
                                  const VirtualParameter &para,
   ...
   template <int N>
   friend TestFunction<N> operator-(const TestFunction<N> &F1,
                                    const TestFunction<N> &F2);
   template <int N>
   friend TestFunction<N> operator+(const TestFunction<N> &F1,
                                    const TestFunction<N> &F2);
   template <int N>
   friend TestFunction<N>
   operator*(std::list<ExpressionFunFEM<typename typeMesh<N>::Mesh>> fh,
             const TestFunction<N> &F2);
   template <int N>
   friend TestFunction<N> operator*(const CutFEM_Rd<N> &cc,
                                    const TestFunction<N> &T);
   template <int N>
   friend TestFunction<N> operator*(const typename typeRd<N>::Rd &cc,
                                    const TestFunction<N> &T);
   // template<int N> friend TestFunction<N> operator * (const
   // ExpressionAverage<typename typeMesh<N>::Mesh>& fh, const TestFunction<N>&
   // F2); template<int N> friend TestFunction<N> operator * (const
   // TestFunction<N>& F2, const ExpressionAverage<typename typeMesh<N>::Mesh>&
   // fh);
};

template <int N>
TestFunction<N> operator+(const TestFunction<N> &F1,
                          const TestFunction<N> &F2) {
...
// template <int N>
// TestFunction<N> operator * (const ExpressionAverage<typename
// typeMesh<N>::Mesh>& expr, const TestFunction<N>& F) {
...
// ExpressionAverage<typename typeMesh<N>::Mesh>& expr) {
//   assert(F.A.M() == 1 && F.A.N() == 1);
//   int l = F.A(0,0)->size();
//   TestFunction<N> multU(1);
//   multU.A(0,0) = new ItemList<N> (2*l);
...
template <int d>
TestFunction<d> jump(const TestFunction<d> &U, const TestFunction<d> &V, int c1,
                     int c2) {
