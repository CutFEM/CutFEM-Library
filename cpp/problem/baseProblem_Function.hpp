...
*/
template <typename M>
void BaseFEM<M>::addToMatrix(const ItemVF<Rd::d> &VFi, const FElement &FKu,
                             const FElement &FKv, const RNMK_ &fu,
...
template <typename M>
void BaseFEM<M>::addToMatrix(const ItemVF<Rd::d> &VFi, const TimeSlab &In,
                             const FElement &FKu, const FElement &FKv,
    ...
    for (int it = In.dfcbegin(0); it < In.dfcend(0); ++it) {
        for (int jt = In.dfcbegin(0); jt < In.dfcend(0); ++jt) {
            ...
            for (int i = FKv.dfcbegin(VFi.cv); i < FKv.dfcend(VFi.cv); ++i) {
                for (int j = FKu.dfcbegin(VFi.cu); j < FKu.dfcend(VFi.cu);
                     ++j) {
                    this->addToLocalContribution(
                        FKv.loc2glb(i, it), FKu.loc2glb(j, jt), thread_id) +=
                        Cint * tval * fv(i, VFi.cv, VFi.dv) *
                        fu(j, VFi.cu, VFi.du);
                }
            }
        }
...
template <typename M>
void BaseFEM<M>::addToMatrix(const ItemVF<Rd::d> &VFi, const FElement &FKv,
                             const RNMK_ &fv, double Cint) {

    for (int i = FKv.dfcbegin(VFi.cv); i < FKv.dfcend(VFi.cv); ++i) {
        this->addToLocalContribution(FKv.loc2glb(i), 0) +=
            Cint * fv(i, VFi.cv, VFi.dv);
    }
}
template <typename M>
void BaseFEM<M>::addToMatrix(const ItemVF<Rd::d> &VFi, const TimeSlab &In,
                             const FElement &FKv, const RNMK_ &fv,
                             double Cint) {
...
template <typename M>
void BaseFEM<M>::addToMatrix_Opt(const ItemVF<Rd::d> &VFi, const FElement &FK,
                                 const RNMK_ &fv, double Cint) {
...
template <typename M>
void BaseFEM<M>::addToRHS(const ItemVF<Rd::d> &VFi, const FElement &FKv,
                          const RNMK_ &fv, double Cint) {
    for (int i = FKv.dfcbegin(VFi.cv); i < FKv.dfcend(VFi.cv); ++i) {
...
template <typename M>
void BaseFEM<M>::addToRHS(const ItemVF<Rd::d> &VFi, const TimeSlab &In,
                          const FElement &FKv, const RNMK_ &fv, double Cint) {
    RNMK_ bf_time(this->databf_time_, In.NbDoF(), 1, op_dz);

    for (int it = In.dfcbegin(0); it < In.dfcend(0); ++it) {
        for (int i = FKv.dfcbegin(VFi.cv); i < FKv.dfcend(VFi.cv); ++i) {
            (*this)(FKv.loc2glb(i, it)) +=
                Cint * fv(i, VFi.cv, VFi.dv) * bf_time(it, 0, VFi.dtv);
        }
    }
}
...
template <typename M>
void BaseFEM<M>::addElementContribution(const ListItemVF<Rd::d> &VF,
...
template <typename M>
void BaseFEM<M>::addElementContribution_Opt(const ListItemVF<Rd::d> &VF,
                                            const int k, const TimeSlab *In,
                                            int itq, double cst_time) {
...
void BaseFEM<Mesh>::addBilinear(const ListItemVF<Rd::d> &VF, const Mesh &Th,
                                const CFacet &b) {
    assert(!VF.isRHS());
    progress bar("Add Bilinear InnerEdge", Th.last_element(),
                 globalVariable::verbose);
    for (int k = Th.first_element(); k < Th.last_element();
         k += Th.next_element()) {
        bar += Th.next_element();
...
template <typename M>
void BaseFEM<M>::addFaceContribution(const ListItemVF<Rd::d> &VF,
...
void BaseFEM<Mesh>::addBilinear(const ListItemVF<Rd::d> &VF, const Mesh &Th,
    ...
    for (int idx_be = Th.first_boundary_element();
        ...
        if (util::contain(label, BE.lab) || all_label) {
            BaseFEM<Mesh>::addBorderContribution(VF, K, BE, ifac, nullptr, 0,
                                                 1.);
            this->addLocalContribution();
        }
    }
...
template <typename M>
void BaseFEM<M>::addBorderContribution(const ListItemVF<Rd::d> &VF,
                                       const Element &K,
                                       const BorderElement &BE, int iiifac,
                                       const TimeSlab *In, int itq,
                                       double cst_time) {
            ...
            for (int ic = 0; ic < 1; ++ic) {
                for (int df = FK.dfcbegin(ic); df < FK.dfcend(ic); ++df) {
                    ...
                    if (id_item < K.nv) {
                        ...
                        // std::cout << " on edge  " <<FK.DFOnWhat(df) <<
                        // std::endl;
                        ...
                        // std::cout << " on face  " << FK.DFOnWhat(df) <<
                        // std::endl;
                    }
                }
            }
...
void BaseFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF,
    ...
    for (int iface = gamma[itq]->first_element();
        ...
        const typename Interface<M>::Face &face =
...
void BaseFEM<M>::addLinear(const Fct &f, const ListItemVF<Rd::d> &VF,
    ...
    for (int iface = gamma[itq]->first_element();
        ...
        if (util::contain(label, face.lab) || all_label) {
            ...
            addInterfaceContribution(f, VF, *gamma[itq], iface, tid, &In,
                                     cst_time, itq);
        }
    }
}
...
void BaseFEM<M>::addInterfaceContribution(const ListItemVF<Rd::d> &VF,
    ...
    for (int l = 0; l < VF.size(); ++l) {
        ...
        std::vector<int> idxV =
            Vhv.idxAllElementFromBackMesh(kb, VF[l].get_domain_test_function());
        std::vector<int> idxU =
            (same) ? idxV
        ...
        double coef =
            VF[l].computeCoefInterface(h, meas, measK, measCut, {domu, domv});
        coef *= VF[l].computeCoefFromNormal(normal);
        ...
        for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
            ...
            Cint *= VF[l].evaluateFunctionOnBackgroundMesh(
                std::make_pair(kb, kb), std::make_pair(domu, domv), mip, tid,
                normal);
            Cint *= coef * VF[l].c;
...
template <typename M>
template <typename Fct>
void BaseFEM<M>::addInterfaceContribution(
    const Fct &f, const ListItemVF<Rd::d> &VF, const Interface<M> &interface,
...
void BaseFEM<M>::addInterfaceContribution(const ListItemVF<Rd::d> &VF,
                                          ...
                                          int ifac, double tid,
                                          const TimeSlab *In, double cst_time,
                                          int itq, const Mapping<M> &mapping) {
    typedef typename FElement::RdHatBord RdHatBord;
    ...
    for (int l = 0; l < VF.size(); ++l) {
        ...
        for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
            ...
            Cint *= VF[l].evaluateFunctionOnBackgroundMesh(
                std::make_pair(kb, kb), std::make_pair(domu, domv), map_mip,
                tid, normal);
...
// LAGRANGE MULTIPLIER
template <typename Mesh>
void BaseFEM<Mesh>::addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                                          double val, const Mesh &Th) {
    assert(VF.isRHS());
...
template <typename M>
void BaseFEM<M>::addLagrangeContribution(const ListItemVF<Rd::d> &VF,
                                         const int k, const TimeSlab *In,
                                         int itq, double cst_time) {
...
template <typename M>
void BaseFEM<M>::addLagrangeBorderContribution(const ListItemVF<Rd::d> &VF,
                                               const Element &K,
