...
void BaseFEM<M>::addToMatrix(const ItemVF<Rd::d> &VFi, const TimeSlab &In,
   ...
   for (int it = In.dfcbegin(0); it < In.dfcend(0); ++it) {
      for (int jt = In.dfcbegin(0); jt < In.dfcend(0); ++jt) {
         ...
         for (int i = FKv.dfcbegin(VFi.cv); i < FKv.dfcend(VFi.cv); ++i) {
            for (int j = FKu.dfcbegin(VFi.cu); j < FKu.dfcend(VFi.cu); ++j) {
               this->addToLocalContribution(FKv.loc2glb(i, it),
                                            FKu.loc2glb(j, jt), thread_id) +=
   ...
   for (int l = 0; l < VF.size(); ++l) {
      ...
      for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
         ...
         Cint *= VF[l].evaluateFunctionOnBackgroundMesh(
             std::make_pair(kbu, kbv), std::make_pair(domain, domain), mip, tid,
             normal);
         Cint *= coef * VF[l].c;
...
void BaseFEM<M>::addLinear(const Fct &f, const ListItemVF<Rd::d> &VF, const TimeInterface<M> &gamma, const TimeSlab &In,
   ...
   for (int iface = gamma[itq]->first_element();
      ...
      if (util::contain(label, face.lab) || all_label) {
         ...
         addInterfaceContribution(f, VF, *gamma[itq], iface, tid, &In, cst_time,
                                  itq);
      }
   }
}
...
template <typename M>
template <typename Fct>
void BaseFEM<M>::addInterfaceContribution(const Fct &f, const ListItemVF<Rd::d> &VF,
                                          const Interface<M> &interface,
                                          int ifac, double tid,
                                          const TimeSlab *In, double cst_time,
                                          int itq) {
...
void BaseFEM<M>::addInterfaceContribution(const ListItemVF<Rd::d> &VF,
   ...
   for (int l = 0; l < VF.size(); ++l) {
      ...
      for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
         ...
         Cint *= VF[l].evaluateFunctionOnBackgroundMesh(
             std::make_pair(kb, kb), std::make_pair(domu, domv), map_mip, tid,
             normal);
