...
#pragma omp parallel default(shared)
   {
      ...
      progress bar(" Add Bilinear CutMesh", Th.last_element(), verbose,
                   this->get_nb_thread());
#pragma omp for
...
#pragma omp parallel default(shared)
   {
      ...
      int verbose       = (thread_id == 0) * globalVariable::verbose;
      progress bar(title.c_str(), Th.last_element(), verbose,
                   this->get_nb_thread());
...
template <typename M>
void BaseCutFEM<M>::addElementContribution(const ListItemVF<Rd::d> &VF,
                                           const int k, const TimeSlab *In,
   ...
   for (auto it = cutK.element_begin(); it != cutK.element_end(); ++it) {
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         RNMK_ fu(this->databf_ + offset +
                      (same ? 0 : FKv.NbDoF() * FKv.N * lastop),
                  FKu.NbDoF(), FKu.N, lastop); //  the value for basic fonction
         What_d Fop = Fwhatd(lastop);

         // COMPUTE COEFFICIENT
         double coef =
             VF[l].computeCoefElement(h, meas_cut, meas, meas_cut, domain);
         ...
         for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
            ...
            Rd mip =
                cutK.mapToPhysicalElement(it, ip); // to the physical cut part
            Rd cut_ip = K.mapToReferenceElement(
                mip); // back to the cut part in reference element
            ...
            Cint *=
                VF[l].evaluateFunctionOnBackgroundMesh(kb, domain, mip, tid);
            Cint *= coef * VF[l].c;
...
template <typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF, const CutMesh &Th,
                                const CExtension &ext, const int espE) {
   assert(!VF.isRHS());
   for (int k = Th.first_element(); k < Th.last_element();
        k += Th.next_element()) {
      ...
      if (Th.isCut(k, 0)) {
         BaseCutFEM<M>::addElementContribution(VF, k, nullptr, 0, 1.);
         BaseCutFEM<M>::addElementContributionOtherSide(VF, k, nullptr, 0,
                                                        espE);
      } else
         BaseFEM<M>::addElementContribution(VF, k, nullptr, 0, 1.);
...
template <typename M>
void BaseCutFEM<M>::addElementContributionOtherSide(const ListItemVF<Rd::d> &VF,
                                                    const int k,
                                                    const TimeSlab *In, int itq,
                                                    double cst_time) {
   ...
   // LOOP OVER ELEMENTS IN THE CUT
   for (auto it = cutK.other_side_element_begin();
        it != cutK.other_side_element_end(); ++it) {
...
template <typename M>
void BaseCutFEM<M>::addFaceContribution(const ListItemVF<Rd::d> &VF,
   ...
   const Cut_Part<typename Element::Face> cutFace(
       Th.get_cut_face(face, ki, ifac, itq));
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         for (int ipq = 0; ipq < qfb.getNbrOfQuads(); ++ipq) {
            ...
            Cint *= VF[l].evaluateFunctionOnBackgroundMesh(
                std::make_pair(kbu, kbv), std::make_pair(domain, domain), mip,
                tid, normal);
            Cint *= coef * VF[l].c;
            if (In) {
               if (VF.isRHS())
...
template <typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF,
                                const CutMesh &cutTh, const CBorder &b,
                                list<int> label) {
   assert(!VF.isRHS());
   ...
   for (int idx_be = cutTh.first_boundary_element();
      ...
      if (util::contain(label, BE.lab) || all_label) {
         ...
         if (cutTh.isCutFace(idxK[0], ifac, 0))
            BaseCutFEM<M>::addBorderContribution(VF, K, BE, ifac, nullptr, 0,
                                                 1.);
         else {
            if (idxK.size() == 1) {
               BaseFEM<M>::addBorderContribution(VF, K, BE, ifac, nullptr, 0,
                                                 1.);
            } else {
               ...
               BaseFEM<M>::addBorderContribution(
                   VF, K, BE, ifac + id_sub * Element::nea, nullptr, 0, 1.);
            }
         }
         this->addLocalContribution();
      }
...
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF,
   ...
   progress bar(title.c_str(), cutTh.last_boundary_element(),
                globalVariable::verbose);
   ...
   for (int idx_be = cutTh.first_boundary_element();
      ...
      if (util::contain(label, BE.lab) || all_label) {
         ...
         if (cutTh.isCutFace(idxK[0], ifac, itq)) {
            BaseCutFEM<M>::addBorderContribution(VF, K, BE, ifac, &In, itq,
                                                 cst_time);
         } else {
            if (idxK.size() == 1) {
               BaseFEM<M>::addBorderContribution(VF, K, BE, ifac, &In, itq,
                                                 cst_time);
            } else if (!cutTh.isCut(idxK[0], itq)) {
               int id_sub = (cutTh.isInactive(idxK[0], itq)) ? 1 : 0;
               BaseFEM<M>::addBorderContribution(
                   VF, K, BE, ifac + id_sub * Element::nea, &In, itq, cst_time);
            } else {
               assert(cutTh.get_nb_domain() < 3);
...
template <typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d> &VF, const CutMesh &cutTh,
...
template <typename M>
void BaseCutFEM<M>::addBorderContribution(const ListItemVF<Rd::d> &VF,
                                          const Element &K,
   ...
   for (int e = 0; e < Ne; ++e) {
      ...
      for (auto it = cutFace.element_begin(); it != cutFace.element_end();
         ...
         for (int l = 0; l < VF.size(); ++l) {
            ...
            // COMPUTE COEFFICIENT && NORMAL
            double coef =
                VF[l].computeCoefElement(h, meas, measK, meas_cut, domain);
            coef *= VF[l].computeCoefFromNormal(normal);
...
void BaseCutFEM<Mesh>::setDirichlet(const FunFEM<Mesh> &gh,
                                    const CutMesh &cutTh, list<int> label) {
...
void BaseCutFEM<Mesh>::removeDofForHansbo(const FESpace &Vh) {
   ...
   assert(Vh.basisFctType == BasisFctType::P0 ||
          Vh.basisFctType == BasisFctType::P1dc);
...
template <typename M>
void BaseCutFEM<M>::addBilinear(const ListItemVF<Rd::d> &VF,
                                const TimeInterface<M> &interface,
                                const CRidge &innerRidge, const TimeSlab &In,
...
template <typename M>
void BaseCutFEM<M>::addLinear(const ListItemVF<Rd::d> &VF,
                              const Interface<M> &gamma,
                              const CRidge &innerRidge, list<int> label) {
   assert(VF.isRHS());
   bool all_label = (label.size() == 0);
...
void BaseCutFEM<M>::addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                                          double val, const CutMesh &Th) {
   assert(VF.isRHS());
   int ndf = this->rhs_.size();
   this->rhs_.resize(ndf + 1);
   this->rhs_(ndf) = val;
   progress bar(" Add Lagrange Multiplier Kh", Th.last_element(),
                globalVariable::verbose);
...
void BaseCutFEM<M>::addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
   ...
   double cst_time = tq.a * In.get_measure();
   std::string title =
       " Add Lagrange Multiplier Kh, In(" + to_string(itq) + ")";
   progress bar(title.c_str(), Th.last_element(), globalVariable::verbose);
...
template <typename M>
void BaseCutFEM<M>::addLagrangeContribution(const ListItemVF<Rd::d> &VF,
                                            const int k, const TimeSlab *In,
                                            int itq, double cst_time) {
...
template <typename M>
void BaseCutFEM<M>::addLagrangeMultiplier(const ListItemVF<Rd::d> &VF,
                                          double val, const CutMesh &cutTh,
                                          const CBorder &b, list<int> label) {
   assert(VF.isRHS());
   bool all_label = (label.size() == 0);
   ...
   for (int idx_be = cutTh.first_boundary_element();
      ...
      if (util::contain(label, BE.lab) || all_label) {
         ...
         // CHECK IF IT IS A CUT EDGE
         if (cutTh.isCutFace(idxK[0], ifac, 0))
            BaseCutFEM<M>::addLagrangeBorderContribution(VF, K, BE, ifac,
                                                         nullptr, 0, 1.);
         else
            BaseFEM<M>::addLagrangeBorderContribution(VF, K, BE, ifac, nullptr,
                                                      0, 1.);
         this->addLocalContributionLagrange(ndf);
      }
   }
...
template <typename M>
void BaseCutFEM<M>::addLagrangeBorderContribution(const ListItemVF<Rd::d> &VF,
                                                  const Element &K,
                                                  const BorderElement &BE,
                                                  int ifac, const TimeSlab *In,
                                                  int itq, double cst_time) {
...
template <typename M>
void BaseCutFEM<M>::addLagrangeContributionOtherSide(
    const ListItemVF<Rd::d> &VF, const int k, const int epsE) {
