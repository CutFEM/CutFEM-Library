...
double integralCut(const ListItemVF<Mesh::Rd::d> &VF, const FunFEM<Mesh> &f,
                   const FunFEM<Mesh> &g, const ActiveMesh<Mesh> &Th) {
   ...
   for (int k = Th.first_element(); k < Th.last_element();
      ...
      for (auto it = cutK.element_begin(); it != cutK.element_end(); ++it) {
         ...
         for (int l = 0; l < VF.size(); ++l) {
            ...
            double coef =
                1.; // VF[l].computeCoefElement(h,meas,measK, meas, domain);
            ...
            for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
               ...
               double val_fh =
                   f.evalOnBackMesh(kb, domain, mip, VF[l].cu, VF[l].du);
               double val_gh =
                   g.evalOnBackMesh(kb, domain, mip, VF[l].cv, VF[l].dv);
