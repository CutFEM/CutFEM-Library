...
void move_2D(const FunFEM<Mesh> &up, const FunFEM<Mesh> &Betap,
             const FunFEM<Mesh> &Beta, double dt, FunFEM<Mesh> &ls) {
   typedef GFESpace<Mesh> FESpace;
   typedef typename FESpace::FElement FElement;
         ...
         for (int i = FK.dfcbegin(0); i < FK.dfcend(0); ++i) {
            for (int j = FK.dfcbegin(0); j < FK.dfcend(0); ++j) {
               levelSet(FK.loc2glb(i), FK.loc2glb(j)) +=
                   Cint *
                   (((1. / dt) * fu(j, 0, op_id) +
                     0.5 * (Bx * fu(j, 0, op_dx) + By * fu(j, 0, op_dy))) *
                    (fu(i, 0, op_id) +
                     Tsd * (Bx * fu(i, 0, op_dx) + By * fu(i, 0, op_dy))));
            }
...
template <typename Mesh>
void move_3D(const FunFEM<Mesh> &up, const FunFEM<Mesh> &Betap,
             const FunFEM<Mesh> &Beta, double dt, FunFEM<Mesh> &ls) {
   typedef GFESpace<Mesh> FESpace;
   typedef typename FESpace::FElement FElement;
   typedef typename FESpace::Rd Rd;
   ...
   for (int k = Vh.first_element(); k < Vh.last_element();
      ...
      for (int ipq = 0; ipq < qf.getNbrOfQuads(); ++ipq) {
         ...
         for (int i = FK.dfcbegin(0); i < FK.dfcend(0); ++i) {
            ...
            levelSet(FK.loc2glb(i)) +=
                Cint * (((1. / dt) * Up -
                         0.5 * (Bpx * dxup + Bpy * dyup + Bpz * dzup)) *
                        (fu(i, 0, op_id) +
                         Tsd * (Bx * fu(i, 0, op_dx) + By * fu(i, 0, op_dy) +
                                Bz * fu(i, 0, op_dz))));
         }
...
void move(const FunFEM<Mesh2> &up, const FunFEM<Mesh2> &betap,
          const FunFEM<Mesh2> &beta, double dt, FunFEM<Mesh2> &ls) {
   move_2D<Mesh2>(up, betap, beta, dt, ls);
}
// KN<double> move(const FunFEM<MeshQ2> &, const FunFEM<MeshQ2> &, const
// FunFEM<MeshQ2> &, double);
void move(const FunFEM<Mesh3> &up, const FunFEM<Mesh3> &betap,
          const FunFEM<Mesh3> &beta, double dt, FunFEM<Mesh3> &ls) {
   move_3D<Mesh3>(up, betap, beta, dt, ls);
}
void move(const FunFEM<MeshHexa> &up, const FunFEM<MeshHexa> &betap,
          const FunFEM<MeshHexa> &beta, double dt, FunFEM<MeshHexa> &ls) {
