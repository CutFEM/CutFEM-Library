...
template <class Mesh> void CReinitialization<Mesh>::perform(Fun &levelSet_P1) {
   ...
   if (ON_OFF == "ON") {
      ...
      // std::cout << " area before reinitialization ->\t" << areaInitial  <<
      // std::endl;

      Reinitialization<Mesh> reinitialization(
          levelSet_P1, interface, number_iteration, epsilon_diffusion,
          ODE_method, dt);

      Interface interface0(Vh.Th, levelSet_P1.v);
      double areaBefore = computeArea(*levelSet_P1.Vh, interface0);
      // std::cout << " area before correction      ->\t" << areaBefore  <<
      // std::endl;
      ...
      if (mass_correction == "ON") {
         ...
         // std::cout << " area after correction      ->\t" << areaFinal  <<
         // std::endl;
      }
   }
}
...
void CReinitialization<Mesh>::perform(Fun &levelSet_k, Fun &levelSet_P1) {
   ...
   if (ON_OFF == "ON") {
      ...
      Reinitialization<Mesh> reinitialization(
          levelSet_k, interface, number_iteration, epsilon_diffusion,
          ODE_method, dt);
...
template <class Mesh>
double CReinitialization<Mesh>::correction(Fun &levelSet,
                                           const double areaInitial) {
   ...
   for (int i = 0; i < max_iteration_correction; ++i) {
      ...
      if (i == max_iteration_correction - 1 &&
          fabs(diffInitial) > fabs(diff2)) {
         std::cout
             << " Correction of levelSet not reached the required precision ("
             << dA << ")" << std::endl;
         std::cout << " Mass correction error \t" << diff2 << std::endl;
         return dz2;
      }
...
template <class Mesh>
double CReinitialization<Mesh>::computeArea(const FESpace &Vh,
                                            const Interface &gamma) {
