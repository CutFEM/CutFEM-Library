...
class VirtualParameter {
 ...
 public:
   virtual double evaluate(int domain, double h, double meas, double measK,
                           double meas_Cut) const = 0;
};

struct MeshParameter : public VirtualParameter {
   int a1, a2, a3, a4;
   MeshParameter(int b1, int b2, int b3, int b4)
       : a1(b1), a2(b2), a3(b3), a4(b4) {}
   double evaluate(int domain, double h, double meas, double measK,
                   double meas_Cut) const {
...
template <int N> struct CutFEM_Rd {
   ...
   const CutFEMParameter &get_parameter(int i) const { return p[i]; }
};
...
class SumDiff_Parameter : public VirtualParameter {
 ...
 public:
   SumDiff_Parameter(double a, const VirtualParameter &A, double b,
...
class Mult_Parameter : public VirtualParameter {
 ...
 public:
   Mult_Parameter(const VirtualParameter &A, const VirtualParameter &B)
       : A_(&A), B_(&B) {}
   double evaluate(int domain, double h, double meas, double measK,
                   double meas_cut) const {
      double val1 = A_->evaluate(domain, h, meas, measK, meas_cut);
