...
template <typename Mesh> class Mapping {
 public:
 ...
 public:
   ...
   Mapping(const CutSpace &VVh, const Fun_h &levelSetk, const Fun_h &levelSet1)
       : Vh(VVh), Th(Vh.cutTh), levelSet_Pk(levelSetk), levelSet_P1(levelSet1) {
      assert(Vh.N == Rd::d);
      assert(&(levelSetk.Vh->Th) == &(Vh.Th));

      deformation_.resize(Vh.NbDoF());
      deformation_ = 0.;
      ni_.resize(Vh.nbNode);
      ni_ = static_cast<Ubyte>(0);
 ...
 private:
   void make_mapping();
   void searchCorrespondingPoint(const int k, const Rd &init_point,
                                 const R goal_val, const Rd &search_dir,
                                 Rd &final_point) const;
 ...
 public:
   ...
   void transform(const FElement &FK, KNMK_<double> &bf,
                  const KNM_<double> &invJ) const {
      if (bf.K() == 1)
         return;
      int op[3] = {op_dx, op_dy, op_dz};
      KNMK<R> x(bf.N(), bf.M(), bf.K());
      x = bf;
...
template <typename M> void Mapping<M>::make_mapping() {
   int D[3] = {op_dx, op_dy, op_dz};
   ...
   for (int k = 0; k < Vh.NbElement(); ++k) {
      ...
      for (int l = Rd::d + 1; l < nbNode; ++l) {
         ...
         for (int i = 0; i < Rd::d; ++i)
            normal[i] = levelSet_Pk.eval(kb, mip, 0, D[i]);
...
template <typename M>
void Mapping<M>::searchCorrespondingPoint(const int k, const Rd &init_point,
                                          const R goal_val,
                                          const Rd &search_dir,
                                          Rd &final_point) const {
