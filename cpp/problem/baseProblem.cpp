...
template <>
void BaseCutFEM<Mesh2>::addInterfaceRidgeContribution(const itemVFlist_t &VF, const Interface<Mesh2> &interface,
    ...
    for (int inode = 0; inode < 2; ++inode) {
        ...
        for (int l = 0; l < VF.size(); ++l) {
            ...
            std::vector<int> idxU =
                (same) ? idxV : Vhu.idxAllElementFromBackMesh(kbu, VF[l].get_domain_trial_function());
            ...
            coef *= VF[l].evaluateFunctionOnBackgroundMesh(std::make_pair(kbu, kbv), std::make_pair(domu, domv), mip,
                                                           0., std::make_pair(conormalu, conormalv));
            coef *= VF[l].c;
