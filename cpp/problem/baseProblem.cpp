...
template <>
void BaseCutFEM<Mesh2>::addInterfaceRidgeContribution(
    const ListItemVF<Rd::d> &VF, const Interface<Mesh2> &interface, int ifac,
   ...
   for (int inode = 0; inode < 2; ++inode) {
      ...
      Rd conormal0 =
          interface(face0[inode]) - interface(face0[(inode + 1) % 2]);
      Rd conormal1 =
          interface(face1[inode1]) - interface(face1[(inode1 + 1) % 2]);
      ...
      for (int l = 0; l < VF.size(); ++l) {
         ...
         std::vector<int> idxV = Vhv.idxAllElementFromBackMesh(
             kbv, VF[l].get_domain_test_function());
         std::vector<int> idxU =
             (same) ? idxV
                    : Vhu.idxAllElementFromBackMesh(
                          kbu, VF[l].get_domain_trial_function());
         ...
         const Rd normalu =
             (same) ? normalv
                    : VF[l].onWhatElementIsTrialFunction(normal0, normal1);
         const Rd conormalv =
             VF[l].onWhatElementIsTestFunction(conormal0, conormal1);
         const Rd conormalu =
             (same) ? conormalv
                    : VF[l].onWhatElementIsTrialFunction(conormal0, conormal1);
         ...
         double coef = cst_time; // VF[l].computeCoefInterface(h,meas,measK,measK,domu,
                                 // domv) ;
         ...
         coef *= VF[l].evaluateFunctionOnBackgroundMesh(
             std::make_pair(kbu, kbv), std::make_pair(domu, domv), mip, 0.,
             std::make_pair(conormalu, conormalv));
         coef *= VF[l].c;

         if (In) {
            if (VF.isRHS())
               this->addToRHS(VF[l], *In, FKv, fv, coef);
            else
               this->addToMatrix(VF[l], *In, FKu, FKv, fu, fv, coef);
         } else {
            if (VF.isRHS())
               this->addToRHS(VF[l], FKv, fv, coef);
            else
               this->addToMatrix(VF[l], FKu, FKv, fu, fv, coef);
         }
      }
   }
}
