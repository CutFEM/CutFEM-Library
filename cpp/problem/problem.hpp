...
template <typename Mesh> class ShapeOfProblem {
 ...
 public:
   ShapeOfProblem() : nb_dof_(0), nb_dof_time_(1), thread_count_max_(1) {
 ...
 public:
   ...
   void set_map(Matrix &A) {
      assert(thread_count_max_ == 1 &&
             " There are multiple threads. You cannot set only one matrix");
      pmat_.resize(0);
      pmat_.push_back(&A);
   }
   void set_map(std::vector<Matrix> &M) {
      assert(thread_count_max_ <= M.size() &&
             " There are multiple threads. Not good size of vector");
      pmat_.resize(0);
      for (auto &A : M) {
         pmat_.push_back(&A);
      }
 ...
 protected:
   ...
   void addLocalContribution() {
      ...
      for (const auto &aij : local_contribution_matrix_[thread_id]) {
         ...
         A[std::make_pair(aij.first.first + index_i0_[thread_id],
                          aij.first.second + index_j0_[thread_id])] +=
             aij.second;
      }
      // std::cout << thread_id << "\t" <<
   ...
   void addLocalContributionLagrange(int nend) {
      this->index_j0_[0] = 0;
      this->index_i0_[0] = 0;
      for (auto q = local_contribution_matrix_[0].begin();
           q != this->local_contribution_matrix_[0].end(); ++q) {
...
template <typename Mesh> class QuadratureOfProblem {
 ...
 public:
   ...
   QuadratureOfProblem(const QFT &qt, const ProblemOption &option)
       : time_quadrature_formular(&qt) {
      order_space_element_quadrature_ = option.order_space_element_quadrature_;
      order_space_bord_quadrature_    = option.order_space_bord_quadrature_;
   }
   ...
   QPT get_quadrature_time(int itq) const {
      ...
      return (time_quadrature_formular) ? time_quadrature_formular->at(itq)
                                        : QPT(1., 0.);
   }
};
