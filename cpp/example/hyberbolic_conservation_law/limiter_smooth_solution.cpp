...
double fun_solution(R2 P, int elementComp, int domain, double t) { return sin(pi * (P[0] + P[1] - 4 * t)); }
...
double solve() {
    ...
    double dt       = meshSize / 3 / sqrt(10) * 0.5;
    double tend     = 0.1;
    ...
    for (int i = 0; i < niteration; ++i) {
        ...
        // PLOT THE SOLUTION
        // ==================================================
        if (MPIcf::IamMaster() && i % 50 == 0 || i + 1 == niteration) {

            Paraview<mesh_t> writer(Khi, "limiter_smooth_solution_" + std::to_string(ifig++) + ".vtk");
