...
#error "No stabilization defined"
    ...
    // Arrays to hold data
    std::array<double, iterations> hs, dts, errors, errors_surf, errors_T, errors_T_surf, global_conservation_errors;
    ...
    for (int j = 0; j < iterations; ++j) {
        ...
        while (iter < total_number_iteration) {
            ...
            // Surface active mesh
            activemesh_t ThGamma(Th);
            ThGamma.createSurfaceMesh(interface);

            // Surface FE space
            cut_fespace_t WhGamma(ThGamma, Vh);
            ...
            // Test functions
            fun_test_t uB(Wh, 1), uS(WhGamma, 1), vB(Wh, 1), vS(WhGamma, 1);
            ...
            while (1) {
                ...
                if (newton_iterations == 0) {
                    ...
                    if (stabilization == "fullstab") {
                        ...
                        AlgoimMacroSurface<mesh_t, Levelset<2>> TimeMacroSurf(ThGamma, delta_surf, phi, In, qTime);
                        ...
                        std::cerr << "No stabilization chosen!\n";
                        exit(1);
                    }
